...
! Initialize arrays for the FARSITE head fire spread rate.
...
DO JJG=1,JBAR
   DO IIG=1,IBAR
      ...
      ELSE IF_CFD_COUPLED  ! The wind velocity is specified by the user
         ...
         CALL GET_WIND_AT_HEIGHT(T,REF_WIND_HEIGHT,U_LS_INST,V_LS_INST,WW)
...
REAL(EB), PARAMETER :: N_EFOLD_LS = 5._EB  !< Cutoff after fill + N e-foldings for exponential burnout
...
IF (SF%VEG_LSET_EXPONENTIAL_BURNOUT) THEN
   ! Convolution: TAU_LS is cell fill time; FIREBASE_TIME is e-folding burnout time t_f
   B2%TAU_LS = CROSSING_DISTANCE/ROS_MAG
   B1%BURN_DURATION = B2%TAU_LS + N_EFOLD_LS*SF%VEG_LSET_FIREBASE_TIME
ELSE
   ! Trapezoidal HRR: fire depth vs cell size sets ramp/plateau times
