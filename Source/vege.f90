...
REAL(EB) :: UMF_TMP,PHX,PHY,MAG_PHI,PHI_W_X,PHI_W_Y,UMF_X,UMF_Y,UMAG,ROS_MAG,UMF_MAG,ROTH_FACTOR,&
            SIN_THETA,COS_THETA,THETA,U_Z(2),V_Z(2),ZWIND(2),Z_WIND_REF
...
DO JJG=1,JBAR
   DO IIG=1,IBAR
      ...
      IF (.NOT. SF%VEG_LSET_SPREAD) CYCLE
         ...
         ! Find wind at ~6.1 m height for Farsite-like approach or locally 
         IF (LEVEL_SET_WIND_ALOFT) THEN
            Z_WIND_REF = 6.1_EB
         ELSE
            Z_WIND_REF = 2._EB*SF%VEG_LSET_HT
         ENDIF
      ...
      ELSE IF_CFD_COUPLED  ! The wind velocity is specified by the user
         ...
         IF (I_RAMP_DIRECTION_T/=0 .OR. I_RAMP_DIRECTION_Z/=0) THEN
            IF (I_RAMP_DIRECTION_T==0) THEN
               THETA=EVALUATE_RAMP(Z_WIND_REF,I_RAMP_DIRECTION_Z)*DEG2RAD
            ELSEIF (I_RAMP_DIRECTION_Z==0) THEN
               THETA=EVALUATE_RAMP(T,I_RAMP_DIRECTION_T)*DEG2RAD
             ELSE
               THETA=(EVALUATE_RAMP(Z_WIND_REF,I_RAMP_DIRECTION_Z)+EVALUATE_RAMP(T,I_RAMP_DIRECTION_T))*DEG2RAD
             ENDIF
            SIN_THETA = -SIN(THETA)
            COS_THETA = -COS(THETA)
         ELSE
            ...
            COS_THETA = 1._EB
         ENDIF
         U_LS(IIG,JJG) = U0*EVALUATE_RAMP(Z_WIND_REF,I_RAMP_SPEED_Z)*EVALUATE_RAMP(T,I_RAMP_SPEED_T)*SIN_THETA
         V_LS(IIG,JJG) = V0*EVALUATE_RAMP(Z_WIND_REF,I_RAMP_SPEED_Z)*EVALUATE_RAMP(T,I_RAMP_SPEED_T)*COS_THETA
         ...
         IF (LEVEL_SET_COUPLED_WIND) THEN
            ...
            ELSE
               ...
               DO KDUM = K_LS(IIG,JJG),KBAR
                  IF ((ZC(KDUM)-Z_LS(IIG,JJG))>=Z_WIND_REF) THEN
                     KWIND = KDUM
               ...
               ENDIF
               U_LS(IIG,JJG) = U_Z(1) + (Z_WIND_REF-ZWIND(1))/(ZWIND(2)-ZWIND(1))*(U_Z(2)-U_Z(1))
               V_LS(IIG,JJG) = V_Z(1) + (Z_WIND_REF-ZWIND(1))/(ZWIND(2)-ZWIND(1))*(V_Z(2)-V_Z(1))
