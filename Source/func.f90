...
CALL SET_OUTPUT_CLOCK(DT_BNDF   ,RAMP_BNDF_INDEX,BNDF_CLOCK,BNDF_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_CPU    ,RAMP_CPU_INDEX , CPU_CLOCK, CPU_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_CTRL   ,RAMP_CTRL_INDEX,CTRL_CLOCK,CTRL_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_DEVC   ,RAMP_DEVC_INDEX,DEVC_CLOCK,DEVC_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_FLUSH  ,RAMP_FLSH_INDEX,FLSH_CLOCK,FLSH_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_GEOM   ,RAMP_GEOM_INDEX,GEOM_CLOCK,GEOM_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_HRR    ,RAMP_HRR_INDEX , HRR_CLOCK, HRR_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_ISOF   ,RAMP_ISOF_INDEX,ISOF_CLOCK,ISOF_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_MASS   ,RAMP_MASS_INDEX,MASS_CLOCK,MASS_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_PART   ,RAMP_PART_INDEX,PART_CLOCK,PART_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_PL3D   ,RAMP_PL3D_INDEX,PL3D_CLOCK,PL3D_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_PROF   ,RAMP_PROF_INDEX,PROF_CLOCK,PROF_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_RADF   ,RAMP_RADF_INDEX,RADF_CLOCK,RADF_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_RESTART,RAMP_RSRT_INDEX,RSRT_CLOCK,RSRT_COUNTER,.FALSE.)
CALL SET_OUTPUT_CLOCK(DT_SLCF   ,RAMP_SLCF_INDEX,SLCF_CLOCK,SLCF_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_SL3D   ,RAMP_SL3D_INDEX,SL3D_CLOCK,SL3D_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_SMOKE3D,RAMP_SM3D_INDEX,SM3D_CLOCK,SM3D_COUNTER,.TRUE.)
CALL SET_OUTPUT_CLOCK(DT_UVW    ,RAMP_UVW_INDEX , UVW_CLOCK, UVW_COUNTER,.TRUE.)
...
SUBROUTINE SET_OUTPUT_CLOCK(DT_OUT,RAMP_INDEX,CLOCK,COUNTER,MESH_SPECIFIC_COUNTER)
...
REAL(EB) :: DT_OUT
INTEGER :: NF
LOGICAL, INTENT(IN) :: MESH_SPECIFIC_COUNTER
...
ELSE
   ...
   ALLOCATE(CLOCK(0:NF))
   DO N=0,NF
      CLOCK(N) = MIN(T_BEGIN+N*DT_OUT,T_END)
   ENDDO
...
!> \brief Determines the size of the BOUNDARY_ONE_D_TYPE derived type variable
...
SF%ONE_D_REALS_ARRAY_SIZE( 7) = N_TRACKED_SPECIES            ! ZZ_F
SF%ONE_D_REALS_ARRAY_SIZE( 8) = N_TRACKED_SPECIES            ! RHO_D_F
SF%ONE_D_REALS_ARRAY_SIZE( 9) = N_TRACKED_SPECIES            ! RHO_D_DZDN_F
SF%ONE_D_REALS_ARRAY_SIZE(10) = SF%N_CELLS_MAX               ! RHO_C_S
SF%ONE_D_REALS_ARRAY_SIZE(11) = SF%N_CELLS_MAX+2             ! K_S
SF%ONE_D_REALS_ARRAY_SIZE(12) = N_SURFACE_DENSITY_SPECIES    ! AWM_AEROSOL
SF%ONE_D_REALS_ARRAY_SIZE(13) = SF%N_LAYERS                  ! DDSUM
SF%ONE_D_REALS_ARRAY_SIZE(14) = SF%N_LAYERS                  ! SMALLEST_CELL_SIZE
SF%ONE_D_REALS_ARRAY_SIZE(15) = SF%N_LPC                     ! PART_MASS
SF%ONE_D_REALS_ARRAY_SIZE(16) = SF%N_LPC                     ! PART_ENTHALPY
SF%ONE_D_REALS_ARRAY_SIZE(17) = (SF%N_CELLS_MAX+2)*SF%N_MATL ! RHO
SF%ONE_D_REALS_ARRAY_SIZE(18) = (SF%N_CELLS_MAX+2)*SF%N_MATL ! RHO_DOT
SF%ONE_D_REALS_ARRAY_SIZE(19) = (SF%N_CELLS_MAX+2)*SF%N_SPEC ! RHO (gas)

SF%ONE_D_INTEGERS_ARRAY_SIZE(1) = SF%N_LAYERS                ! N_LAYER_CELLS

! Update last array size index also to the sum below

SF%N_ONE_D_STORAGE_REALS    = N_ONE_D_SCALAR_REALS    + SUM(SF%ONE_D_REALS_ARRAY_SIZE(1:19))
...
!> \brief Determines the size of the BOUNDARY_PROPS_TYPE derived type variable

SUBROUTINE COMPUTE_BOUNDARY_PROPS_STORAGE_DIMENSIONS

USE GLOBAL_CONSTANTS, ONLY: N_LP_ARRAY_INDICES

BOUNDARY_PROPS_REALS_ARRAY_SIZE(1) = N_LP_ARRAY_INDICES ! A_LP_MPUA
BOUNDARY_PROPS_REALS_ARRAY_SIZE(2) = N_LP_ARRAY_INDICES ! LP_CPUA
BOUNDARY_PROPS_REALS_ARRAY_SIZE(3) = N_LP_ARRAY_INDICES ! LP_MPUA
BOUNDARY_PROPS_REALS_ARRAY_SIZE(4) = N_LP_ARRAY_INDICES ! LP_TEMP

N_BOUNDARY_PROPS_STORAGE_REALS    = N_BOUNDARY_PROPS_SCALAR_REALS    + SUM(BOUNDARY_PROPS_REALS_ARRAY_SIZE(1:4))
N_BOUNDARY_PROPS_STORAGE_INTEGERS = N_BOUNDARY_PROPS_SCALAR_INTEGERS
N_BOUNDARY_PROPS_STORAGE_LOGICALS = N_BOUNDARY_PROPS_SCALAR_LOGICALS

END SUBROUTINE COMPUTE_BOUNDARY_PROPS_STORAGE_DIMENSIONS


!> \brief Determines the size of the BOUNDARY_RADIA_TYPE derived type variable

SUBROUTINE COMPUTE_BOUNDARY_RADIA_STORAGE_DIMENSIONS

USE GLOBAL_CONSTANTS, ONLY: NUMBER_SPECTRAL_BANDS,NUMBER_RADIATION_ANGLES

BOUNDARY_RADIA_REALS_ARRAY_SIZE(1) = NUMBER_RADIATION_ANGLES*NUMBER_SPECTRAL_BANDS  ! ILW
BOUNDARY_RADIA_REALS_ARRAY_SIZE(2) = NUMBER_SPECTRAL_BANDS ! IL

N_BOUNDARY_RADIA_STORAGE_REALS    = N_BOUNDARY_RADIA_SCALAR_REALS    + SUM(BOUNDARY_RADIA_REALS_ARRAY_SIZE(1:2))
N_BOUNDARY_RADIA_STORAGE_INTEGERS = N_BOUNDARY_RADIA_SCALAR_INTEGERS
N_BOUNDARY_RADIA_STORAGE_LOGICALS = N_BOUNDARY_RADIA_SCALAR_LOGICALS

END SUBROUTINE COMPUTE_BOUNDARY_RADIA_STORAGE_DIMENSIONS


!> \brief Determines the size of the BOUNDARY_COORD_TYPE derived type variable

SUBROUTINE COMPUTE_BOUNDARY_COORD_STORAGE_DIMENSIONS

N_BOUNDARY_COORD_STORAGE_REALS    = N_BOUNDARY_COORD_SCALAR_REALS
N_BOUNDARY_COORD_STORAGE_INTEGERS = N_BOUNDARY_COORD_SCALAR_INTEGERS
N_BOUNDARY_COORD_STORAGE_LOGICALS = N_BOUNDARY_COORD_SCALAR_LOGICALS

END SUBROUTINE COMPUTE_BOUNDARY_COORD_STORAGE_DIMENSIONS


!> \brief Determine the size of the LAGRAGIAN_PARTICLE_TYPE derived type variable
...
SF  => SURFACE(LPC%SURF_INDEX)
                                     ...
                                     LPC%N_STORAGE_REALS = N_PARTICLE_SCALAR_REALS
IF (LPC%INCLUDE_BOUNDARY_COORD_TYPE) LPC%N_STORAGE_REALS = LPC%N_STORAGE_REALS + N_BOUNDARY_COORD_STORAGE_REALS
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) LPC%N_STORAGE_REALS = LPC%N_STORAGE_REALS + SF%N_ONE_D_STORAGE_REALS
IF (LPC%INCLUDE_BOUNDARY_PROPS_TYPE) LPC%N_STORAGE_REALS = LPC%N_STORAGE_REALS + N_BOUNDARY_PROPS_STORAGE_REALS
IF (LPC%INCLUDE_BOUNDARY_RADIA_TYPE) LPC%N_STORAGE_REALS = LPC%N_STORAGE_REALS + N_BOUNDARY_RADIA_STORAGE_REALS

                                     LPC%N_STORAGE_INTEGERS = N_PARTICLE_SCALAR_INTEGERS
IF (LPC%INCLUDE_BOUNDARY_COORD_TYPE) LPC%N_STORAGE_INTEGERS = LPC%N_STORAGE_INTEGERS + N_BOUNDARY_COORD_STORAGE_INTEGERS
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) LPC%N_STORAGE_INTEGERS = LPC%N_STORAGE_INTEGERS + SF%N_ONE_D_STORAGE_INTEGERS
IF (LPC%INCLUDE_BOUNDARY_PROPS_TYPE) LPC%N_STORAGE_INTEGERS = LPC%N_STORAGE_INTEGERS + N_BOUNDARY_PROPS_STORAGE_INTEGERS
IF (LPC%INCLUDE_BOUNDARY_RADIA_TYPE) LPC%N_STORAGE_INTEGERS = LPC%N_STORAGE_INTEGERS + N_BOUNDARY_RADIA_STORAGE_INTEGERS

                                     LPC%N_STORAGE_LOGICALS = N_PARTICLE_SCALAR_LOGICALS
IF (LPC%INCLUDE_BOUNDARY_COORD_TYPE) LPC%N_STORAGE_LOGICALS = LPC%N_STORAGE_LOGICALS + N_BOUNDARY_COORD_STORAGE_LOGICALS
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) LPC%N_STORAGE_LOGICALS = LPC%N_STORAGE_LOGICALS + SF%N_ONE_D_STORAGE_LOGICALS
IF (LPC%INCLUDE_BOUNDARY_PROPS_TYPE) LPC%N_STORAGE_LOGICALS = LPC%N_STORAGE_LOGICALS + N_BOUNDARY_PROPS_STORAGE_LOGICALS
IF (LPC%INCLUDE_BOUNDARY_RADIA_TYPE) LPC%N_STORAGE_LOGICALS = LPC%N_STORAGE_LOGICALS + N_BOUNDARY_RADIA_STORAGE_LOGICALS

END SUBROUTINE COMPUTE_PARTICLE_STORAGE_DIMENSIONS


!> \brief Determines the size of the WALL_TYPE derived type variable
!> \param SURF_INDEX Index of the surface type
...
SF => SURFACE(SURF_INDEX)
                                    ...
                                    SF%N_WALL_STORAGE_REALS = N_WALL_SCALAR_REALS
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) SF%N_WALL_STORAGE_REALS = SF%N_WALL_STORAGE_REALS + N_BOUNDARY_COORD_STORAGE_REALS
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) SF%N_WALL_STORAGE_REALS = SF%N_WALL_STORAGE_REALS + N_BOUNDARY_PROPS_STORAGE_REALS
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) SF%N_WALL_STORAGE_REALS = SF%N_WALL_STORAGE_REALS + SF%N_ONE_D_STORAGE_REALS
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) SF%N_WALL_STORAGE_REALS = SF%N_WALL_STORAGE_REALS + N_BOUNDARY_RADIA_STORAGE_REALS

                                    SF%N_WALL_STORAGE_INTEGERS = N_WALL_SCALAR_INTEGERS
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) SF%N_WALL_STORAGE_INTEGERS = SF%N_WALL_STORAGE_INTEGERS + N_BOUNDARY_COORD_STORAGE_INTEGERS
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) SF%N_WALL_STORAGE_INTEGERS = SF%N_WALL_STORAGE_INTEGERS + N_BOUNDARY_PROPS_STORAGE_INTEGERS
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) SF%N_WALL_STORAGE_INTEGERS = SF%N_WALL_STORAGE_INTEGERS + SF%N_ONE_D_STORAGE_INTEGERS
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) SF%N_WALL_STORAGE_INTEGERS = SF%N_WALL_STORAGE_INTEGERS + N_BOUNDARY_RADIA_STORAGE_INTEGERS

                                    SF%N_WALL_STORAGE_LOGICALS = N_WALL_SCALAR_LOGICALS
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) SF%N_WALL_STORAGE_LOGICALS = SF%N_WALL_STORAGE_LOGICALS + N_BOUNDARY_COORD_STORAGE_LOGICALS
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) SF%N_WALL_STORAGE_LOGICALS = SF%N_WALL_STORAGE_LOGICALS + N_BOUNDARY_PROPS_STORAGE_LOGICALS
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) SF%N_WALL_STORAGE_LOGICALS = SF%N_WALL_STORAGE_LOGICALS + SF%N_ONE_D_STORAGE_LOGICALS
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) SF%N_WALL_STORAGE_LOGICALS = SF%N_WALL_STORAGE_LOGICALS + N_BOUNDARY_RADIA_STORAGE_LOGICALS

IF (.NOT.ALLOCATED(SF%WALL_STORAGE%REALS))    ALLOCATE(SF%WALL_STORAGE%REALS(SF%N_WALL_STORAGE_REALS,1))
IF (.NOT.ALLOCATED(SF%WALL_STORAGE%INTEGERS)) ALLOCATE(SF%WALL_STORAGE%INTEGERS(SF%N_WALL_STORAGE_INTEGERS,1))
IF (.NOT.ALLOCATED(SF%WALL_STORAGE%LOGICALS)) ALLOCATE(SF%WALL_STORAGE%LOGICALS(SF%N_WALL_STORAGE_LOGICALS,1))

END SUBROUTINE COMPUTE_WALL_STORAGE_DIMENSIONS


!> \brief Determines the size of the CFACE_TYPE derived type variable
!> \param SURF_INDEX Index of the surface type
...
SF => SURFACE(SURF_INDEX)
                                    ...
                                    SF%N_CFACE_STORAGE_REALS = N_CFACE_SCALAR_REALS     + N_TRACKED_SPECIES
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) SF%N_CFACE_STORAGE_REALS = SF%N_CFACE_STORAGE_REALS + N_BOUNDARY_COORD_STORAGE_REALS
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) SF%N_CFACE_STORAGE_REALS = SF%N_CFACE_STORAGE_REALS + N_BOUNDARY_PROPS_STORAGE_REALS
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) SF%N_CFACE_STORAGE_REALS = SF%N_CFACE_STORAGE_REALS + SF%N_ONE_D_STORAGE_REALS
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) SF%N_CFACE_STORAGE_REALS = SF%N_CFACE_STORAGE_REALS + N_BOUNDARY_RADIA_STORAGE_REALS

                                    SF%N_CFACE_STORAGE_INTEGERS = N_CFACE_SCALAR_INTEGERS
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) SF%N_CFACE_STORAGE_INTEGERS = SF%N_CFACE_STORAGE_INTEGERS + N_BOUNDARY_COORD_STORAGE_INTEGERS
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) SF%N_CFACE_STORAGE_INTEGERS = SF%N_CFACE_STORAGE_INTEGERS + N_BOUNDARY_PROPS_STORAGE_INTEGERS
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) SF%N_CFACE_STORAGE_INTEGERS = SF%N_CFACE_STORAGE_INTEGERS + SF%N_ONE_D_STORAGE_INTEGERS
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) SF%N_CFACE_STORAGE_INTEGERS = SF%N_CFACE_STORAGE_INTEGERS + N_BOUNDARY_RADIA_STORAGE_INTEGERS

                                    SF%N_CFACE_STORAGE_LOGICALS = N_CFACE_SCALAR_LOGICALS
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) SF%N_CFACE_STORAGE_LOGICALS = SF%N_CFACE_STORAGE_LOGICALS + N_BOUNDARY_COORD_STORAGE_LOGICALS
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) SF%N_CFACE_STORAGE_LOGICALS = SF%N_CFACE_STORAGE_LOGICALS + N_BOUNDARY_PROPS_STORAGE_LOGICALS
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) SF%N_CFACE_STORAGE_LOGICALS = SF%N_CFACE_STORAGE_LOGICALS + SF%N_ONE_D_STORAGE_LOGICALS
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) SF%N_CFACE_STORAGE_LOGICALS = SF%N_CFACE_STORAGE_LOGICALS + N_BOUNDARY_RADIA_STORAGE_LOGICALS
...
SUBROUTINE ALLOCATE_STORAGE(NM,LP_INDEX,LPC_INDEX,SURF_INDEX,WALL_INDEX,CFACE_INDEX)

USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES
INTEGER, INTENT(IN):: NM,SURF_INDEX
INTEGER, INTENT(IN), OPTIONAL :: LP_INDEX,LPC_INDEX,WALL_INDEX,CFACE_INDEX
INTEGER :: N_NEW_STORAGE_SLOTS,OD_INDEX,BP_INDEX,BR_INDEX,BC_INDEX,I,NAS
...
TYPE (BOUNDARY_PROPS_TYPE), ALLOCATABLE, DIMENSION(:) :: BP_DUMMY
...
IF (PRESENT(LP_INDEX)) THEN
   ...
   ENDIF
   BC_INDEX = M%LAGRANGIAN_PARTICLE(LP_INDEX)%BC_INDEX
   OD_INDEX = M%LAGRANGIAN_PARTICLE(LP_INDEX)%OD_INDEX
   BP_INDEX = M%LAGRANGIAN_PARTICLE(LP_INDEX)%BP_INDEX
   BR_INDEX = M%LAGRANGIAN_PARTICLE(LP_INDEX)%BR_INDEX
   ...
   IF (LPC%INCLUDE_BOUNDARY_PROPS_TYPE) CALL ALLOCATE_BOUNDARY_PROPS_ARRAYS
   IF (LPC%INCLUDE_BOUNDARY_RADIA_TYPE) CALL ALLOCATE_BOUNDARY_RADIA_ARRAYS
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%LP_INDEX = LP_INDEX
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%BC_INDEX = BC_INDEX
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%OD_INDEX = OD_INDEX
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%BP_INDEX = BP_INDEX
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%BR_INDEX = BR_INDEX
...
ELSEIF (PRESENT(WALL_INDEX)) THEN
   ...
   ENDIF
   BC_INDEX = M%WALL(WALL_INDEX)%BC_INDEX
   OD_INDEX = M%WALL(WALL_INDEX)%OD_INDEX
   BP_INDEX = M%WALL(WALL_INDEX)%BP_INDEX
   BR_INDEX = M%WALL(WALL_INDEX)%BR_INDEX
   ...
   IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) CALL ALLOCATE_BOUNDARY_PROPS_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) CALL ALLOCATE_BOUNDARY_RADIA_ARRAYS
   M%WALL(WALL_INDEX)%WALL_INDEX = WALL_INDEX
   M%WALL(WALL_INDEX)%BC_INDEX   = BC_INDEX
   M%WALL(WALL_INDEX)%OD_INDEX   = OD_INDEX
   M%WALL(WALL_INDEX)%BP_INDEX   = BP_INDEX
   M%WALL(WALL_INDEX)%BR_INDEX   = BR_INDEX
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   ENDIF
   BC_INDEX = M%CFACE(CFACE_INDEX)%BC_INDEX
   OD_INDEX = M%CFACE(CFACE_INDEX)%OD_INDEX
   BP_INDEX = M%CFACE(CFACE_INDEX)%BP_INDEX
   BR_INDEX = M%CFACE(CFACE_INDEX)%BR_INDEX
   ...
   IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) CALL ALLOCATE_BOUNDARY_PROPS_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) CALL ALLOCATE_BOUNDARY_RADIA_ARRAYS
   M%CFACE(CFACE_INDEX)%CFACE_INDEX = CFACE_INDEX
   M%CFACE(CFACE_INDEX)%BC_INDEX = BC_INDEX
   M%CFACE(CFACE_INDEX)%OD_INDEX = OD_INDEX
   M%CFACE(CFACE_INDEX)%BP_INDEX = BP_INDEX
   M%CFACE(CFACE_INDEX)%BR_INDEX = BR_INDEX
...
USE GLOBAL_CONSTANTS, ONLY: THERMALLY_THICK,TMPM,SIGMA,TMPA4,RHOA,N_TRACKED_SPECIES
...
IF (.NOT.ALREADY_ALLOCATED) THEN
   IF (ALLOCATED(ONE_D%N_LAYER_CELLS)) DEALLOCATE(ONE_D%N_LAYER_CELLS)
   ALLOCATE(ONE_D%N_LAYER_CELLS(SF%ONE_D_INTEGERS_ARRAY_SIZE(1)))
ENDIF
ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS) = SF%N_LAYER_CELLS(1:SF%N_LAYERS)

ONE_D%SURF_INDEX      = SURF_INDEX

ONE_D%Q_RAD_IN        = SF%EMISSIVITY*SIGMA*TMPA4
ONE_D%Q_RAD_OUT       = SF%EMISSIVITY*SIGMA*TMPA4
ONE_D%EMISSIVITY      = SF%EMISSIVITY
ONE_D%T_IGN           = SF%T_IGN
ONE_D%TMP_F           = SF%TMP_FRONT
ONE_D%TMP_B           = SF%TMP_INNER(SF%N_LAYERS)
ONE_D%RHO_F           = RHOA
ONE_D%TMP_F_OLD       = SF%TMP_FRONT
ONE_D%BURN_DURATION   = SF%BURN_DURATION

IF (.NOT.ALREADY_ALLOCATED) THEN
   IF (ALLOCATED(ONE_D%M_DOT_G_PP_ACTUAL)) DEALLOCATE(ONE_D%M_DOT_G_PP_ACTUAL)
   ALLOCATE(ONE_D%M_DOT_G_PP_ACTUAL(SF%ONE_D_REALS_ARRAY_SIZE(1)))
   IF (ALLOCATED(ONE_D%M_DOT_S_PP)) DEALLOCATE(ONE_D%M_DOT_S_PP)
   ALLOCATE(ONE_D%M_DOT_S_PP(SF%ONE_D_REALS_ARRAY_SIZE(2)))
   IF (ALLOCATED(ONE_D%M_DOT_G_PP_ADJUST)) DEALLOCATE(ONE_D%M_DOT_G_PP_ADJUST)
   ALLOCATE(ONE_D%M_DOT_G_PP_ADJUST(SF%ONE_D_REALS_ARRAY_SIZE(3)))
   IF (ALLOCATED(ONE_D%X)) DEALLOCATE(ONE_D%X)
   ALLOCATE(ONE_D%X(0:SF%ONE_D_REALS_ARRAY_SIZE(4)-1))
   IF (ALLOCATED(ONE_D%TMP)) DEALLOCATE(ONE_D%TMP)
   ALLOCATE(ONE_D%TMP(0:SF%ONE_D_REALS_ARRAY_SIZE(5)-1))
   IF (ALLOCATED(ONE_D%LAYER_THICKNESS)) DEALLOCATE(ONE_D%LAYER_THICKNESS)
   ALLOCATE(ONE_D%LAYER_THICKNESS(SF%ONE_D_REALS_ARRAY_SIZE(6)))
   IF (ALLOCATED(ONE_D%ZZ_F)) DEALLOCATE(ONE_D%ZZ_F)
   ALLOCATE(ONE_D%ZZ_F(SF%ONE_D_REALS_ARRAY_SIZE(7)))
   IF (ALLOCATED(ONE_D%RHO_D_F)) DEALLOCATE(ONE_D%RHO_D_F)
   ALLOCATE(ONE_D%RHO_D_F(SF%ONE_D_REALS_ARRAY_SIZE(8)))
   IF (ALLOCATED(ONE_D%RHO_D_DZDN_F)) DEALLOCATE(ONE_D%RHO_D_DZDN_F)
   ALLOCATE(ONE_D%RHO_D_DZDN_F(SF%ONE_D_REALS_ARRAY_SIZE(9)))
   IF (ALLOCATED(ONE_D%RHO_C_S)) DEALLOCATE(ONE_D%RHO_C_S)
   ALLOCATE(ONE_D%RHO_C_S(SF%ONE_D_REALS_ARRAY_SIZE(10)))
   IF (ALLOCATED(ONE_D%K_S)) DEALLOCATE(ONE_D%K_S)
   ALLOCATE(ONE_D%K_S(0:SF%ONE_D_REALS_ARRAY_SIZE(11)-1))
   IF (ALLOCATED(ONE_D%AWM_AEROSOL)) DEALLOCATE(ONE_D%AWM_AEROSOL)
   ALLOCATE(ONE_D%AWM_AEROSOL(SF%ONE_D_REALS_ARRAY_SIZE(12)))
   IF (ALLOCATED(ONE_D%DDSUM)) DEALLOCATE(ONE_D%DDSUM)
   ALLOCATE(ONE_D%DDSUM(SF%ONE_D_REALS_ARRAY_SIZE(13)))
   IF (ALLOCATED(ONE_D%SMALLEST_CELL_SIZE)) DEALLOCATE(ONE_D%SMALLEST_CELL_SIZE)
   ALLOCATE(ONE_D%SMALLEST_CELL_SIZE(SF%ONE_D_REALS_ARRAY_SIZE(14)))
   IF (ALLOCATED(ONE_D%PART_MASS)) DEALLOCATE(ONE_D%PART_MASS)
   ALLOCATE(ONE_D%PART_MASS(SF%ONE_D_REALS_ARRAY_SIZE(15)))
   IF (ALLOCATED(ONE_D%PART_ENTHALPY)) DEALLOCATE(ONE_D%PART_ENTHALPY)
   ALLOCATE(ONE_D%PART_ENTHALPY(SF%ONE_D_REALS_ARRAY_SIZE(16)))
   IF (ALLOCATED(ONE_D%MATL_COMP)) DEALLOCATE(ONE_D%MATL_COMP)
   ALLOCATE(ONE_D%MATL_COMP(SF%N_MATL))
   DO NN=1,SF%N_MATL
      IF (ALLOCATED(ONE_D%MATL_COMP(NN)%RHO_DOT)) DEALLOCATE(ONE_D%MATL_COMP(NN)%RHO_DOT)
      ALLOCATE(ONE_D%MATL_COMP(NN)%RHO_DOT(0:SF%N_CELLS_MAX+1))
...
ONE_D%M_DOT_G_PP_ADJUST = 0._EB
ONE_D%X=0._EB ; ONE_D%X(0:SF%N_CELLS_INI) = SF%X_S(0:SF%N_CELLS_INI)
...
SUBROUTINE ALLOCATE_BOUNDARY_PROPS_ARRAYS

USE GLOBAL_CONSTANTS, ONLY: TMPA
TYPE(BOUNDARY_PROPS_TYPE), POINTER :: BP

IF (BP_INDEX==0) THEN
   NAS = M%NEXT_AVAILABLE_BOUNDARY_PROPS_SLOT
   DO I=NAS,M%N_BOUNDARY_PROPS_DIM
      IF (M%BOUNDARY_PROPS_OCCUPANCY(I)==0) EXIT
      M%NEXT_AVAILABLE_BOUNDARY_PROPS_SLOT = M%NEXT_AVAILABLE_BOUNDARY_PROPS_SLOT + 1
   ENDDO
   BP_INDEX = M%NEXT_AVAILABLE_BOUNDARY_PROPS_SLOT
ENDIF
...
IF (BP_INDEX>M%N_BOUNDARY_PROPS_DIM) THEN  ! There are no open slots for boundary coordinates
   ...
   ALLOCATE(DUMMY(1:M%N_BOUNDARY_PROPS_DIM+N_NEW_STORAGE_SLOTS)) ; DUMMY = 0
   DUMMY(1:M%N_BOUNDARY_PROPS_DIM) = M%BOUNDARY_PROPS_OCCUPANCY(1:M%N_BOUNDARY_PROPS_DIM)
   CALL MOVE_ALLOC(DUMMY,M%BOUNDARY_PROPS_OCCUPANCY)
   M%N_BOUNDARY_PROPS_DIM = M%N_BOUNDARY_PROPS_DIM + N_NEW_STORAGE_SLOTS
ENDIF

M%BOUNDARY_PROPS_OCCUPANCY(BP_INDEX) = 1

BP => M%BOUNDARY_PROPS(BP_INDEX)

IF (BP%SURF_INDEX==SURF_INDEX) THEN
   ALREADY_ALLOCATED = .TRUE.
ELSE
   ALREADY_ALLOCATED = .FALSE.
ENDIF
...
IF (.NOT.ALREADY_ALLOCATED) THEN
   IF (ALLOCATED(BP%A_LP_MPUA)) DEALLOCATE(BP%A_LP_MPUA)
   ALLOCATE(BP%A_LP_MPUA(BOUNDARY_PROPS_REALS_ARRAY_SIZE(1)))
   IF (ALLOCATED(BP%LP_CPUA)) DEALLOCATE(BP%LP_CPUA)
   ALLOCATE(BP%LP_CPUA(BOUNDARY_PROPS_REALS_ARRAY_SIZE(2)))
   IF (ALLOCATED(BP%LP_MPUA)) DEALLOCATE(BP%LP_MPUA)
   ALLOCATE(BP%LP_MPUA(BOUNDARY_PROPS_REALS_ARRAY_SIZE(3)))
   IF (ALLOCATED(BP%LP_TEMP)) DEALLOCATE(BP%LP_TEMP)
   ALLOCATE(BP%LP_TEMP(BOUNDARY_PROPS_REALS_ARRAY_SIZE(4)))
ENDIF
...
BP%SURF_INDEX = SURF_INDEX

END SUBROUTINE ALLOCATE_BOUNDARY_PROPS_ARRAYS
...
IF (.NOT.ALLOCATED(BR%IL)) ALLOCATE(BR%IL(BOUNDARY_RADIA_REALS_ARRAY_SIZE(2)))
...
!> \param NM Mesh index
!> \param LP_INDEX Index of the particle
!> \param LPC_INDEX Class index of the particle
!> \param STORAGE_INDEX Slot in a PARTICLE_STORAGE array in which to store the variables
!> \param UNPACK_IT Logical indicating whether the data is to be packed or unpacked
!> \param NOM Index of the mesh that sent or is to receive the particles. Also, an indicator of an MPI exchange, not restart.

SUBROUTINE PACK_PARTICLE(NM,LP_INDEX,LPC_INDEX,STORAGE_INDEX,UNPACK_IT,NOM)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,LP_INDEX,LPC_INDEX,STORAGE_INDEX
...
LPC => LAGRANGIAN_PARTICLE_CLASS(LPC_INDEX)
SF => SURFACE(LPC%SURF_INDEX)
IF (PRESENT(NOM) .AND. UNPACK_IT) THEN
   OS => MESHES(NM)%OMESH(NOM)%ADOPT_PARTICLE_STORAGE(LPC_INDEX)
ELSEIF (PRESENT(NOM) .AND. .NOT.UNPACK_IT) THEN
   OS => MESHES(NM)%OMESH(NOM)%ORPHAN_PARTICLE_STORAGE(LPC_INDEX)
ELSE
   OS => LPC%PARTICLE_STORAGE
ENDIF

IF (UNPACK_IT) CALL ALLOCATE_STORAGE(NM,LP_INDEX=LP_INDEX,LPC_INDEX=LPC_INDEX,SURF_INDEX=LPC%SURF_INDEX)
...
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%TAG,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%CLASS_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%ORIENTATION_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%WALL_INDEX,UNPACK_IT) ; IF (PRESENT(NOM) .AND. UNPACK_IT) LP%WALL_INDEX = 0
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%DUCT_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%DUCT_CELL_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%INIT_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%CFACE_INDEX,UNPACK_IT) ; IF (PRESENT(NOM) .AND. UNPACK_IT) LP%CFACE_INDEX = 0
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%PROP_INDEX,UNPACK_IT)

! Assign logical pointers and values

LC = 0

LC=LC+1 ; CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%SHOW,UNPACK_IT)
LC=LC+1 ; CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%SPLAT,UNPACK_IT)
LC=LC+1 ; CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%EMBER,UNPACK_IT)
LC=LC+1 ; CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%PATH_PARTICLE,UNPACK_IT)

! Assign real pointers and values

RC = 0

RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%U,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%V,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%W,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%PWT,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ACCEL_X,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ACCEL_Y,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ACCEL_Z,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%RE,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%MASS,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%T_INSERT,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%DX,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%DY,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%DZ,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%C_DRAG,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ZI,UNPACK_IT)

! Boundary Coordinates

IF (LP%BC_INDEX>0) CALL PACK_BOUNDARY_COORD(NM,IC,RC,OS,STORAGE_INDEX,LP%BC_INDEX,UNPACK_IT)
IF (LP%OD_INDEX>0) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,STORAGE_INDEX,LP%OD_INDEX,UNPACK_IT)
IF (LP%BP_INDEX>0) CALL PACK_BOUNDARY_PROPS(NM,IC,RC,OS,1,LP%BP_INDEX,UNPACK_IT)
IF (LP%BR_INDEX>0) CALL PACK_BOUNDARY_RADIA(NM,RC,OS,1,LP%BR_INDEX,UNPACK_IT)
...
!> \param SURF_INDEX Index of the surface
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it

SUBROUTINE PACK_WALL(NM,WALL_INDEX,SURF_INDEX,UNPACK_IT)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,SURF_INDEX,WALL_INDEX
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER :: IC,RC,LC
...
IF (UNPACK_IT) CALL ALLOCATE_STORAGE(NM,WALL_INDEX=WALL_INDEX,SURF_INDEX=SURF_INDEX)
...
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%BACK_MESH,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%BOUNDARY_TYPE,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%SURF_INDEX_ORIG,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%OBST_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%PRESSURE_BC_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%VENT_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%JD11_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%JD12_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%JD21_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%JD22_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),WC%CUT_FACE_INDEX,UNPACK_IT)

! Assign and initialize reals

RC = 0

RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),WC%DUNDT,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),WC%Q_LEAK,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),WC%V_DEP,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),WC%VEL_ERR_NEW,UNPACK_IT)

! Assign and initialize logicals

LC = 0

IF (WC%BC_INDEX>0) CALL PACK_BOUNDARY_COORD(NM,IC,RC,OS,1,WC%BC_INDEX,UNPACK_IT)
IF (WC%OD_INDEX>0) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,1,WC%OD_INDEX,UNPACK_IT)
IF (WC%BP_INDEX>0) CALL PACK_BOUNDARY_PROPS(NM,IC,RC,OS,1,WC%BP_INDEX,UNPACK_IT)
IF (WC%BR_INDEX>0) CALL PACK_BOUNDARY_RADIA(NM,RC,OS,1,WC%BR_INDEX,UNPACK_IT)
...
!> \param NM Mesh index
!> \param CFACE_INDEX Index of the CFACE cell
!> \param SURF_INDEX Index of the surface
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it

SUBROUTINE PACK_CFACE(NM,CFACE_INDEX,SURF_INDEX,UNPACK_IT)

USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES
USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,SURF_INDEX,CFACE_INDEX
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER :: IC,RC,LC,I1,I2
...
IF (UNPACK_IT) CALL ALLOCATE_STORAGE(NM,CFACE_INDEX=CFACE_INDEX,SURF_INDEX=SURF_INDEX)
...
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%VENT_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%BACK_MESH,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%BOUNDARY_TYPE,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%CUT_FACE_IND1,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,1),CFA%CUT_FACE_IND2,UNPACK_IT)

! Assign and initialize reals

RC = 0

RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%AREA,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%NVEC(1),UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%NVEC(2),UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%NVEC(3),UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%VEL_ERR_NEW,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%V_DEP,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%Q_LEAK,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%DUNDT,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%RSUM_G,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%TMP_G,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%RHO_G,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%MU_G,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,1),CFA%PRES_BXN,UNPACK_IT)

I2 = RC
I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
CALL EQUATE(OS%REALS(I1:I2,1) , CFA%ZZ_G(1:I2-I1+1) , UNPACK_IT)
RC = I2
...
IF (CFA%BC_INDEX>0) CALL PACK_BOUNDARY_COORD(NM,IC,RC,OS,1,CFA%BC_INDEX,UNPACK_IT)
IF (CFA%OD_INDEX>0) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,1,CFA%OD_INDEX,UNPACK_IT)
IF (CFA%BP_INDEX>0) CALL PACK_BOUNDARY_PROPS(NM,IC,RC,OS,1,CFA%BP_INDEX,UNPACK_IT)
IF (CFA%BR_INDEX>0) CALL PACK_BOUNDARY_RADIA(NM,RC,OS,1,CFA%BR_INDEX,UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_COORD(NM,IC,RC,OS,STORAGE_INDEX,BC_INDEX,UNPACK_IT)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,BC_INDEX
INTEGER, INTENT(INOUT) :: IC,RC
LOGICAL, INTENT(IN) :: UNPACK_IT
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE(STORAGE_TYPE), POINTER :: OS

BC => MESHES(NM)%BOUNDARY_COORD(BC_INDEX)

IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%II,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%JJ,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%KK,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%IIG,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%JJG,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%KKG,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%IOR,UNPACK_IT)

RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%X,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Y,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Z,UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,OD_INDEX
...
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%PRESSURE_ZONE,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%NODE_INDEX,UNPACK_IT)
IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_SUBSTEPS,UNPACK_IT)

I2 = IC

I1 = I2+1 ; I2 = I1 + SF%ONE_D_INTEGERS_ARRAY_SIZE(1) - 1
CALL EQUATE(OS%INTEGERS(I1:I2,STORAGE_INDEX) , ONE_D%N_LAYER_CELLS(1:I2-I1+1) , UNPACK_IT)

IC = I2  ! Update Integer Counter

! Logicals

LC=LC+1 ; CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),ONE_D%BURNAWAY,UNPACK_IT)

! Reals

RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%AREA,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%HEAT_TRANS_COEF,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_CON_F,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_RAD_IN,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_RAD_OUT,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%EMISSIVITY,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%AREA_ADJUST,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%T_IGN,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_F,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_B,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_NORMAL,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_NORMAL_S,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_NORMAL_0,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_TANG,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%RHO_F,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%RDN,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%K_G,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_DOT_G_PP,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_DOT_O2_PP,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_CONDENSE,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_F_OLD,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%BURN_DURATION,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%T_SCALE,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_SCALE,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%T_MATL_PART,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%B_NUMBER,UNPACK_IT)

I2 = RC  ! I1 and I2 continue counting reals

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(1) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_G_PP_ACTUAL(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(2) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_S_PP(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(3) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_G_PP_ADJUST(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(4) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%X(0:I2-I1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(5) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%TMP(0:I2-I1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(6) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%LAYER_THICKNESS(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(7) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%ZZ_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(8) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%RHO_D_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(9) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%RHO_D_DZDN_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(10) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%RHO_C_S(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(11) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%K_S(0:I2-I1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(12) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%AWM_AEROSOL(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(13) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%DDSUM(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(14) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%SMALLEST_CELL_SIZE(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(15) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%PART_MASS(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(16) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%PART_ENTHALPY(1:I2-I1+1) , UNPACK_IT)

DO NN=1,SF%N_MATL
   I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
   CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%MATL_COMP(NN)%RHO(0:I2-I1) , UNPACK_IT)
ENDDO
...
!> \param BP_INDEX Index within BOUNDARY_PROPS array
!> \param UNPACK_IT Logical indicating whether to pack or unpack

SUBROUTINE PACK_BOUNDARY_PROPS(NM,IC,RC,OS,STORAGE_INDEX,BP_INDEX,UNPACK_IT)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,BP_INDEX
...
BP => MESHES(NM)%BOUNDARY_PROPS(BP_INDEX)

IC=IC+1 ; CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BP%SURF_INDEX,UNPACK_IT)

RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%U_TAU,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%Y_PLUS,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%Z_STAR,UNPACK_IT)
RC=RC+1 ; CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%PHI_LS,UNPACK_IT)
...
I1 = I2+1 ; I2 = I1 + BOUNDARY_PROPS_REALS_ARRAY_SIZE(1) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%A_LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + BOUNDARY_PROPS_REALS_ARRAY_SIZE(2) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%LP_CPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + BOUNDARY_PROPS_REALS_ARRAY_SIZE(3) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + BOUNDARY_PROPS_REALS_ARRAY_SIZE(4) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%LP_TEMP(1:I2-I1+1),UNPACK_IT)

RC = I2

END SUBROUTINE PACK_BOUNDARY_PROPS
...
SUBROUTINE PACK_BOUNDARY_RADIA(NM,RC,OS,STORAGE_INDEX,BR_INDEX,UNPACK_IT)

USE COMP_OPERATORS, ONLY: EQUATE
USE GLOBAL_CONSTANTS, ONLY: NUMBER_SPECTRAL_BANDS,NUMBER_RADIATION_ANGLES
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,BR_INDEX
...
BR => MESHES(NM)%BOUNDARY_RADIA(BR_INDEX)

I2 = RC

DO NN=1,NUMBER_SPECTRAL_BANDS
   I1 = I2+1 ; I2 = I1 + NUMBER_RADIATION_ANGLES - 1
   CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BR%BAND(NN)%ILW(1:I2-I1+1),UNPACK_IT)
ENDDO

I1 = I2+1 ; I2 = I1 + BOUNDARY_RADIA_REALS_ARRAY_SIZE(2) - 1
CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BR%IL(1:I2-I1+1),UNPACK_IT)
...
!> \brief Get enthalpy of an HT3D cell at a specified temperature
!> \param RHOH_OUT Enthalpy of cell (J/m3)
!> \param TMP_s Cell temperature (K)
!> \param SURF_INDEX Index of wall cell surface type
!> \param RHO_IN Current cell density (kg/m3)

SUBROUTINE GET_SOLID_RHOH(RHOH_OUT,TMP_S,SURF_INDEX,RHO_IN)

INTEGER, INTENT(IN) :: SURF_INDEX
REAL(EB), INTENT(IN) :: RHO_IN(1:N_MATL)
REAL(EB), INTENT(IN) :: TMP_S
REAL(EB), INTENT(OUT) :: RHOH_OUT
INTEGER :: N,ITMP
TYPE(MATERIAL_TYPE), POINTER :: ML=>NULL()
TYPE(SURFACE_TYPE), POINTER :: SF=>NULL()
...
DO N=1,SF%N_MATL
   ...
   RHOH_OUT = RHOH_OUT + (ML%H(ITMP)+(TMP_S-REAL(ITMP,EB))*(ML%H(ITMP+1)-ML%H(ITMP)))*RHO_IN(N)
ENDDO

IF (RHOH_OUT<=TWO_EPSILON_EB) RHOH_OUT = 0.001_EB
...
SELECT CASE(LES_FILTER_WIDTH_TYPE)
   CASE(MEAN_LES_FILTER_WIDTH)
      IF (TWO_D) THEN
         LES_FILTER_WIDTH_FUNCTION = SQRT(DX*DZ)
      ELSE
         LES_FILTER_WIDTH_FUNCTION = (DX*DY*DZ)**ONTH
      ENDIF
   CASE(MAX_LES_FILTER_WIDTH)
      ! May be needed to avoid spurious fluctuations at coarse fine mesh interfaces when
      ! flow is parallel to the interface, as often happens in outdoor wind cases.
      IF (TWO_D) THEN
...
DO K=0,M%KBP1
   ...
   M%W_WIND(K) = W0*EVALUATE_RAMP(T,I_RAMP_W0_T)*EVALUATE_RAMP(M%ZC(K),I_RAMP_W0_Z)
ENDDO
