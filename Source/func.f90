...
SF%N_WALL_STORAGE_REALS    = N_WALL_SCALAR_REALS    + SUM(SF%WALL_REALS_ARRAY_SIZE(1:4))    + SF%N_ONE_D_STORAGE_REALS
...
LP%INIT_INDEX          => OS%INTEGERS(10,STORAGE_INDEX) ; IF (NEW) LP%INIT_INDEX = 0
...
CALL ONE_D_POINTERS(WALL_INDEX_LOCAL,NEW,N_WALL_SCALAR_REALS+SUM(SF%WALL_REALS_ARRAY_SIZE(1:4)), &
...
DHOR_T = (H_V_H2O(INT(TEMP))+(TEMP-REAL(INT(TEMP,EB)))*H_V_H2O(INT(TEMP)+1))*MW_H2O/R0
P_RATIO = PZONE/P_STP
