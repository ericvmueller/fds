...
!> \brief return the current value of RSS memory (in KB) for the process. Only works for Linux
...
CHARACTER(LEN=200):: FN_PROC_STATUS=' '
CHARACTER(LEN=80) :: LINE
CHARACTER(LEN=8)  :: PID_CHAR=' '
INTEGER :: PID,LU_PROC_STATUS
LOGICAL :: IFXST

SYSTEM_MEM_USAGE=-1    ! RETURN NEGATIVE NUMBER IF NOT FOUND

! GET PROCESS ID

PID=GETPID()
WRITE(PID_CHAR,'(I8)') PID
FN_PROC_STATUS='/PROC/'//TRIM(ADJUSTL(PID_CHAR))//'/STATUS'

! READ SYSTEM FILE

INQUIRE (FILE=FN_PROC_STATUS,EXIST=IFXST)
IF (.NOT.IFXST) THEN
  WRITE(LU_ERR,*) 'SYSTEM FILE DOES NOT EXIST'
  RETURN
ENDIF

LU_PROC_STATUS=GET_FILE_NUMBER()
OPEN(UNIT=LU_PROC_STATUS, FILE=FN_PROC_STATUS, ACTION='READ')
DO
  READ(LU_PROC_STATUS,'(A)',END=120) LINE
