...
REAL(EB) :: INPUT_REAL
REAL(EB) :: T_HEARTBEAT,DT_PAUSE
LOGICAL :: HEARTBEAT_FOUND
LOGICAL,INTENT(OUT) :: FAILED

FAILED = .FALSE.
IF (ALLOCATED(RAMPS)) EXTERNAL_RAMP = RAMPS%LAST
IF (ALLOCATED(CONTROL)) EXTERNAL_CTRL = CONTROL%CURRENT_STATE
...
IF (DT_EXTERNAL_HEARTBEAT > 0) THEN
   ...
   HEARTBEAT_LOOP: DO
      ...
      IF (HEARTBEAT_FOUND) THEN
         EXIT HEARTBEAT_LOOP
      ELSE
         DT_PAUSE = MIN(1._EB,DT_EXTERNAL_HEARTBEAT - T_HEARTBEAT)
...
SUBROUTINE MOLAR_CONC_TO_MASS_FRAC(CC_IN,ZZ_OUT)
   ...
   SUM_ZZ = SUM(ZZ_OUT)
   IF (SUM_ZZ > 0._EB) THEN
      ZZ_OUT = ZZ_OUT / SUM_ZZ
   ELSE
      ZZ_OUT = 0._EB
   ENDIF
