...
SELECT CASE(RAMP_INDEX)
   ...
   CASE(1:)
      ...
      ELSEIF (RAMPS(RAMP_INDEX)%EXTERNAL_FILE) THEN
         ...
         RETURN
      ELSE
         IF (RAMPS(RAMP_INDEX)%CYCLING) THEN
            RAMP_POSITION = RAMP_INPUT - RAMPS(RAMP_INDEX)%T_MIN
            IF (RAMP_POSITION > RAMPS(RAMP_INDEX)%SPAN) THEN
               RAMP_POSITION = RAMP_POSITION - REAL(INT(RAMP_POSITION/RAMPS(RAMP_INDEX)%SPAN),EB)*RAMPS(RAMP_INDEX)%SPAN
            ELSEIF (RAMP_POSITION < 0._EB) THEN
               RAMP_POSITION = RAMPS(RAMP_INDEX)%SPAN + &
                  RAMP_POSITION - REAL(INT(RAMP_POSITION/RAMPS(RAMP_INDEX)%SPAN),EB)*RAMPS(RAMP_INDEX)%SPAN
            ENDIF
         ELSE
            RAMP_POSITION = &
...
PARAMETER ( &
QA = 9.16461398268964E-01_EB, &
QB = 2.31729200323405E-01_EB, &
QC = 4.88826640273108E-01_EB, &
QD = 1.24610454613712E-01_EB, &
Q0 = 4.99999303439796E-01_EB, &
Q1 = 1.16065025341614E-01_EB, &
Q2 = 1.50689047360223E-01_EB, &
Q3 = 2.69999308670029E-01_EB, &
Q4 = -7.28846765585675E-02_EB)
PARAMETER ( &
PA = 3.97886080735226000E+00_EB, &
PB = 1.20782237635245222E-01_EB, &
P0 = 2.44044510593190935E-01_EB, &
P1 = 4.34397492331430115E-01_EB, &
P2 = 6.86265948274097816E-01_EB, &
P3 = 9.56464974744799006E-01_EB, &
P4 = 1.16374581931560831E+00_EB, &
P5 = 1.21448730779995237E+00_EB, &
P6 = 1.05375024970847138E+00_EB, &
P7 = 7.13657635868730364E-01_EB, &
P8 = 3.16847638520135944E-01_EB, &
P9 = 1.47297938331485121E-02_EB, &
P10 = -1.05872177941595488E-01_EB, &
P11 = -7.43424357241784861E-02_EB)
PARAMETER ( &
P12 = 2.20995927012179067E-03_EB, &
P13 = 3.46494207789099922E-02_EB, &
P14 = 1.42961988697898018E-02_EB, &
P15 = -1.18598117047771104E-02_EB, &
P16 = -1.12749169332504870E-02_EB, &
P17 = 3.39721910367775861E-03_EB, &
P18 = 6.85649426074558612E-03_EB, &
P19 = -7.71708358954120939E-04_EB, &
P20 = -3.51287146129100025E-03_EB, &
P21 = 1.05739299623423047E-04_EB, &
P22 = 1.12648096188977922E-03_EB)
...
DO I=1,N
   ...
   INDXR(I) = IROW
   INDXC(I) = ICOL
   IF (ABS(A(ICOL,ICOL))<=TWENTY_EPSILON_EB) THEN
      IERROR = 103  ! Singular matrix in gaussj
      RETURN
      ENDIF
...
INTEGER(KIND=8) :: K,BITS53
REAL(EB) :: R

! Build deterministic key from (vent, eddy, cycle, component, seed), then mix.
K = BASE_SEED
...
K = K + INT(Z'9E3779B97F4A7C15',KIND=8)
K = IEOR(K,ISHFT(K,-30))
K = K*INT(Z'BF58476D1CE4E5B9',KIND=8)
K = IEOR(K,ISHFT(K,-27))
K = K*INT(Z'94D049BB133111EB',KIND=8)
K = IEOR(K,ISHFT(K,-31))

BITS53 = IAND(ISHFT(K,-11),INT(Z'001FFFFFFFFFFFFF',KIND=8))
...
USE GLOBAL_CONSTANTS, ONLY: CENTRAL_LIMITER,GODUNOV_LIMITER,MINMOD_LIMITER,SUPERBEE_LIMITER,CHARM_LIMITER,MP5_LIMITER
REAL(EB), INTENT(IN), POINTER, DIMENSION(:,:,:) :: A,U
REAL(EB), INTENT(INOUT), POINTER, DIMENSION(:,:,:) :: F
...
SELECT CASE(LIMITER)
         ...
         DO J=J1,J2
            DO I=I1,I2
               ...
               IF (A(I,J,K)>0._EB) THEN
                  ...
                  R = DU_UP/(DU_LOC+SIGN(TWENTY_EPSILON_EB,DU_LOC))
   ...
   CASE(CHARM_LIMITER) ! CHARM
         ...
         DO J=J1,J2
            DO I=I1,I2
               ...
               IF (A(I,J,K)>0._EB) THEN
                  ...
                  R = 0._EB
                  B = 0._EB
                  IF (ABS(DU_UP)>TWENTY_EPSILON_EB) R = DU_LOC/DU_UP
      ...
      ENDDO
      !$OMP END DO
   CASE(MP5_LIMITER) ! MP5, Suresh and Huynh (1997)
      !$OMP SINGLE
      DO K=K1,K2
         DO J=J1,J2
            DO I=I1,I2
...
IF (PRESENT(Y_IN)) THEN
   IF (ABS(Y_IN(INDEX_IN)-1._EB) > TWENTY_EPSILON_EB) THEN
      DO NS=1,N_SPECIES
         IF (NS==INDEX_IN) CYCLE
         MW_RATIO = MW_RATIO + Y_IN(NS)/SPECIES(NS)%MW
      ENDDO
      IF (MW_RATIO<=TWENTY_EPSILON_EB) THEN
...
ELSE
   IF (ABS(Z_IN(INDEX_IN)-1._EB) > TWENTY_EPSILON_EB) THEN
...
DHOR = H_V_EFF*MW/R0
T_BOIL_EFF = MAX(0._EB,DHOR*T_BOIL_EFF/(DHOR-T_BOIL_EFF*LOG(PRES_IN/P_STP)+TWENTY_EPSILON_EB))
H_V_EFF=H_V_DATA(MIN(I_MAX_TEMP,NINT(T_BOIL_EFF)))
...
ELSE
   ...
   DO I=1,SUM(SF%N_LAYER_CELLS)
      ...
      MATL_REMESH: DO N=1,SF%N_MATL
         IF (SF%RHO_0(I,N)<=TWENTY_EPSILON_EB) CYCLE MATL_REMESH
...
DO N=1,ONE_D%N_MATL
   IF (ONE_D%MATL_COMP(N)%RHO(NODE_INDEX)<=TWENTY_EPSILON_EB) CYCLE
...
CP_LOOP: DO ! Newton method to find solution of T (and hence cpbar) from enthalpy
   ...
   IF (ABS(TMP - TGUESS) < TWENTY_EPSILON_EB) EXIT CP_LOOP
   IF (ABS(TMP - TGUESS)/(TMP+TWENTY_EPSILON_EB) < 0.0005_EB) EXIT CP_LOOP
   IF (ITCOUNT > 10) THEN
...
IF (NUMER <= 0.0_EB) THEN
   EQUIV= 0._EB;
ELSEIF (DENOM <= 0.0_EB) THEN
   EQUIV= 1000._EB; ! Set a high value
ELSE
   EQUIV = NUMER/ DENOM;
ENDIF
...
P_RATIO = PZONE/P_STP
T_BOIL_EFF = MAX(0._EB,DHOR_T_B*T_BOIL/(DHOR_T_B-T_BOIL*LOG(P_RATIO)+TWENTY_EPSILON_EB))
...
!> \brief Prevailing wind components at height Z and time T
!> \param T Current time (s)
!> \param Z Height (m)
!> \param UW Wind component in x (m/s)
!> \param VW Wind component in y (m/s)
!> \param WW Wind component in z (m/s)

SUBROUTINE GET_WIND_AT_HEIGHT(T,Z,UW,VW,WW)

USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
REAL(EB), INTENT(IN) :: T,Z
REAL(EB), INTENT(OUT) :: UW,VW,WW
REAL(EB) :: THETA,SIN_THETA,COS_THETA,SPEED_FAC

IF (I_RAMP_DIRECTION_T/=0 .OR. I_RAMP_DIRECTION_Z/=0) THEN
   IF (I_RAMP_DIRECTION_T==0) THEN
      THETA=EVALUATE_RAMP(Z,I_RAMP_DIRECTION_Z)*DEG2RAD
   ELSEIF (I_RAMP_DIRECTION_Z==0) THEN
      THETA=EVALUATE_RAMP(T,I_RAMP_DIRECTION_T)*DEG2RAD
   ELSE
      THETA=(EVALUATE_RAMP(Z,I_RAMP_DIRECTION_Z)+EVALUATE_RAMP(T,I_RAMP_DIRECTION_T))*DEG2RAD
   ENDIF
   SIN_THETA = -SIN(THETA)
   COS_THETA = -COS(THETA)
ELSE
   ...
   COS_THETA = 1._EB
ENDIF
SPEED_FAC = EVALUATE_RAMP(Z,I_RAMP_SPEED_Z)*EVALUATE_RAMP(T,I_RAMP_SPEED_T)
UW = U0*SPEED_FAC*SIN_THETA
VW = V0*SPEED_FAC*COS_THETA
WW = W0
...
DO K=0,M%KBP1
   CALL GET_WIND_AT_HEIGHT(T,M%ZC(K),M%U_WIND(K),M%V_WIND(K),M%W_WIND(K))
ENDDO
...
IF (OM_SUM_FILM<TWENTY_EPSILON_EB) THEN
...
ELSE
   ...
   IF (ABS(SUM_GAS-1._EB) < TWENTY_EPSILON_EB) THEN
      ZZ_FILM = 0._EB
      DO I=1,N_VAP
         ZZ_FILM(ZZ_INDEX(I)) = Y_FILM(I)
      ENDDO
   ...
   ELSE
      ZZ_FILM = ZZ_GAS
      LOOP2: DO I=1,N_VAP
         IF (ZZ_INDEX(I)==0 .OR. Y_FILM(I)<TWENTY_EPSILON_EB) CYCLE LOOP2
...
X_SUM = 0._EB
LOOP3: DO I=1,N_VAP
   IF (ZZ_INDEX(I)==0 .OR. Y_FILM(I)<TWENTY_EPSILON_EB) CYCLE LOOP3
...
IF (X_SUM > TWENTY_EPSILON_EB) THEN
   D_FILM = D_FILM/X_SUM
ELSE
...
Y_ALL_LOOP: DO I=1,N_MATS
   IF (X_SV(I) < TWENTY_EPSILON_EB) CYCLE
   ZZ_2(SMIX_INDEX(I))=0._EB
ENDDO Y_ALL_LOOP

IF (SUM(ZZ_2) > TWENTY_EPSILON_EB) THEN
   ZZ_2 = ZZ_2 / SUM(ZZ_2)
   MW_DRY = 0._EB
...
DO WHILE (INT(TMP)<I_MAX_TEMP)
   ...
   IF (RHO_S<TWENTY_EPSILON_EB) EXIT
ENDDO
...
IF (F_HRRPUA < TWENTY_EPSILON_EB) THEN
   Q_REF_FIT = Q_INC
   RETURN
ENDIF
...
I_CHI_R = MIN(5,INT(CHI_R*10._EB))
F_CHI_R = MIN(6._EB,CHI_R*10._EB) - REAL(I_CHI_R,EB)
...
!> \param SKIP_BLOWING Optional flag indicating to not perform blowing correction (gauge imbedded in a blowing surface)
...
LOGICAL, INTENT(IN), OPTIONAL :: BACK_SIDE,SKIP_BLOWING
LOGICAL :: BLOWING
...
IF (PRESENT(SKIP_BLOWING)) THEN
   BLOWING = .NOT. SKIP_BLOWING
ELSE
   BLOWING = SF%BLOWING
ENDIF
...
IF (PRESENT(PARTICLE_INDEX_IN)) THEN
   ...
   DN = SF%CONV_LENGTH
   R_DROP = LP%RADIUS
   IF (R_DROP>TWENTY_EPSILON_EB .AND. SF%GEOMETRY/=SURF_CARTESIAN) CONV_LENGTH = 2._EB*R_DROP
...
IF (BLOWING .AND. .NOT. SF%BOUNDARY_FUEL_MODEL .AND. SIM_MODE /= DNS_MODE .AND. ALLOCATED(B1%M_DOT_G_PP_ACTUAL)) THEN
   PHI = 0._EB
   IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) B2%BLOWING_CORRECTION=0._EB
   ITMP = INT(TMP_FILM)
   DO I=1,N_TRACKED_SPECIES
      IF (ABS(B1%M_DOT_G_PP_ACTUAL(I)) <= TWENTY_EPSILON_EB) CYCLE
      PHI = PHI + B1%M_DOT_G_PP_ACTUAL(I)*CP_Z(ITMP,I)
   ENDDO
   IF (ABS(PHI)>TWENTY_EPSILON_EB .AND. ABS(HEAT_TRANSFER_COEFFICIENT)>=TWENTY_EPSILON_EB) THEN
...
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),B1%INT_FTP,UNPACK_IT)
...
DX_MIN = 0._EB
IF (LAYER_THICKNESS<=TWENTY_EPSILON_EB) THEN
   N_CELLS = 0
   DX_MIN = 0._EB
   RETURN
ENDIF
...
IF (FDS_VOLUME>TWENTY_EPSILON_EB) VOLUME_ADJUST = ACTUAL_VOLUME/FDS_VOLUME
...
IF ((X2 <= X0-RAD) .OR. (X1 >= X0+RAD) .OR. (Y2 <= Y0-RAD) .OR. (Y1 >= Y0+RAD) .OR. RAD<TWENTY_EPSILON_EB) THEN
...
IF ((X_MIN-M%XS)>TWENTY_EPSILON_EB .AND. (M%XF-X_MAX)>TWENTY_EPSILON_EB &
   .AND. (Y_MIN-M%YS)>TWENTY_EPSILON_EB .AND. (M%YF-Y_MAX)>TWENTY_EPSILON_EB) THEN
   IF (G_FACTOR==0) THEN
...
IF (SUM(ABS(MV%T34(1:3,1:4))) > TWENTY_EPSILON_EB) THEN
