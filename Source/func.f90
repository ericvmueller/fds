...
SF%N_WALL_STORAGE_REALS    = N_WALL_SCALAR_REALS    + SF%N_ONE_D_STORAGE_REALS    + N_BOUNDARY_PROPERTY_STORAGE_REALS
SF%N_WALL_STORAGE_INTEGERS = N_WALL_SCALAR_INTEGERS + SF%N_ONE_D_STORAGE_INTEGERS + N_BOUNDARY_PROPERTY_STORAGE_INTEGERS
SF%N_WALL_STORAGE_LOGICALS = N_WALL_SCALAR_LOGICALS + SF%N_ONE_D_STORAGE_LOGICALS + N_BOUNDARY_PROPERTY_STORAGE_LOGICALS
...
SF%N_CFACE_STORAGE_REALS    = N_CFACE_SCALAR_REALS    + SF%N_ONE_D_STORAGE_REALS    + N_BOUNDARY_PROPERTY_STORAGE_REALS
SF%N_CFACE_STORAGE_INTEGERS = N_CFACE_SCALAR_INTEGERS + SF%N_ONE_D_STORAGE_INTEGERS + N_BOUNDARY_PROPERTY_STORAGE_INTEGERS
SF%N_CFACE_STORAGE_LOGICALS = N_CFACE_SCALAR_LOGICALS + SF%N_ONE_D_STORAGE_LOGICALS + N_BOUNDARY_PROPERTY_STORAGE_LOGICALS
...
IF (LPC%N_ORIENTATION>0) THEN
   LPC%N_STORAGE_REALS    = N_PARTICLE_SCALAR_REALS + SF%N_ONE_D_STORAGE_REALS + NUMBER_SPECTRAL_BANDS*(NUMBER_RADIATION_ANGLES+1)
ELSE
...
LP%PROP_INDEX          => OS%INTEGERS(11,STORAGE_INDEX) ; IF (NEW) LP%PROP_INDEX = 0

IC = N_PARTICLE_SCALAR_INTEGERS  ! Integer Counter
...
LC = N_PARTICLE_SCALAR_LOGICALS  ! Logical Counter
...
LP%ZI                => OS%REALS(20,STORAGE_INDEX) ; IF (NEW) LP%ZI      = 0._EB

RC = N_PARTICLE_SCALAR_REALS  ! Real Counter
...
! Add ONE_D parameters

CALL ONE_D_POINTERS(LP_INDEX_LOCAL,NEW)
...
WC%VENT_INDEX         => OS%INTEGERS( 9,STORAGE_INDEX) ; IF (NEW) WC%VENT_INDEX = 0
WC%JD11_INDEX         => OS%INTEGERS(10,STORAGE_INDEX) ; IF (NEW) WC%JD11_INDEX = 0
WC%JD12_INDEX         => OS%INTEGERS(11,STORAGE_INDEX) ; IF (NEW) WC%JD12_INDEX = 0
WC%JD21_INDEX         => OS%INTEGERS(12,STORAGE_INDEX) ; IF (NEW) WC%JD21_INDEX = 0
WC%JD22_INDEX         => OS%INTEGERS(13,STORAGE_INDEX) ; IF (NEW) WC%JD22_INDEX = 0
WC%CUT_FACE_INDEX     => OS%INTEGERS(14,STORAGE_INDEX) ; IF (NEW) WC%CUT_FACE_INDEX = 0

IC = N_WALL_SCALAR_INTEGERS  ! Integer Counter
...
RC = N_WALL_SCALAR_REALS  ! Real Counter

! Assign and initialize logicals

LC = N_WALL_SCALAR_LOGICALS  ! LOGICAL Counter

! Add ONE_D (interior) parameters and BOUNDARY_PROPERTY (exterior) parameters

CALL ONE_D_POINTERS(WALL_INDEX_LOCAL,NEW)
CALL BOUNDARY_PROPERTY_POINTERS(WALL_INDEX_LOCAL,NEW)
...
CFA%DUNDT              => OS%REALS(11,STORAGE_INDEX) ; IF (NEW) CFA%DUNDT       = 0._EB

RC = N_CFACE_SCALAR_REALS

! Assign and initialize logicals

LC = N_CFACE_SCALAR_LOGICALS

! Add ONE_D and BOUNDARY_PROPERTY parameters

CALL ONE_D_POINTERS(CFACE_INDEX_LOCAL,NEW)
CALL BOUNDARY_PROPERTY_POINTERS(CFACE_INDEX_LOCAL,NEW)
...
ONE_D%II              => OS%INTEGERS(IC+ 1,STORAGE_INDEX) ; IF (NEW) ONE_D%II            = 0
ONE_D%JJ              => OS%INTEGERS(IC+ 2,STORAGE_INDEX) ; IF (NEW) ONE_D%JJ            = 0
ONE_D%KK              => OS%INTEGERS(IC+ 3,STORAGE_INDEX) ; IF (NEW) ONE_D%KK            = 0
ONE_D%IIG             => OS%INTEGERS(IC+ 4,STORAGE_INDEX) ; IF (NEW) ONE_D%IIG           = 0
ONE_D%JJG             => OS%INTEGERS(IC+ 5,STORAGE_INDEX) ; IF (NEW) ONE_D%JJG           = 0
ONE_D%KKG             => OS%INTEGERS(IC+ 6,STORAGE_INDEX) ; IF (NEW) ONE_D%KKG           = 0
ONE_D%IOR             => OS%INTEGERS(IC+ 7,STORAGE_INDEX) ; IF (NEW) ONE_D%IOR           = 0
ONE_D%PRESSURE_ZONE   => OS%INTEGERS(IC+ 8,STORAGE_INDEX) ; IF (NEW) ONE_D%PRESSURE_ZONE = 0
ONE_D%NODE_INDEX      => OS%INTEGERS(IC+ 9,STORAGE_INDEX) ; IF (NEW) ONE_D%NODE_INDEX    = 0
ONE_D%N_SUBSTEPS      => OS%INTEGERS(IC+10,STORAGE_INDEX) ; IF (NEW) ONE_D%N_SUBSTEPS    = 1

I2 = IC + N_ONE_D_SCALAR_INTEGERS
...
LC = LC + N_ONE_D_SCALAR_LOGICALS  ! Update Logical Counter
...
ONE_D%T_IGN           => OS%REALS(RC+ 8,STORAGE_INDEX) ; IF (NEW) ONE_D%T_IGN           = SF%T_IGN
...
ONE_D%Q_DOT_G_PP      => OS%REALS(RC+22,STORAGE_INDEX) ; IF (NEW) ONE_D%Q_DOT_G_PP      = 0._EB
ONE_D%Q_DOT_O2_PP     => OS%REALS(RC+23,STORAGE_INDEX) ; IF (NEW) ONE_D%Q_DOT_O2_PP     = 0._EB
ONE_D%Q_CONDENSE      => OS%REALS(RC+24,STORAGE_INDEX) ; IF (NEW) ONE_D%Q_CONDENSE      = 0._EB
ONE_D%TMP_F_OLD       => OS%REALS(RC+25,STORAGE_INDEX) ; IF (NEW) ONE_D%TMP_F_OLD       = SF%TMP_FRONT
ONE_D%BURN_DURATION   => OS%REALS(RC+26,STORAGE_INDEX) ; IF (NEW) ONE_D%BURN_DURATION   = SF%BURN_DURATION
ONE_D%T_SCALE         => OS%REALS(RC+27,STORAGE_INDEX) ; IF (NEW) ONE_D%T_SCALE         = 0._EB
ONE_D%Q_SCALE         => OS%REALS(RC+28,STORAGE_INDEX) ; IF (NEW) ONE_D%Q_SCALE         = 0._EB
ONE_D%T_MATL_PART     => OS%REALS(RC+29,STORAGE_INDEX) ; IF (NEW) ONE_D%T_MATL_PART     = 0._EB
ONE_D%B_NUMBER        => OS%REALS(RC+30,STORAGE_INDEX) ; IF (NEW) ONE_D%B_NUMBER        = 0._EB

I2 = RC + N_ONE_D_SCALAR_REALS  ! I1 and I2 continue counting reals
...
BP%U_TAU           => OS%REALS(RC+1,STORAGE_INDEX) ; IF (NEW) BP%U_TAU = 0._EB
BP%Y_PLUS          => OS%REALS(RC+2,STORAGE_INDEX) ; IF (NEW) BP%Y_PLUS = 1._EB
BP%Z_STAR          => OS%REALS(RC+3,STORAGE_INDEX) ; IF (NEW) BP%Z_STAR = 1._EB
BP%PHI_LS          => OS%REALS(RC+4,STORAGE_INDEX) ; IF (NEW) BP%PHI_LS = -1._EB
BP%WORK1           => OS%REALS(RC+5,STORAGE_INDEX) ; IF (NEW) BP%WORK1 = 0._EB
BP%WORK2           => OS%REALS(RC+6,STORAGE_INDEX) ; IF (NEW) BP%WORK2 = 0._EB
BP%K_SUPPRESSION   => OS%REALS(RC+7,STORAGE_INDEX) ; IF (NEW) BP%K_SUPPRESSION   = 0._EB
BP%L_OBUKHOV       => OS%REALS(RC+8,STORAGE_INDEX) ; IF (NEW) BP%L_OBUKHOV       = 0._EB

I2 = RC + N_BOUNDARY_PROPERTY_SCALAR_REALS
