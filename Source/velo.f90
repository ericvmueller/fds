      ...
      DO J=OB%J1+1,OB%J2
         DO I=OB%I1  ,OB%I2
            ...
            IF (CELL(IC1)%SOLID .AND. CELL(IC2)%SOLID) THEN
               ...
               IF (TUNNEL_PRECONDITIONER) FVX(I,J,K) = FVX(I,J,K) + DUDT_BAR_P(I_OFFSET(NM)+I)
            ENDIF
         ENDDO
      ENDDO
   ENDDO
...
ELSE FREEZE_VELOCITY_IF
   ...
   IF (TUNNEL_PRECONDITIONER) THEN
      ...
      DO I=0,IBAR
         US(I,1:JBAR,1:KBAR) = US(I,1:JBAR,1:KBAR) + DT*DUDT_BAR(I_OFFSET(NM)+I)
      ENDDO
      !$OMP END PARALLEL DO
   ENDIF
...
ELSE FREEZE_VELOCITY_IF
   ...
   IF (TUNNEL_PRECONDITIONER) THEN
      !$OMP PARALLEL DO PRIVATE(I) SCHEDULE(STATIC)
      DO I=0,IBAR
         U(I,1:JBAR,1:KBAR) = U(I,1:JBAR,1:KBAR) + 0.5_EB*DT*DUDT_BAR_S(I_OFFSET(NM)+I)
      ENDDO
      !$OMP END PARALLEL DO
   ENDIF
...
REAL(EB) :: UODX,VODY,WODZ,UVW,UVWMAX,R_DX2,MU_MAX,MUTRM,PART_CFL,MU_TMP, UVWMAX_TMP, DT_CLIP, T_NOW, Y_O2,M_DOT_O2_PP
REAL(EB) :: DT_NEW(NMESHES)
INTEGER  :: I,J,K,IW,IIG,JJG,KKG, ICFL_TMP, JCFL_TMP, KCFL_TMP, IP
...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   ...
   PART_LOOP1: DO IP=1,NLP
      LP=>LAGRANGIAN_PARTICLE(IP)
      LPC=>LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      IF (.NOT. LPC%SOLID_PARTICLE) CYCLE PART_LOOP1
      BC=>BOUNDARY_COORD(LP%BC_INDEX)
      B1=>BOUNDARY_PROP1(LP%B1_INDEX)
      IIG = BC%IIG
      JJG = BC%JJG
      KKG = BC%KKG
      UVW = (ABS(B1%Q_CON_F)/B1%RHO_F)**ONTH * 2._EB*(LP%RVC**ONTH)
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL=IIG
         JCFL=JJG
...
MASS_TRANSFER_IF: IF (CHECK_MT) THEN
   PART_LOOP2: DO IP=1,NLP
      LP=>LAGRANGIAN_PARTICLE(IP)
      LPC=>LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      IF (.NOT. LPC%SOLID_PARTICLE) CYCLE PART_LOOP2
      BC=>BOUNDARY_COORD(LP%BC_INDEX)
      B1=>BOUNDARY_PROP1(LP%B1_INDEX)
      IIG = BC%IIG
      JJG = BC%JJG
      KKG = BC%KKG
      CALL GET_MASS_FRACTION(B1%ZZ_G,O2_INDEX,Y_O2)
      M_DOT_O2_PP = LP%PWT*DOT_PRODUCT(Z2Y(O2_INDEX,1:N_TRACKED_SPECIES),B1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES))
      UVW = ABS(M_DOT_O2_PP/(B1%RHO_F*Y_O2+TWO_EPSILON_EB)) * 0.1_EB*(LP%RVC**ONTH)
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL=IIG
         JCFL=JJG
