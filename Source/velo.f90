...
MASS_TRANSFER_IF: IF (CHECK_MT) THEN
   PART_LOOP: DO IP=1,NLP
      LP=>LAGRANGIAN_PARTICLE(IP)
      LPC=>LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      IF (.NOT. LPC%SOLID_PARTICLE) CYCLE PART_LOOP
      ...
      M_DOT_O2_PP = DOT_PRODUCT(Z2Y(O2_INDEX,1:N_TRACKED_SPECIES),B1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES))
      UVW = ABS(M_DOT_O2_PP/(B1%RHO_G*Y_O2)) * 4._EB*(LP%RVC**ONTH)
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL=IIG
         JCFL=JJG
