...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   ...
   ! Particle heat transfer: WORK1 = sum (|Q_CON_F|/RHO_F)*AREA*PWT*RVC, WORK2 = sum AREA*PWT*RVC
   ! then UVW = ((q''/rho)_avg)^(1/3)*AREA_TOT*RVC
   ...
   SOLID_PARTICLES_IF: IF (SOLID_PARTICLES) THEN
      ...
      PARTICLE_LOOP: DO IP=1,NLP
         ...
         WORK2(IIG,JJG,KKG) = WORK2(IIG,JJG,KKG) + B1%AREA*LP%PWT*LP%RVC
      ENDDO PARTICLE_LOOP
      WHERE (WORK2>TWO_EPSILON_EB)
         WORK1 = (WORK1/WORK2)**ONTH * WORK2
      ELSEWHERE
         WORK1 = 0._EB
      END WHERE
      ICFL_ARR = MAXLOC(WORK1(1:IBAR,1:JBAR,1:KBAR))
      UVW = WORK1(ICFL_ARR(1),ICFL_ARR(2),ICFL_ARR(3))
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL = ICFL_ARR(1)
         JCFL = ICFL_ARR(2)
