...
INTEGER :: I,J,K,IIG,JJG,KKG,II,JJ,IW,IC
...
! Compute viscosity in near-wall gas cells (IIG,JJG,KKG). Corner cells use the average of surrounding solid cells.
...
WALL_LOOP_1: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   WC=>WALL(IW)
   IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE WALL_LOOP_1
   BC=>BOUNDARY_COORD(WC%BC_INDEX)
   IIG = BC%IIG
   JJG = BC%JJG
   KKG = BC%KKG
...
! Mirror gas phase viscosity into ghost cells (II,JJ,KK) at solid or external boundaries.
...
WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (CELL(IC)%SOLID .OR. CELL(IC)%EXTERIOR) THEN
      KRES(BC%II,BC%JJ,BC%KK) = KRES(BC%IIG,BC%JJG,BC%KKG)
      MU(BC%II,BC%JJ,BC%KK) = MU(BC%IIG,BC%JJG,BC%KKG)
   ENDIF
ENDDO WALL_LOOP_2
...
! Copy MU and KRES into edge cells of domain
...
USE CC_SCALARS, ONLY : CC_VELOCITY_BC,CC_RESCALE_OMESH_STAGGERED_TO_CUTFACE,GET_OPENBC_TANGENTIAL_CUTFACE_VEL
...
IF (CC_IBM) CALL CC_RESCALE_OMESH_STAGGERED_TO_CUTFACE(NM,APPLY_TO_ESTIMATED_VARIABLES)
...
IF(CC_IBM) CALL CC_VELOCITY_BC(T,NM,APPLY_TO_ESTIMATED_VARIABLES,DO_IBEDGES=.TRUE.,USE_OM_VEL_CF=.TRUE.)
...
REAL(EB) :: DT_NEW(NMESHES)
INTEGER  :: I,J,K,IW,IP,IIG,JJG,KKG, ICFL_TMP, JCFL_TMP, KCFL_TMP, ICFL_ARR(3)
...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   ...
   ! Particle heat transfer (volumetric): WORK1 = sum (|Q_CON_F|/RHO_F)*AREA*PWT*RVC = q'''/rho
   ! then UVW = (q'''/rho)^(1/3)

   SOLID_PARTICLES_IF: IF (SOLID_PARTICLES) THEN
      WORK1 = 0._EB
      PARTICLE_LOOP: DO IP=1,NLP
         LP => LAGRANGIAN_PARTICLE(IP)
         LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
         IF (.NOT.LPC%SOLID_PARTICLE .OR. LP%RVC<=TWO_EPSILON_EB) CYCLE PARTICLE_LOOP
         BC => BOUNDARY_COORD(LP%BC_INDEX)
         B1 => BOUNDARY_PROP1(LP%B1_INDEX)
         IIG = BC%IIG
         JJG = BC%JJG
         KKG = BC%KKG
         WORK1(IIG,JJG,KKG) = WORK1(IIG,JJG,KKG) + &
            ABS(B1%Q_CON_F)/MAX(B1%RHO_F,TWO_EPSILON_EB)*B1%AREA*LP%PWT*LP%RVC
      ENDDO PARTICLE_LOOP
      ICFL_ARR = MAXLOC(WORK1(1:IBAR,1:JBAR,1:KBAR))
      UVW = WORK1(ICFL_ARR(1),ICFL_ARR(2),ICFL_ARR(3))**ONTH*0.1_EB
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL = ICFL_ARR(1)
         JCFL = ICFL_ARR(2)
