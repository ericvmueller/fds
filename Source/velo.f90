...
REAL(EB) :: MUA,TSI,WGT,T_NOW,RAMP_T,OMW,MU_WALL,RHO_WALL,SLIP_COEF,VEL_T, &
            ...
            DRAG_FACTOR,HT_SCALE_FACTOR,VEG_HT,VEL_N,UN_OTHER,L_STAB
         ...
         INTERPOLATION_IF: IF (NOM(ICD)==0 .OR. &
            ...
            BOUNDARY_CONDITION: SELECT CASE(VELOCITY_BC_INDEX)
               ...
               CASE (BOUNDARY_FUEL_MODEL_BC) BOUNDARY_CONDITION
                  ...
                  SELECT CASE(IEC)
                     CASE(IAXIS) ; L_STAB = DXN(II)
                     CASE(JAXIS) ; L_STAB = DYN(JJ)
                     CASE(KAXIS) ; L_STAB = DZN(KK)
                  END SELECT
                  DRAG_STABILITY_MAX = MAX(DRAG_STABILITY_MAX,SQRT(ABS(MU_DUIDXJ(ICD_SGN))/ &
                                       (RHO_WALL*L_STAB**2+TWO_EPSILON_EB)))
                  ALTERED_GRADIENT(ICD_SGN) = .TRUE.
...
DRAG_CFL = DT*DRAG_STABILITY_MAX
...
ELSE RAMP_TIME_IF
   ...
   IF ((CFL<CFL_MAX .AND. VN<VN_MAX .AND. PART_CFL<PARTICLE_CFL_MAX .AND. DRAG_CFL<DRAG_CFL_MAX) .OR. LOCK_TIME_STEP) THEN
      DT_NEW(NM) = DT
      IF (CFL<=CFL_MIN .AND. VN<VN_MIN .AND. PART_CFL<PARTICLE_CFL_MIN .AND. DRAG_CFL<DRAG_CFL_MIN &
   ...
   ELSE
      DT_NEW(NM) = 0.9_EB*MIN( CFL_MAX/MAX(UVWMAX,DT_EPS)               , &
                               ...
                               DRAG_CFL_MAX/MAX(DRAG_STABILITY_MAX,DT_EPS) , &
                               DT_CLIP)
      CHANGE_TIME_STEP_INDEX(NM) = -1
   ENDIF
