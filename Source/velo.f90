...
REAL(EB) :: MUA,TSI,WGT,T_NOW,RAMP_T,OMW,MU_WALL,RHO_WALL,SLIP_COEF,VEL_T, &
            ...
            DRAG_FACTOR,HT_SCALE_FACTOR,VEG_HT,VEL_N,UN_OTHER,DRAG_UVWMAX_PARTICLE
...
DRAG_UVWMAX_PARTICLE = DRAG_UVWMAX
DRAG_UVWMAX = 0._EB
...
DRAG_UVWMAX = MAX(DRAG_UVWMAX,DRAG_UVWMAX_PARTICLE)
...
REAL(EB) :: UODX,VODY,WODZ,UVW,UVWMAX,R_DX2,MU_MAX,MUTRM,PART_CFL,DRAG_CFL,MU_TMP, UVWMAX_TMP, DT_CLIP, T_NOW
REAL(EB) :: DT_NEW(NMESHES)
...
ELSE RAMP_TIME_IF
   ...
   IF ((CFL<CFL_MAX .AND. VN<VN_MAX .AND. PART_CFL<PARTICLE_CFL_MAX .AND. DRAG_CFL<PARTICLE_CFL_MAX) .OR. LOCK_TIME_STEP) THEN
      DT_NEW(NM) = DT
      IF (CFL<=CFL_MIN .AND. VN<VN_MIN .AND. PART_CFL<PARTICLE_CFL_MIN .AND. DRAG_CFL<PARTICLE_CFL_MIN &
         .AND. .NOT.LOCK_TIME_STEP) THEN
   ...
   ELSE
      DT_NEW(NM) = 0.9_EB*MIN( CFL_MAX/MAX(UVWMAX,DT_EPS)               , &
                               ...
                               PARTICLE_CFL_MAX/MAX(DRAG_UVWMAX,DT_EPS) , &
                               DT_CLIP)
      CHANGE_TIME_STEP_INDEX(NM) = -1
   ENDIF
