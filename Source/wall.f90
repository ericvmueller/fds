...
REAL(EB), INTENT(IN) :: T
REAL(EB) :: DT_BC,DTMP,TSI,UBAR,VBAR,WBAR,RAMP_FACTOR,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),RSUM_G,MU_G
   ...
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
         ...
         IF (LPC%MASSLESS_TARGET) THEN
            ...
            ELSE
               B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(B1%TMP_G-PY%GAUGE_TEMPERATURE,SF%H_FIXED,SURF_INDEX,&
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER  ! Only for WALL cells
      ...
      ELSE
         B1%TMP_F = B1%TMP_G
         B1%ZZ_F(1:N_TRACKED_SPECIES) = ZZ_G(1:N_TRACKED_SPECIES)
      ENDIF
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,WALL_INDEX_IN=WALL_INDEX)
      ELSEIF (PRESENT(CFACE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,CFACE_INDEX_IN=CFACE_INDEX)
      ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,PARTICLE_INDEX_IN=PARTICLE_INDEX)
      ENDIF
...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%HT_DIM==1 .OR. WC%BACK_INDEX<=0) CYCLE WALL_LOOP
...
SELECT CASE(SIM_MODE)
   CASE DEFAULT
      DO N=1,N_TRACKED_SPECIES
         B1%RHO_D_F(N) = MU_G*RSC*B1%RHO_F/RHO_G
      ENDDO
   CASE (LES_MODE)
      ...
      KKG = BC%KKG
      DO N=1,N_TRACKED_SPECIES
         B1%RHO_D_F(N) = B1%RHO_F*( D_Z(ITMP,N) + (MU_G-MU_DNS(IIG,JJG,KKG))/RHO_G*RSC )
      ENDDO
   CASE (DNS_MODE)
      ITMP = MIN(I_MAX_TEMP-1,NINT(B1%TMP_F))
      DO N=1,N_TRACKED_SPECIES
...
REAL(EB) :: RADIUS,AREA_SCALING,RVC,M_DOT_PPP_SINGLE,M_DOT_SINGLE,CP,MW_RATIO,H_G,&
            ZZ_GET(1:N_TRACKED_SPECIES),DENOM,M_GAS,RHO_G,ZZ_G(1:N_TRACKED_SPECIES)
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(ICG)%SOLID) &
       ZZP(II,JJ,KK,1:N_TRACKED_SPECIES) = 2._EB*B1%ZZ_F(1:N_TRACKED_SPECIES) - ZZ_G(1:N_TRACKED_SPECIES)
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. N_REACTIONS>=1 .AND. PREDICTOR .AND. B1%T_IGN <=T) THEN
         ...
         ELSE
            B1%T_SCALE = B1%T_SCALE + DT * MAX(0._EB,B1%Q_IN_SMOOTH) / (SF%REFERENCE_HEAT_FLUX)
         ENDIF
         CALL INTERPOLATE1D_UNIFORM(1,RP%INTERPOLATED_DATA(1:RP%NUMBER_INTERPOLATION_POINTS),B1%T_SCALE*RP%RDT,Q_NEW)
      ENDIF
      ...
      SUM_MASSFLUX_LOOP: DO N=1,N_TRACKED_SPECIES
         IF (ABS(SF%MASS_FLUX(N)) > TWO_EPSILON_EB) THEN  ! Use user-specified ramp-up of mass flux
            ...
            ENDIF
            B1%M_DOT_G_PP_ADJUST(N) = SF%ADJUST_BURN_RATE(N)*B1%M_DOT_G_PP_ACTUAL(N)*B1%AREA_ADJUST
         ENDIF
      ...
      ELSEIF (PRESENT(WALL_INDEX) .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(IC)%EXTERIOR) THEN  ! this is a thin obstruction
         ...
         IF (CORRECTOR) THEN  ! calculate the mass production rate of gases in the adjacent gas cell
            ...
            CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,B1%TMP_G)
            H_G = CP*B1%TMP_G
            D_SOURCE(IIG,JJG,KKG) = D_SOURCE(IIG,JJG,KKG) - B1%Q_DOT_G_PP*B1%AREA*RVC/(H_G*RHO_G)
         ENDIF
      ELSE
         ...
         DO ITER=1,3
            ...
            SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
               ...
               IF ( ABS(DENOM) > TWO_EPSILON_EB ) THEN
                  B1%ZZ_F(N) = ( B1%M_DOT_G_PP_ADJUST(N) + DD*ZZ_G(N) ) / DENOM
               ELSE
                  B1%ZZ_F(N) = ZZ_G(N)
               ENDIF
            ENDDO SPECIES_LOOP
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. BOUNDARY_TYPE/=OPEN_BOUNDARY) RHOP(BC%II,BC%JJ,BC%KK) = 2._EB*B1%RHO_F - RHO_G
ENDDO WALL_CELL_LOOP
...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),UN,MFT,RSUM_F,H_D,H_G,ZZ_G(1:N_TRACKED_SPECIES)
...
UNPACK_WALL_PARTICLE: IF (PRESENT(WALL_INDEX)) THEN
   ...
   IF (WC%BACK_INDEX>0 .AND. WC%BACK_MESH==NM) THEN
      ...
      B1_BACK => BOUNDARY_PROP1(WC_BACK%B1_INDEX)
      B2_BACK => BOUNDARY_PROP2(WC_BACK%B2_INDEX)
      BC_BACK => BOUNDARY_COORD(WC_BACK%BC_INDEX)
      SURF_INDEX_BACK = WC_BACK%SURF_INDEX
   ELSEIF (WC%BACK_INDEX>0 .AND. WC%BACK_MESH/=NM) THEN
      WC_BACK => MESHES(WC%BACK_MESH)%WALL(WC%BACK_INDEX)
      B1_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_PROP1(WC_BACK%B1_INDEX)
      B2_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_PROP2(WC_BACK%B2_INDEX)
      BC_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_COORD(WC_BACK%BC_INDEX)
      SURF_INDEX_BACK = WC_BACK%SURF_INDEX
   ENDIF
   BACKING = SF%BACKING
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (SF%HT_DIM>1 .AND. ABS(BC%IOR)/=HT_3D_SWEEP_DIRECTION) RETURN
   I_OBST = TW%OBST_INDEX
   IWB = TW%BACK_INDEX  ! Wall cell index of backside of side
   IF (TW%BACK_INDEX>0 .AND. TW%BACK_MESH==NM) THEN
      TW_BACK => THIN_WALL(TW%BACK_INDEX)
      B1_BACK => BOUNDARY_PROP1(TW_BACK%B1_INDEX)
      BC_BACK => BOUNDARY_COORD(TW_BACK%BC_INDEX)
      SURF_INDEX_BACK = TW_BACK%SURF_INDEX
   ELSEIF (TW%BACK_INDEX>0 .AND. TW%BACK_MESH/=NM) THEN
      TW_BACK => MESHES(TW%BACK_MESH)%THIN_WALL(TW%BACK_INDEX)
      B1_BACK => MESHES(TW%BACK_MESH)%BOUNDARY_PROP1(TW_BACK%B1_INDEX)
      BC_BACK => MESHES(TW%BACK_MESH)%BOUNDARY_COORD(TW_BACK%BC_INDEX)
      SURF_INDEX_BACK = TW_BACK%SURF_INDEX
   ENDIF
   BACKING = SF%BACKING
   B1%TMP_G = TMP(BC%IIG,BC%JJG,BC%KKG)
...
COMPUTE_GRID: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .OR. SF%HT_DIM>1) THEN
...
SELECT CASE(BACKING)
   ...
   CASE(EXPOSED)  ! The backside is exposed to gas in current or adjacent mesh.
      ...
      ELSE  ! Back side is in current mesh.
         ...
         IF (PRESENT(THIN_WALL_INDEX)) HTCB = 0._EB
...
IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .AND. .NOT.PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (PRESENT(WALL_INDEX)) THEN
      IF (OBSTRUCTION(I_OBST)%THIN .AND. .NOT.CELL(CELL_INDEX(BC%II,BC%JJ,BC%KK))%SOLID) RDT = 1._EB/DT_BC
   ENDIF
   ...
   B1%M_DOT_G_PP_ACTUAL       = B1%M_DOT_G_PP_ACTUAL       +                M_DOT_G_PP_ACTUAL*DT_BC_SUB*RDT
...
POINT_LOOP3: DO I=1,NWP
   ...
   MATERIAL_LOOP3: DO N=1,ONE_D%N_MATL
      ...
      IF (.NOT.E_FOUND) B1%EMISSIVITY = B1%EMISSIVITY + ONE_D%MATL_COMP(N)%RHO(I)*ML%EMISSIVITY/SF%RHO_S(LAYER_INDEX(I),N)
...
ELSEIF (.NOT.RADIATION .OR. SF%INTERNAL_RADIATION .OR. PRESENT(THIN_WALL_INDEX)) THEN
   RFACF  = 0.5_EB*HTCF
...
ELSEIF (PRESENT(WALL_INDEX_IN)) THEN
   ...
   P1X => BOUNDARY_PROP1(WCX%B1_INDEX)
   P2X => BOUNDARY_PROP2(WCX%B2_INDEX)
...
IF ( (SIM_MODE==DNS_MODE .OR. SOLID_PHASE_ONLY) .AND. (PRESENT(WALL_INDEX_IN) .OR. PRESENT(CFACE_INDEX_IN)) ) THEN
   ...
   IF (PRESENT(WALL_INDEX_IN)) THEN
      ...
      SELECT CASE(BCX%IOR)
         ...
         CASE(-3); IC2=CELL_INDEX(IIG,JJG,KKG-1)
      END SELECT
      IF (CELL(IC2)%SOLID .OR. CELL(IC2)%EXTERIOR) THEN
         HEAT_TRANSFER_COEFFICIENT = 2._EB * P1X%K_G * P1X%RDN
         RETURN
      ENDIF
      SELECT CASE(BCX%IOR)
...
TMP_FILM = 0.5_EB*(TMP_G+P1X%TMP_F)
...
HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
   ...
   CASE(RAYLEIGH_HTC_MODEL)
      CALL GET_SPECIFIC_HEAT(ZZ_G,CP_G,TMP_FILM)
      CALL RAYLEIGH_HEAT_FLUX_MODEL(H_NATURAL,ZSTAR,DN,P1X%TMP_F,TMP_G,K_G,RHO_G,CP_G,MU_G)
      IF (PRESENT(WALL_INDEX_IN) .OR. PRESENT(CFACE_INDEX_IN)) P2X%Z_STAR = ZSTAR
