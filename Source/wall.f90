...
REAL(EB), INTENT(IN) :: T,DT
REAL(EB) :: DT_BC,DTMP,DT_BC_HT3D,TMP_FRONT,WGT,TSI,UBAR,VBAR,WBAR,RAMP_FACTOR,TMP_G,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),RSUM_G,MU_G
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      ELSE
         TMP_FRONT = TMP_0(KKG)
      ENDIF
      ONE_D%TMP_F = WGT*TMP_G + (1._EB-WGT)*TMP_FRONT
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   IF (LPC%DRAG_LAW /= SCREEN_DRAG .AND. LPC%DRAG_LAW /= POROUS_DRAG) THEN
      SELECT CASE(SF%GEOMETRY)
         CASE(SURF_CARTESIAN)
            IF (.NOT. (LP%EMBER .AND. .NOT. LPC%MONODISPERSE)) ONE_D%AREA = 2._EB*SF%LENGTH*SF%WIDTH
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      CONSUME_MASS: IF (OBST_INDEX>0 .AND. OBSTRUCTION(OBST_INDEX)%CONSUMABLE .AND. CORRECTOR .AND. &
                        SF%THERMAL_BC_INDEX/=THERMALLY_THICK_HT3D) THEN
...
INTEGER :: IIB,JJB,KKB,IWB,NWP,I,NR,NL,N,I_OBST,NS,N_LAYER_CELLS_NEW(MAX_LAYERS),N_CELLS,EXPON,ITMP,ITER
...
DTMP = TMP_G-0.5_EB*(ONE_D%TMP_F_OLD+ONE_D%TMP_F)

ONE_D%TMP_F_OLD = ONE_D%TMP_F  ! Save this value for use at the next time step
...
REAL(EB) :: RE,H_NATURAL,H_FORCED,NUSSELT,FRICTION_VELOCITY=0._EB,YPLUS=0._EB,ZSTAR,DN,TMP_FILM,MU_G,K_G,CP_G,&
            RA,X_W,TMP_G,MW_RATIO,Y_DROP,Y_GAS,B_NUMBER,Y_AIR,ZZ_AIR(1:N_TRACKED_SPECIES),MU_AIR,K_AIR,CP_AIR,R_AIR,RHO_AIR,&
            LENGTH,R_DROP,VREL,U2,V2,W2,U_SURF,V_SURF,W_SURF,H_R,H_R_B,TMP_2,H_1,H_2,GAMMA,DTDX_W,&
            T_BOIL_EFF,Y_ALL(1:N_SPECIES),EVAP_FILM_FAC,DUMMY,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),BFAC
...
HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
   CASE(DEFAULT_HTC_MODEL)
      ...
      RE = RHO_G*ONE_DX%U_TANG*SFX%CONV_LENGTH/MU_G
   ...
   CASE(ABL_HTC_MODEL)
      ...
      KKG=BCX%KKG
      H_NATURAL = 0._EB
      CALL GET_CONDUCTIVITY(ZZ_G,K_G,TMP_G)
      CALL GET_SPECIFIC_HEAT(ZZ_G,CP_G,TMP_G)
   ...
   CASE(YUAN_HTC_MODEL)
      ...
      CALL GET_CONDUCTIVITY(ZZ_G,K_G,TMP_FILM)
      CALL GET_SPECIFIC_HEAT(ZZ_G,CP_G,TMP_FILM)
      CALL YUAN_HEAT_FLUX_MODEL(H_NATURAL,ZSTAR,DN,ONE_DX%TMP_F,TMP_G,K_G,RHO_G,CP_G)
      IF (PRESENT(WALL_INDEX_IN) .OR. PRESENT(CFACE_INDEX_IN)) BPX%Z_STAR = ZSTAR
   CASE(FREE_HORIZONTAL_CYLINDER_HTC_MODEL)
      ...
      CALL GET_CONDUCTIVITY(ZZ_G,K_G,TMP_FILM)
      CALL GET_SPECIFIC_HEAT(ZZ_G,CP_G,TMP_FILM)
      RA = GRAV*ABS(DELTA_TMP)*SFX%CONV_LENGTH**3*RHO_G**2*CP_G/(TMP_FILM*MU_G*K_G)
   ...
   CASE(BLOWING_HTC_MODEL)
      ...
      ELSE
         Y_GAS   = ZZ_G(SMIX_INDEX)
         CALL GET_MW_RATIO(SMIX_INDEX,MW_RATIO,Z_IN=ZZ_G)
      ENDIF
      ...
      CALL NATURAL_CONVECTION_MODEL(H_NATURAL,DELTA_TMP,SFX%C_VERTICAL,SFX%C_HORIZONTAL,SFX%GEOMETRY,BCX%IOR,K_AIR,LENGTH)
      CALL FORCED_CONVECTION_MODEL(H_FORCED,RE,K_AIR,PR_AIR**ONTH,LENGTH,SFX%GEOMETRY)
      IF (Y_DROP-Y_GAS>TWO_EPSILON_EB) THEN
         BFAC = LOG(1._EB+B_NUMBER)/B_NUMBER
         H_NATURAL = H_NATURAL*BFAC
         H_FORCED  = H_FORCED *BFAC
      ENDIF
