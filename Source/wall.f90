...
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_GAS_CONSTANT,GET_SENSIBLE_ENTHALPY,GET_MASS_FRACTION
...
REAL(EB) :: RDT,DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2,Q_RAD_IN_B,RFLUX_UP,RFLUX_DOWN,E_WALLB, &
            ...
            RADIUS,HTC_LIMIT,CP1,CP2,DENOM,SF_HTC_F,SF_HTC_B,DDSUM,THICKNESS,DT_FO,AVAILABLE_O2,CONSUMED_O2,Y_O2
REAL(EB), DIMENSION(N_TRACKED_SPECIES) :: M_DOT_G_PP_ADJUST,M_DOT_G_PP_ADJUST_NET,M_DOT_G_PP_ACTUAL,M_DOT_G_PP_ACTUAL_NET,ZZ_GET
...
! Check for excessive O2 consumption
CONSUMED_O2=-B1%M_DOT_G_PP_ACTUAL(O2_INDEX)*DT_BC*B1%AREA
CALL GET_MASS_FRACTION(ZZ_GET,O2_INDEX,Y_O2)
AVAILABLE_O2 = Y_O2*RHO(BC%II,BC%JJ,BC%KK)*DX(BC%II)*RC(BC%II)*DY(BC%JJ)*DZ(BC%KK)
IF (CONSUMED_O2>AVAILABLE_O2) PRINT*, 'O2 OVERCONSUMPTION ERROR',CONSUMED_O2,AVAILABLE_O2
