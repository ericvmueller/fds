...
IF (SOLID_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO IP=1, NLP
      ...
      IF (LPC%SOLID_PARTICLE) THEN
         ...
         IF (CORRECTOR) CALL DEPOSIT_PARTICLE_MASS(NM,IP,LP,LPC,CALL_HT_1D)  ! Add the particle off-gas to the gas phase mesh
      ENDIF
...
REAL(EB) :: ARO,QNET,RAMP_FACTOR,RHO_G_2,RSUM_F,PBAR_F,TMP_OTHER_SOLID,TSI,UN, &
      ...
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
               SELECT CASE(BC%IOR)
                  CASE( 1)
                     ARO = MIN(1._EB , (MM%DY(JJO)*MM%DZ(KKO))/(DY(BC%JJ)*DZ(BC%KK)) )
                  CASE(-1)
                     ARO = MIN(1._EB , (MM%DY(JJO)*MM%DZ(KKO))/(DY(BC%JJ)*DZ(BC%KK)) )
                  CASE( 2)
                     ARO = MIN(1._EB , (MM%DX(IIO)*MM%DZ(KKO))/(DX(BC%II)*DZ(BC%KK)) )
                  CASE(-2)
                     ARO = MIN(1._EB , (MM%DX(IIO)*MM%DZ(KKO))/(DX(BC%II)*DZ(BC%KK)) )
                  CASE( 3)
                     ARO = MIN(1._EB , (MM%DX(IIO)*MM%DY(JJO))/(DX(BC%II)*DY(BC%JJ)) )
                     DDO = (DZ(BC%KK)+DZ(BC%KKG))/(MM%DZ(KKO)+DZ(BC%KKG))
                  CASE(-3)
                     ARO = MIN(1._EB , (MM%DX(IIO)*MM%DY(JJO))/(DX(BC%II)*DY(BC%JJ)) )
                     DDO = (DZ(BC%KK)+DZ(BC%KKG))/(MM%DZ(KKO)+DZ(BC%KKG))
               END SELECT
...
SUBROUTINE DEPOSIT_PARTICLE_MASS(NM,IP,LP,LPC,CALL_HT_1D)
...
CALC_LP_MASS: IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
   SELECT CASE (SF%GEOMETRY)
      CASE (SURF_CARTESIAN)
         LP%MASS = B1%AREA*SF%THICKNESS*SURFACE_DENSITY(NM,1,LAGRANGIAN_PARTICLE_INDEX=IP)
       CASE (SURF_CYLINDRICAL)
         LP%MASS = LENGTH*PI*(SF%INNER_RADIUS+SF%THICKNESS)**2*SURFACE_DENSITY(NM,1,LAGRANGIAN_PARTICLE_INDEX=IP)
      CASE (SURF_SPHERICAL)
...
REAL(EB), DIMENSION(MAX_LAYERS) :: SMALLEST_CELL_SIZE,DX_MIN
REAL(EB), DIMENSION(MAX_LAYERS,MAX_MATERIALS) :: RHO_ADJUSTED
REAL(EB), DIMENSION(NWP_MAX) :: AAS,BBS,CCS,DDS,DDT,Q_S,TWO_DX_KAPPA_S,DX_S,MF_FRAC,REGRID_FACTOR
...
USE MATH_FUNCTIONS, ONLY: INTERPOLATE1D_UNIFORM,IERFC
...
REAL(EB) :: REACTION_RATE,Y_O2,X_O2,Q_DOT_S_PPP,MW(N_MATS),Y_GAS(N_MATS),Y_TMP(N_MATS),Y_SV(N_MATS),X_SV(N_MATS),X_L(N_MATS),&
            ...
            RHO_DOT,DR,R_S_0,R_S_1,H_R,H_R_B,H_S_B,H_S,LENGTH_SCALE,SUM_Y_GAS,SUM_Y_SV,NU_O2_CHAR,Y_O2_S,Z_E,&
   ...
   REACTION_LOOP: DO J=1,ML%N_REACTIONS  ! Loop over the reactions (beta subscript)
      ...
      SELECT CASE (ML%PYROLYSIS_MODEL)
         ...
         CASE (PYROLYSIS_SOLID)
            ...
            ! DAE model
            IF (ML%SIGMA_E(J)>0._EB) THEN
               Z_E = IERFC(-2*RHO_S(N)/SF%RHO_0(SOLID_CELL_INDEX,N))
               REACTION_RATE = ML%A(J)*(RHO_S(N))**ML%N_S(J)*EXP(-(ML%E(J)+ML%SIGMA_E(J)*Z_E)/(R0*TMP_S))
            ENDIF
      ...
      ! Limit mass loss rate if the layer is nearing removal

      IF (REMOVE_LAYER) RHO_DOT = RHO_S(N)/DT_BC
...
DO I=1,N_TGA
   ...
   IF (TGA_WALL_INDEX>0) THEN
      CALL SOLID_HEAT_TRANSFER(1,T_TGA,DT_TGA,WALL_INDEX=IW)
   ELSE
      CALL SOLID_HEAT_TRANSFER(1,T_TGA,DT_TGA,PARTICLE_INDEX=IP)
   ENDIF
