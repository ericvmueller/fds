...
IF (T>=UVW_CLOCK(UVW_COUNTER(NM))) THEN
   IF (PERIODIC_TEST==9) THEN
      WRITE(FN_SPEC,'(A,A,I0,A)') TRIM(CHID),'_spec_',UVW_COUNTER(NM),'.csv'
...
ENDIF
ALLOCATE(FN_SMOKE3D(N_SMOKE3D*2,NMESHES)) ! also allocate unit numbers and file names for the size files
...
WRITE(LU_CORE(NM)) DT_BNDF,DT_CPU,DT_CTRL,DT_DEVC,DT_FLUSH,DT_GEOM,DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RADF,&
                   DT_RESTART,DT_SLCF,DT_SL3D,DT_SMOKE3D,DT_UVW
...
IF (WALL_INDEX(CELL_INDEX(I,J,K),-1)>0 .AND. DV%X<XC(I)) THEN
   IF (WALL(WALL_INDEX(CELL_INDEX(I,J,K),-1))%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      WALL_VALUE_LOC(1) = SELECT_WALL_VALUE(DV%QUANTITY_INDEX(1),WALL_INDEX(CELL_INDEX(I,J,K),-1),-1)
      WALL_WGT_LOC(1) =  1._EB - 2._EB*ABS(DV%X-X(I-1))*RDX(I)
   ENDIF
ENDIF
IF (WALL_INDEX(CELL_INDEX(I,J,K),+1)>0 .AND. DV%X>XC(IBAR)) THEN
   IF (WALL(WALL_INDEX(CELL_INDEX(I,J,K),+1))%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      WALL_VALUE_LOC(1) = SELECT_WALL_VALUE(DV%QUANTITY_INDEX(1),WALL_INDEX(CELL_INDEX(I,J,K),+1),+1)
...
IF (WALL_INDEX(CELL_INDEX(I,J,K),-2)>0 .AND. DV%Y<YC(J)) THEN
   IF (WALL(WALL_INDEX(CELL_INDEX(I,J,K),-2))%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      WALL_VALUE_LOC(2) = SELECT_WALL_VALUE(DV%QUANTITY_INDEX(1),WALL_INDEX(CELL_INDEX(I,J,K),-2),-2)
      WALL_WGT_LOC(2) =  1._EB - 2._EB*ABS(DV%Y-Y(J-1))*RDY(J)
   ENDIF
ENDIF
IF (WALL_INDEX(CELL_INDEX(I,J,K),+2)>0 .AND. DV%Y>YC(JBAR)) THEN
   IF (WALL(WALL_INDEX(CELL_INDEX(I,J,K),+2))%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      WALL_VALUE_LOC(2) = SELECT_WALL_VALUE(DV%QUANTITY_INDEX(1),WALL_INDEX(CELL_INDEX(I,J,K),+2),+2)
...
IF (WALL_INDEX(CELL_INDEX(I,J,K),-3)>0 .AND. DV%Z<ZC(K)) THEN
   IF (WALL(WALL_INDEX(CELL_INDEX(I,J,K),-3))%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      WALL_VALUE_LOC(3) = SELECT_WALL_VALUE(DV%QUANTITY_INDEX(1),WALL_INDEX(CELL_INDEX(I,J,K),-3),-3)
      WALL_WGT_LOC(3) =  1._EB - 2._EB*ABS(DV%Z-Z(K-1))*RDZ(K)
   ENDIF
ENDIF
IF (WALL_INDEX(CELL_INDEX(I,J,K),+3)>0 .AND. DV%Z>ZC(KBAR)) THEN
   IF (WALL(WALL_INDEX(CELL_INDEX(I,J,K),+3))%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      WALL_VALUE_LOC(3) = SELECT_WALL_VALUE(DV%QUANTITY_INDEX(1),WALL_INDEX(CELL_INDEX(I,J,K),+3),+3)
...
SELECT CASE(QUANTITY_INDEX)
   ...
   CASE(5) ! WALL TEMPERATURE
      WALL_VALUE_RES = ONE_D%TMP_F - TMPM
   CASE(6) ! U-VELOCITY
...
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,IP,JP,KP,FED_ACTIVITY,IP1,JP1,KP1,IM1,JM1,KM1,IIM1,JJM1,KKM1,NR,NS,RAM,&
           ICC,JCC,NCELL,IZ,AXIS,ICF,NFACE,JCF,OBST_INDEX
REAL(FB) :: RN
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(254)  ! ZONE PRESSURE SOLVER TYPE
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (PRES_FLAG==ULMAT_FLAG) THEN
   ...
   CASE(520)  ! SOLID CELL Q_S
      OBST_INDEX = OBST_INDEX_C(CELL_INDEX(II,JJ,KK))
      IF (OBST_INDEX>0 .AND. SOLID_HT3D) THEN
...
SELECT CASE(IND)
   ...
   CASE(450)  ! PARTICLE DRAG FORCE X
      ...
      PARTICLE_OUTPUT = RHO(BC%IIG,BC%JJG,BC%KKG) / RVC * LP%ACCEL_X
   CASE(451)  ! PARTICLE DRAG FORCE Y
      BC=>BOUNDARY_COORD(LP%BC_INDEX)
      RVC=RDX(BC%IIG)*RRN(BC%IIG)*RDY(BC%JJG)*RDZ(BC%KKG)
      PARTICLE_OUTPUT = RHO(BC%IIG,BC%JJG,BC%KKG) / RVC * LP%ACCEL_Y
   CASE(452)  ! PARTICLE DRAG FORCE Z
      BC=>BOUNDARY_COORD(LP%BC_INDEX)
      RVC=RDX(BC%IIG)*RRN(BC%IIG)*RDY(BC%JJG)*RDZ(BC%KKG)
      PARTICLE_OUTPUT = RHO(BC%IIG,BC%JJG,BC%KKG) / RVC * LP%ACCEL_Z
   CASE(453)  ! PARTICLE DRAG COEFFICIENT
      PARTICLE_OUTPUT = LP%C_DRAG
   CASE(454)  ! PARTICLE BULK DENSITY
      BC=>BOUNDARY_COORD(LP%BC_INDEX)
      RVC=RDX(BC%IIG)*RRN(BC%IIG)*RDY(BC%JJG)*RDZ(BC%KKG)
      PARTICLE_OUTPUT = LP%MASS * LP%PWT * RVC
   CASE(455)  ! PARTICLE DISK AREA
