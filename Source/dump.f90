...
USE EVAC, ONLY : N_FIRE_MESHES_IN_FED
INTEGER, INTENT(IN) :: NM
      ...
      IF (SL%SLICETYPE=='STRUCTURED') THEN
         ...
         ELSE
            IF (EVACUATION_MC_MODE) THEN
               WRITE(M%STRING(M%N_STRINGS),'(A,I6,A)') 'SLCF',NM+N_FIRE_MESHES_IN_FED,TRIM(SLICELABEL)
            ELSE
...
PARTICLE_IF: IF ( (PARTICLE_FILE .AND. .NOT.EVACUATION_ONLY(NM)) .OR. (EVACUATION_SKIP(NM)) ) THEN
   ...
   ELSE APPEND_PARTICLE_FILE
      ...
      EVAC_ONLY: IF (EVACUATION_ONLY(NM)) THEN
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A,I6,F10.4)') 'EVA5',NM+N_FIRE_MESHES_IN_FED,EVACUATION_Z_OFFSET(NM)
         M%N_STRINGS = M%N_STRINGS + 1
...
USE EVAC, ONLY: N_DOORS, N_EXITS, N_ENTRYS, N_SSTANDS, EVAC_DOORS, EVAC_EXITS, EVAC_ENTRYS, EVAC_SSTANDS, &
     ...
     EVACUATION_TYPE, EVAC_HOLE_TYPE, EVAC_EVACS, EVAC_HOLES, N_FIRE_MESHES_IN_FED
...
INTEGER :: II, JJ
INTEGER (4) :: ntmp1, ntmp2, ntmp3, ntmp4, ntmp5, ntmp6
...
EVACUATION_SMV_APPEND: IF (EVACUATION_MC_MODE .AND. .NOT.EVACUATION_DRILL) THEN
   ...
   EVAC_SMV_DO: DO
      ...
      IF (TRIM(SMV_LINE)=='NMESHES') THEN
         ...
         WRITE(LU_SMV,FMT='(I3)') N_FIRE_MESHES_IN_FED+NMESHES
         READ (LU_EVACXYZ,FMT='(A)',END=30) SMV_LINE ! number of meshes is replaced above
      ELSE
         WRITE(LU_SMV,FMT='(A)') TRIM(SMV_LINE)
      ENDIF
...
EVAC_ONLY5: IF (ANY(EVACUATION_ONLY) .AND. MY_RANK==MAX(0,EVAC_PROCESS)) THEN
   DO N=1,N_DOORS
...
WRITE(LU_CORE(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),SMOKE3D_CLOCK(NM),RADF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDF_CLOCK(NM),BNDC_CLOCK,GEOC_CLOCK,GEOM_CLOCK,DEVC_CLOCK,HRR_CLOCK,MASS_CLOCK, &
...
DO N=0,N_SURF
   ...
   ENDIF
   WRITE(LU_CORE(NM)) CFACE_STORAGE(N)%N_STORAGE_SLOTS
   IF (CFACE_STORAGE(N)%N_STORAGE_SLOTS>0) THEN
      WRITE(LU_CORE(NM)) CFACE_STORAGE(N)%REALS
      WRITE(LU_CORE(NM)) CFACE_STORAGE(N)%INTEGERS
      WRITE(LU_CORE(NM)) CFACE_STORAGE(N)%LOGICALS
   ENDIF
ENDDO
...
DO N=0,N_SURF
   ...
   ENDIF
   READ(LU_RESTART(NM)) CFACE_STORAGE(N)%N_STORAGE_SLOTS
   IF (CFACE_STORAGE(N)%N_STORAGE_SLOTS>0) THEN
      READ(LU_RESTART(NM)) CFACE_STORAGE(N)%REALS
      READ(LU_RESTART(NM)) CFACE_STORAGE(N)%INTEGERS
      READ(LU_RESTART(NM)) CFACE_STORAGE(N)%LOGICALS
   ENDIF
ENDDO
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   LOAD_LOOP: DO IP=1,NLP
      ...
      TA(NPP) = LP%TAG
      XP(NPP) = LP%BOUNDARY_COORD%X
      YP(NPP) = LP%BOUNDARY_COORD%Y
      ZP(NPP) = LP%BOUNDARY_COORD%Z
      DO NN=1,LPC%N_QUANTITIES
      ...
      IF (DV%LP_TAG>0) THEN
         ...
         ELSE
            ...
            IF (LAGRANGIAN_PARTICLE(LP_INDEX)%PATH_PARTICLE) THEN
               ...
               CALL GET_IJK(LP%BOUNDARY_COORD%X,LP%BOUNDARY_COORD%Y,LP%BOUNDARY_COORD%Z,NM,XI,YJ,ZK,DV%I(1),DV%J(1),DV%K(1))
            ENDIF
         ENDIF
      ELSE
         CYCLE DEVICE_LOOP
         ...
         SOLID_STATS_SELECT: SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE DEFAULT SOLID_STATS_SELECT
               ...
               WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
                  ...
                  ONE_D => WC%ONE_D
                  IF (DV%IOR/=0 .AND. DV%IOR/=WC%BOUNDARY_COORD%IOR) CYCLE WALL_CELL_LOOP
                  IF (DV%SURF_ID/='null' .AND. SURFACE(SURF_INDEX)%ID/=DV%SURF_ID) CYCLE WALL_CELL_LOOP
                  X_CENTER = WC%BOUNDARY_COORD%X ; Y_CENTER = WC%BOUNDARY_COORD%Y ; Z_CENTER = WC%BOUNDARY_COORD%Z
               ...
               CFACE_LOOP : DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
                  ...
                  ONE_D => CFA%ONE_D
                  X_CENTER = CFA%BOUNDARY_COORD%X ; Y_CENTER = CFA%BOUNDARY_COORD%Y ; Z_CENTER = CFA%BOUNDARY_COORD%Z
               ...
               PARTICLE_LOOP: DO IP=1,NLP
                  ...
                  ONE_D => LP%ONE_D
                  X_CENTER = LP%BOUNDARY_COORD%X ; Y_CENTER = LP%BOUNDARY_COORD%Y ; Z_CENTER = LP%BOUNDARY_COORD%Z
                  IF (X_CENTER<SDV%X1-MICRON .OR. X_CENTER>SDV%X2+MICRON .OR. &
                      Y_CENTER<SDV%Y1-MICRON .OR. Y_CENTER>SDV%Y2+MICRON .OR. &
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,MU_G,DROPLET_COOLING,&
            ...
            RHO_GET(N_MATL),VSRVC,GAS_PHASE_OUTPUT_CFA,CFACE_AREA,VELOCITY_COMPONENT(1:3)
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(231) ! PDPA
      ...
      IF ( ((PY%PDPA_START<=T) .AND. (PY%PDPA_END>=T)) .OR. .NOT.PY%PDPA_INTEGRATE ) THEN
         ...
         DLOOP: DO I=1,NLP
            ...
            IF ((LP%BOUNDARY_COORD%X-DV%X)**2+(LP%BOUNDARY_COORD%Y-DV%Y)**2+(LP%BOUNDARY_COORD%Z-DV%Z)**2 > PY%PDPA_RADIUS**2) &
               CYCLE DLOOP
   ...
   CASE(538)  ! PYRO3D IOR
      ...
      IF (WALL_INDEX_HT3D(CELL_INDEX(II,JJ,KK),OB%PYRO3D_IOR)>0) THEN
         ...
         GAS_PHASE_OUTPUT_RES = REAL(WC%BOUNDARY_COORD%IOR,EB)
      ENDIF
...
INTEGER :: II1,II2,IIG,JJG,KKG,NN,IWX,SURF_INDEX,I,J,K,IW,II,JJ,KK,NWP,IOR,M_INDEX,ICC,IND1,IND2,IC2,LPX,ITMP,ICF,JCF,NFACE
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(13) ! GAUGE HEAT FLUX
      IF (PY%HEAT_TRANSFER_COEFFICIENT>0._EB) THEN
         Q_CON = PY%HEAT_TRANSFER_COEFFICIENT*(ONE_D%TMP_G-PY%GAUGE_TEMPERATURE)
      ELSE
   ...
   CASE(17) ! RADIANCE
      IF (PRESENT(OPT_WALL_INDEX) .OR. PRESENT(OPT_CFACE_INDEX)) THEN
         SOLID_PHASE_OUTPUT = SUM(BP%IL(1:NUMBER_SPECTRAL_BANDS))*0.001_EB
      ELSEIF (PRESENT(OPT_LP_INDEX)) THEN
         SOLID_PHASE_OUTPUT = SUM(LP%IL(1:NUMBER_SPECTRAL_BANDS))*0.001_EB
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
...
SELECT CASE(IND)
   ...
   CASE(436)  ! PARTICLE PHASE
      PARTICLE_OUTPUT = LP%BOUNDARY_COORD%IOR
   ...
   CASE(441)  ! PARTICLE X
      PARTICLE_OUTPUT = LP%BOUNDARY_COORD%X
   CASE(442)  ! PARTICLE Y
      PARTICLE_OUTPUT = LP%BOUNDARY_COORD%Y
   CASE(443)  ! PARTICLE Z
      PARTICLE_OUTPUT = LP%BOUNDARY_COORD%Z
   CASE(444)  ! PARTICLE U
...
WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   DO N=1,N_TRACKED_SPECIES
      M_DOT(N,NM) = M_DOT(N,NM) + WC%ONE_D%M_DOT_G_PP_ACTUAL(N)*WC%ONE_D%AREA*WC%ONE_D%AREA_ADJUST
   ENDDO
ENDDO WALL_LOOP2
...
CFACE_LOOP_2 : DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   DO N=1,N_TRACKED_SPECIES
      M_DOT(N,NM) = M_DOT(N,NM) + CFA%ONE_D%M_DOT_G_PP_ACTUAL(N)*CFA%ONE_D%AREA*CFA%ONE_D%AREA_ADJUST
   ENDDO
ENDDO CFACE_LOOP_2
...
DO IP=1,NLP
   LP=>LAGRANGIAN_PARTICLE(IP)
   LPC=>LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
   IF (LP%BOUNDARY_COORD%X<=XS) CYCLE
   IF (LP%BOUNDARY_COORD%X>=XF) CYCLE
   IF (LP%BOUNDARY_COORD%Y<=YS) CYCLE
   IF (LP%BOUNDARY_COORD%Y>=YF) CYCLE
   IF (LP%BOUNDARY_COORD%Z<=ZS) CYCLE
   IF (LP%BOUNDARY_COORD%Z>=ZF) CYCLE
...
DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IIG = WALL(IW)%BOUNDARY_COORD%IIG
   JJG = WALL(IW)%BOUNDARY_COORD%JJG
   KKG = WALL(IW)%BOUNDARY_COORD%KKG
