...
SUBROUTINE SELECT_SPATIAL_STATISTIC(OPT_LP_INDEX)

INTEGER, OPTIONAL :: OPT_LP_INDEX
REAL(EB) :: PWT
...
SELECT CASE(DV%SPATIAL_STATISTIC)
   ...
   CASE('SURFACE INTEGRAL')
      IF (VALUE <= DV%QUANTITY_RANGE(2) .AND. VALUE >=DV%QUANTITY_RANGE(1)) &
         SDV%VALUE_1 = SDV%VALUE_1 + VALUE*ONE_D%AREA*PWT
   CASE('SURFACE AREA')
      IF (VALUE <= DV%QUANTITY_RANGE(2) .AND. VALUE >=DV%QUANTITY_RANGE(1)) &
         SDV%VALUE_1 = SDV%VALUE_1 + ONE_D%AREA*PWT
   CASE('SUM')
      IF (VALUE <= DV%QUANTITY_RANGE(2) .AND. VALUE >=DV%QUANTITY_RANGE(1)) &
         SDV%VALUE_1 = SDV%VALUE_1 + VALUE
END SELECT
...
REAL(EB) :: Q_CON,VOLSUM,MFT,ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,DEPTH,UN,H_S,RHO_D_DYDN,U_CELL,V_CELL,W_CELL,&
            ...
            P1,P2,Z1,Z2,RADIUS
INTEGER :: II1,II2,IIG,JJG,KKG,NN,IWX,SURF_INDEX,I,J,K,IW,II,JJ,KK,NWP,IOR,M_INDEX,ICC,IND1,IND2,IC2,LPX,ITMP
