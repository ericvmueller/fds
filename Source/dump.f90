...
IF (T>=ISOF_CLOCK(NM)) THEN
   ...
   ENDDO
ENDIF
IF (T>=SMOKE3D_CLOCK(NM) .AND. SMOKE3D) THEN
   CALL DUMP_SMOKE3D(T,DT,NM)
   DO NN=1,CEILING(DT/DT_SMOKE3D)
      SMOKE3D_CLOCK(NM) = SMOKE3D_CLOCK(NM) + DT_SMOKE3D
      IF (SMOKE3D_CLOCK(NM)>=T) THEN
         SMOKE3D_CLOCK(NM) = MIN(SMOKE3D_CLOCK(NM),T_END)
         EXIT
      ENDIF
   ENDDO
ENDIF
IF (T>=SLCF_CLOCK(NM)) THEN
   ...
   ENDDO
ENDIF
IF (T>=SLCF_TIMER(MESHES(NM)%ISLCF_TIMER)) THEN
   CALL DUMP_SLCF(T,DT,NM,0)
   MESHES(NM)%ISLCF_TIMER = MESHES(NM)%ISLCF_TIMER + 1
ENDIF
IF (T>=SL3D_CLOCK(NM) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
   ...
   ENDDO
ENDIF
IF (T>=SL3D_TIMER(MESHES(NM)%ISL3D_TIMER)) THEN
   CALL DUMP_SLCF(T,DT,NM,2)
   MESHES(NM)%ISL3D_TIMER = MESHES(NM)%ISL3D_TIMER + 1
ENDIF
IF (T>=BNDF_CLOCK(NM)) THEN
   ...
   ENDDO
ENDIF
IF (T>=BNDF_TIMER(MESHES(NM)%IBNDF_TIMER)) THEN
   CALL DUMP_BNDF(T,DT,NM)
   MESHES(NM)%IBNDF_TIMER = MESHES(NM)%IBNDF_TIMER + 1
ENDIF
...
IF (T>=PROF_CLOCK(NM)) THEN
   ...
   ENDDO
ENDIF
IF (T>=UVW_TIMER(MESHES(NM)%IUVW_TIMER)) THEN
   WRITE(FN_UVW,'(A,A,I0,A,I0,A)') TRIM(CHID),'_uvw_t',MESHES(NM)%IUVW_TIMER,'_m',NM,'.csv'
   CALL DUMP_UVW(NM,FN_UVW)
   MESHES(NM)%IUVW_TIMER = MESHES(NM)%IUVW_TIMER + 1
ENDIF
PERIODIC_TEST_SELECT: SELECT CASE(PERIODIC_TEST)
   CASE(2)
      IF (T>=UVW_CLOCK_CBC(MESHES(NM)%IUVW_TIMER)) THEN
         WRITE(FN_UVW,'(A,A,I0,A)') TRIM(CHID),'_uvw_',MESHES(NM)%IUVW_TIMER,'.csv'
         CALL DUMP_UVW(NM,FN_UVW)
         MESHES(NM)%IUVW_TIMER = MESHES(NM)%IUVW_TIMER + 1
      ENDIF
   CASE(7,11)
   ...
   CASE(9)
      IF (T>=UVW_CLOCK_CBC(MESHES(NM)%IUVW_TIMER)) THEN
         WRITE(FN_SPEC,'(A,A,I0,A)') TRIM(CHID),'_spec_',MESHES(NM)%IUVW_TIMER,'.csv'
...
IF (REMOVE_FIREBRANDS) THEN
   ALLOCATE(FN_FBRAND(NMESHES))
   ALLOCATE(LU_FBRAND(NMESHES))
ENDIF
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   ! Firebrand output files
   IF (REMOVE_FIREBRANDS) THEN
      LU_FBRAND(NM) = GET_FILE_NUMBER()
      WRITE(FN_FBRAND(NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_FBRAND.csv'
   ENDIF
...
! Initialize FBRAND data files 
FBRAND_IF: IF (REMOVE_FIREBRANDS) THEN
   IF (APPEND) THEN
   ...
   ELSE
      ...
      WRITE(LU_FBRAND(NM),'(A)') "Deposition time [s], Flight time[s], x [m], y [m], z [m], z_i [m], thickness [um], mass [ug], A [m^2], PWT"
   ENDIF
ENDIF FBRAND_IF
...
#ifndef NO_SCARC
USE SCRC, ONLY: SCARC_METHOD, SCARC_MESH, SCARC_PRECON, SCARC_COARSE, SCARC_PRECISION, &
                SCARC_KRYLOV_ITERATIONS, SCARC_KRYLOV_TOLERANCE, SCARC_MGM_BOUNDARY, SCARC_MGM_LAPLACE_SOLVER
#endif
...
USE MEMORY_FUNCTIONS, ONLY: PACK_PARTICLE,PACK_WALL,PACK_CFACE
REAL(EB), INTENT(IN) :: T,DT
INTEGER :: NOM,N,IP,IW,ICF
INTEGER, INTENT(IN) :: NM
...
IF (SOLID_HT3D) WRITE(LU_CORE(NM)) Q_DOT_PPP_S
...
DO N=1,N_OBST
   ...
   IF (OB%HT3D_RESTART) WRITE(LU_CORE(NM)) OB%RHO
ENDDO

WRITE(LU_CORE(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),SMOKE3D_CLOCK(NM),RADF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDF_CLOCK(NM),BNDC_CLOCK,GEOC_CLOCK,GEOM_CLOCK,CTRL_CLOCK,DEVC_CLOCK,HRR_CLOCK,MASS_CLOCK, &
...
WRITE(LU_CORE(NM)) N_BOUNDARY_COORD_DIM,N_BOUNDARY_ONE_D_DIM,N_BOUNDARY_PROPS_DIM,N_BOUNDARY_RADIA_DIM
WRITE(LU_CORE(NM)) NEXT_AVAILABLE_BOUNDARY_COORD_SLOT,NEXT_AVAILABLE_BOUNDARY_ONE_D_SLOT,&
                   NEXT_AVAILABLE_BOUNDARY_PROPS_SLOT,NEXT_AVAILABLE_BOUNDARY_RADIA_SLOT

WRITE(LU_CORE(NM)) N_WALL_CELLS,N_WALL_CELLS_DIM
DO IW=0,N_WALL_CELLS
   WC => WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)
   CALL PACK_WALL(NM,IW,WC%SURF_INDEX,UNPACK_IT=.FALSE.)
   WRITE(LU_CORE(NM)) WC%SURF_INDEX
   WRITE(LU_CORE(NM)) SF%WALL_STORAGE%REALS,SF%WALL_STORAGE%INTEGERS,SF%WALL_STORAGE%LOGICALS
ENDDO
...
DO ICF=1,N_CFACE_CELLS_DIM
   ...
   CALL PACK_CFACE(NM,ICF,CFA%SURF_INDEX,UNPACK_IT=.FALSE.)
   WRITE(LU_CORE(NM)) CFA%SURF_INDEX
   WRITE(LU_CORE(NM)) SF%CFACE_STORAGE%REALS,SF%CFACE_STORAGE%INTEGERS,SF%CFACE_STORAGE%LOGICALS
ENDDO
...
DO IP=1,NLP
   ...
   CALL PACK_PARTICLE(NM,IP,LP%CLASS_INDEX,1,UNPACK_IT=.FALSE.)
   WRITE(LU_CORE(NM)) LP%CLASS_INDEX
   WRITE(LU_CORE(NM)) LPC%PARTICLE_STORAGE%REALS,LPC%PARTICLE_STORAGE%INTEGERS,LPC%PARTICLE_STORAGE%LOGICALS
ENDDO
...
USE MEMORY_FUNCTIONS, ONLY: REALLOCATE,PACK_PARTICLE,PACK_WALL,PACK_CFACE
REAL(EB), INTENT(OUT) :: T,DT
INTEGER :: NOM,N,N_T_E_MAX,IP,CLASS_INDEX,IW,SURF_INDEX,ICF
INTEGER, INTENT(IN) :: NM
LOGICAL :: EX
CHARACTER(MESSAGE_LENGTH) :: MESSAGE
TYPE(OMESH_TYPE), POINTER :: M2=>NULL()
...
IF (SOLID_HT3D) READ(LU_RESTART(NM)) Q_DOT_PPP_S
...
DO N=1,N_OBST
   ...
   IF (OB%HT3D_RESTART) READ(LU_RESTART(NM)) OB%RHO
ENDDO

READ(LU_RESTART(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),SMOKE3D_CLOCK(NM),RADF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDF_CLOCK(NM),BNDC_CLOCK,GEOC_CLOCK,GEOM_CLOCK,CTRL_CLOCK,DEVC_CLOCK,HRR_CLOCK,MASS_CLOCK, &
             ISOF_CLOCK(NM),BC_CLOCK,WALL_COUNTER,DT, &
             PBAR,D_PBAR_DT,N_EDGES,RAD_CALL_COUNTER,ANGLE_INC_COUNTER,T_LAST_DUMP_HRR,T_LAST_DUMP_MASS, &
             RTE_SOURCE_CORRECTION_FACTOR,RAD_Q_SUM,KFST4_SUM,ENTHALPY_SUM(NM),&
...
READ(LU_RESTART(NM)) N_BOUNDARY_COORD_DIM,N_BOUNDARY_ONE_D_DIM,N_BOUNDARY_PROPS_DIM,N_BOUNDARY_RADIA_DIM
...
IF (ALLOCATED(MESHES(NM)%BOUNDARY_PROPS_OCCUPANCY)) DEALLOCATE(MESHES(NM)%BOUNDARY_PROPS_OCCUPANCY)
...
ALLOCATE(MESHES(NM)%BOUNDARY_PROPS_OCCUPANCY(N_BOUNDARY_PROPS_DIM)) ; MESHES(NM)%BOUNDARY_PROPS_OCCUPANCY = 0
...
NEXT_AVAILABLE_BOUNDARY_PROPS_SLOT = 1
...
IF (ALLOCATED(MESHES(NM)%BOUNDARY_PROPS)) DEALLOCATE(MESHES(NM)%BOUNDARY_PROPS)
...
ALLOCATE(MESHES(NM)%BOUNDARY_PROPS(N_BOUNDARY_PROPS_DIM))
...
DO IW=0,N_WALL_CELLS
   ...
   READ(LU_RESTART(NM)) SF%WALL_STORAGE%REALS,SF%WALL_STORAGE%INTEGERS,SF%WALL_STORAGE%LOGICALS
   CALL PACK_WALL(NM,IW,SURF_INDEX,UNPACK_IT=.TRUE.)
ENDDO

READ(LU_RESTART(NM)) N_CFACE_CELLS_DIM
IF (ALLOCATED(MESHES(NM)%CFACE)) DEALLOCATE(MESHES(NM)%CFACE)
ALLOCATE(MESHES(NM)%CFACE(N_CFACE_CELLS_DIM))
DO ICF=1,N_CFACE_CELLS_DIM
   READ(LU_RESTART(NM)) SURF_INDEX
   SF => SURFACE(SURF_INDEX)
   READ(LU_RESTART(NM)) SF%CFACE_STORAGE%REALS,SF%CFACE_STORAGE%INTEGERS,SF%CFACE_STORAGE%LOGICALS
   CALL PACK_CFACE(NM,ICF,SURF_INDEX,UNPACK_IT=.TRUE.)
ENDDO
...
IF (NLPDIM>0) THEN
   ...
   DO IP=1,NLP
      ...
      READ(LU_RESTART(NM)) LPC%PARTICLE_STORAGE%REALS,LPC%PARTICLE_STORAGE%INTEGERS,LPC%PARTICLE_STORAGE%LOGICALS
      CALL PACK_PARTICLE(NM,IP,CLASS_INDEX,1,UNPACK_IT=.TRUE.)
   ENDDO
ENDIF
...
#ifndef NO_SCARC
USE SCRC, ONLY: SCARC_METHOD, SCARC_CAPPA, SCARC_ITERATIONS, SCARC_RESIDUAL, SCARC_MGM_TOLERANCE
#endif
...
IF (N_INTERNAL_CFACE_CELLS>0) THEN
   ...
   DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
     ...
     VERTS(6*(ICF-N_EXTERNAL_CFACE_CELLS-1)+1:6*(ICF-N_EXTERNAL_CFACE_CELLS)) = REAL(XLOC(1:6),FB)
   ENDDO
...
!      write(LU_ERR,*)"option 0 IND=",IND," IND2=",IND2, "Y_INDEX=",Y_INDEX,"Z_INDEX=",Z_INDEX
      CALL GET_GEOMVALS(CC_FACE_CENTERED,CC_CELL_CENTERED,SLICETYPE,&
                       I1,I2,J1,J2,K1,K2,NFACES,NFACES_CUTCELLS,VALS,&
                       IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,T,DT,NM)
!       WRITE(LU_ERR, *)"option 0",(VALS(I),I=1,NVALS)
   ELSE
      NVALS = NVERTS
...
! get values at geometry nodes
      ...
      CALL GET_GEOMVALS(CC_FACE_CENTERED,CC_CELL_CENTERED,SLICETYPE,&
                        I1,I2,J1,J2,K1,K2,NFACES,NFACES_CUTCELLS,VALS,&
                        IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,T,DT,NM)
!         write(LU_ERR,*)"option 2 IND=",IND," IND2=",IND2, "Y_INDEX=",Y_INDEX,"Z_INDEX=",Z_INDEX
!         WRITE(LU_ERR, *)"option 2",(VALS(I),I=1,NVALS)
!         write(LU_ERR,*)""
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   ELSE
      ...
      IF(.NOT.CC_CELL_CENTERED .AND. TRIM(SL%SLICETYPE)/='STRUCTURED') CC_FACE_CENTERED = .TRUE.
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   OUTPUT_INDEX_SELECT: SELECT CASE(DV%QUANTITY_INDEX(1))
         ...
         SOLID_STATS_SELECT: SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE DEFAULT SOLID_STATS_SELECT
               ...
               WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
                  ...
                  IF (DV%IOR/=0) THEN
                     SELECT CASE(DV%IOR)
                        CASE( 1); EPS_X1 = 1._EB/ONE_D%RDN
                        CASE(-1); EPS_X2 = 1._EB/ONE_D%RDN
                        CASE( 2); EPS_Y1 = 1._EB/ONE_D%RDN
                        CASE(-2); EPS_Y2 = 1._EB/ONE_D%RDN
                        CASE( 3); EPS_Z1 = 1._EB/ONE_D%RDN
                        CASE(-3); EPS_Z2 = 1._EB/ONE_D%RDN
                     END SELECT
                  ENDIF
      ...
      CASE(400:452) OUTPUT_INDEX_SELECT  ! Particle-specific output
...
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,IP,JP,KP,FED_ACTIVITY,IP1,JP1,KP1,IM1,JM1,KM1,IIM1,JJM1,KKM1,NR,NS,RAM,&
           ICC,JCC,NCELL,IZ,AXIS,ICF,NFACE,JCF
REAL(FB) :: RN
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(99)  ! ZONE PRESSURE SOLVER TYPE
   ...
   CASE(162) ! ASSUMED GAS TEMPERATURE
      GAS_PHASE_OUTPUT_RES = TMPA + EVALUATE_RAMP(T-T_BEGIN,I_RAMP_AGT)*(ASSUMED_GAS_TEMPERATURE-TMPA) - TMPM
   ...
   CASE(252)  ! AUTO IGNITION TEMPERATURE
      GAS_PHASE_OUTPUT_RES = AUTO_IGNITION_TEMPERATURE - TMPM
      DO IZ=1,MAX_AIT_EXCLUSION_ZONES
         IF (ANY(AIT_EXCLUSION_ZONE(:,IZ)<-1.E5_EB)) EXIT
         IF (XC(II)>=AIT_EXCLUSION_ZONE(1,IZ) .AND. XC(II)<=AIT_EXCLUSION_ZONE(2,IZ) .AND.  &
             YC(JJ)>=AIT_EXCLUSION_ZONE(3,IZ) .AND. YC(JJ)<=AIT_EXCLUSION_ZONE(4,IZ) .AND.  &
             ZC(KK)>=AIT_EXCLUSION_ZONE(5,IZ) .AND. ZC(KK)<=AIT_EXCLUSION_ZONE(6,IZ)) GAS_PHASE_OUTPUT_RES=-TMPM
      ENDDO
   ...
   CASE(520)  ! SOLID CELL Q_S
      ...
      TIME_RAMP_FACTOR = EVALUATE_RAMP(T,OB%RAMP_Q_INDEX)
   ...
   CASE(538)  ! PYRO3D IOR
      ...
      IF (WALL_INDEX_HT3D(CELL_INDEX(II,JJ,KK),OB%PYRO3D_IOR)>0) THEN
         ...
         BC => BOUNDARY_COORD(WC%BC_INDEX)
         GAS_PHASE_OUTPUT_RES = REAL(BC%IOR,EB)
      ENDIF

   CASE(540)  ! SOLID CELL CONDUCTIVITY
      GAS_PHASE_OUTPUT_RES = 0._EB
      OB => OBSTRUCTION(OBST_INDEX_C(CELL_INDEX(II,JJ,KK)))
      IF (OB%MATL_INDEX>0) THEN
         CALL  GET_SOLID_CONDUCTIVITY(GAS_PHASE_OUTPUT_RES,TMP(II,JJ,KK),OPT_MATL_INDEX=OB%MATL_INDEX)
      ELSEIF (OB%MATL_SURF_INDEX>0) THEN
         SF => SURFACE(OB%MATL_SURF_INDEX)
         VSRVC = 0._EB
         DO NN=1,SF%N_MATL
            ML => MATERIAL(SF%MATL_INDEX(NN))
            VSRVC = VSRVC + OB%RHO(II,JJ,KK,NN)/ML%RHO_S
         ENDDO
         IF (VSRVC>TWO_EPSILON_EB) THEN
            RHO_GET(1:SF%N_MATL) = OB%RHO(II,JJ,KK,1:SF%N_MATL) / VSRVC
            CALL GET_SOLID_CONDUCTIVITY(GAS_PHASE_OUTPUT_RES,TMP(II,JJ,KK),OPT_SURF_INDEX=OB%MATL_SURF_INDEX,OPT_RHO_IN=RHO_GET)
         ENDIF
      ENDIF

   CASE(541)  ! SOLID CELL SPECIFIC HEAT
      GAS_PHASE_OUTPUT_RES = 0._EB
      OB => OBSTRUCTION(OBST_INDEX_C(CELL_INDEX(II,JJ,KK)))
      IF (OB%MATL_INDEX>0) THEN
         CALL GET_SOLID_RHOCBAR(GAS_PHASE_OUTPUT_RES,TMP(II,JJ,KK),OPT_MATL_INDEX=OB%MATL_INDEX)
         GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES / MATERIAL(OB%MATL_INDEX)%RHO_S
      ELSEIF (OB%MATL_SURF_INDEX>0) THEN
         SF => SURFACE(OB%MATL_SURF_INDEX)
         VSRVC = 0._EB
         DO NN=1,SF%N_MATL
         ...
         IF (VSRVC>TWO_EPSILON_EB) THEN
            RHO_GET(1:SF%N_MATL) = OB%RHO(II,JJ,KK,1:SF%N_MATL) / VSRVC
            CALL GET_SOLID_RHOCBAR(GAS_PHASE_OUTPUT_RES,TMP(II,JJ,KK),OPT_SURF_INDEX=OB%MATL_SURF_INDEX,OPT_RHO_IN=RHO_GET)
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES / SUM(RHO_GET(1:SF%N_MATL))
         ENDIF
      ENDIF
...
IF (PRESENT(OPT_WALL_INDEX)) THEN
   ...
   IF (WC%BP_INDEX>0) BP    => BOUNDARY_PROPS(WC%BP_INDEX)
...
ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
   ...
   IF (CFA%BP_INDEX>0) BP    => BOUNDARY_PROPS(CFA%BP_INDEX)
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(13) ! GAUGE HEAT FLUX
      ...
      ELSE
         Q_CON = ONE_D%Q_CON_F + ONE_D%HEAT_TRANS_COEF*(ONE_D%TMP_F-PY%GAUGE_TEMPERATURE)
      ENDIF
   ...
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE (Ferrari's Method for Solving the Quartic)
      H_W_EFF = 0._EB
      LTMP = 0._EB
      IF (PRESENT(OPT_WALL_INDEX) .OR. PRESENT(OPT_CFACE_INDEX)) THEN  ! Look for evaporating liquid
         IF (ANY(ABS(BP%LP_CPUA)>TWO_EPSILON_EB)) THEN
            ATMP = 0._EB
            CTMP = 0._EB
            DO NN = 1,N_LAGRANGIAN_CLASSES
               LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
               IF (LPC%LIQUID_DROPLET) THEN
                  CTMP = CTMP + BP%LP_CPUA(LPC%ARRAY_INDEX)
                  ATMP = ATMP + ABS(BP%LP_CPUA(LPC%ARRAY_INDEX))
                  LTMP = LTMP + ABS(BP%LP_CPUA(LPC%ARRAY_INDEX))*BP%LP_TEMP(LPC%ARRAY_INDEX)
               ENDIF
            ENDDO
      ...
      IF (ONE_D%HEAT_TRANS_COEF+H_W_EFF>1.E-5_EB .AND. ABS(ONE_D%Q_RAD_IN-AAA*TMP(BC%IIG,BC%JJG,BC%KKG)**4)>5.E-3_EB) THEN
         AAA = ONE_D%EMISSIVITY*SIGMA
         BBB = ONE_D%HEAT_TRANS_COEF + H_W_EFF
         CCC = -ONE_D%Q_RAD_IN - ONE_D%HEAT_TRANS_COEF*TMP(BC%IIG,BC%JJG,BC%KKG) - H_W_EFF*LTMP
      ...
      ELSE
         SOLID_PHASE_OUTPUT = (ONE_D%Q_RAD_IN/(ONE_D%EMISSIVITY*SIGMA+TWO_EPSILON_EB))**0.25 - TMPM
      ENDIF
   ...
   CASE(30) ! MPUA
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      SOLID_PHASE_OUTPUT = BP%LP_MPUA(LPC%ARRAY_INDEX)

   CASE(31) ! CPUA
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      SOLID_PHASE_OUTPUT = BP%LP_CPUA(LPC%ARRAY_INDEX)*0.001_EB

   CASE(32) ! AMPUA
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      SOLID_PHASE_OUTPUT = BP%A_LP_MPUA(LPC%ARRAY_INDEX)
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         IF (LAGRANGIAN_PARTICLE_CLASS(NN)%LIQUID_DROPLET) THEN
            IF (LAGRANGIAN_PARTICLE_CLASS(NN)%Y_INDEX==Y_INDEX) &
               SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + BP%LP_MPUA(LAGRANGIAN_PARTICLE_CLASS(NN)%ARRAY_INDEX)
         ENDIF
      ENDDO
   ...
   CASE(43) ! CPUA_Z
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
         IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) &
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + BP%LP_CPUA(LPC%ARRAY_INDEX)*0.001_EB
      ENDDO
   ...
   CASE(44) ! AMPUA_Z
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
         IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) &
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + BP%A_LP_MPUA(LPC%ARRAY_INDEX)
      ENDDO
   ...
   CASE(51)  ! ENTHALPY FLUX WALL
      ...
      SOLID_PHASE_OUTPUT = (-ONE_D%RHO_F*H_S*ONE_D%U_NORMAL &
                            -2._EB*ONE_D%K_G*(TMP(BC%IIG,BC%JJG,BC%KKG)-ONE_D%TMP_F)*ONE_D%RDN)*0.001_EB
   ...
   CASE(65) ! WALL PRESSURE (takes optional FORCE_DIRECTION vector)
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         ...
         P1 = RHO(IIG,JJG,KKG)*(H(IIG,JJG,KKG)-KRES(IIG,JJG,KKG))
         P2 = P1
         SELECT CASE(BC%IOR)
            CASE( 1)
               ...
               IF (.NOT.SOLID(IC2)) P2 = RHO(IIG,JJG,KKG)*(H(IIG+1,JJG,KKG)-KRES(IIG+1,JJG,KKG))
            CASE(-1)
               NVEC=(/-1._EB,0._EB,0._EB/)
               Z1 = 0.5_EB*DX(IIG)
               Z2 = DX(IIG)+0.5_EB*DX(IIG-1)
            ...
            CASE( 2)
               ...
               IF (.NOT.SOLID(IC2)) P2 = RHO(IIG,JJG,KKG)*(H(IIG,JJG+1,KKG)-KRES(IIG,JJG+1,KKG))
            CASE(-2)
               NVEC=(/0._EB,-1._EB,0._EB/)
               Z1 = 0.5_EB*DY(JJG)
               Z2 = DY(JJG)+0.5_EB*DY(JJG-1)
            ...
            CASE( 3)
               ...
               IF (.NOT.SOLID(IC2)) P2 = RHO(IIG,JJG,KKG)*(H(IIG,JJG,KKG+1)-KRES(IIG,JJG,KKG+1))
            CASE(-3)
               NVEC=(/0._EB,0._EB,-1._EB/)
               Z1 = 0.5_EB*DZ(KKG)
               Z2 = DZ(KKG)+0.5_EB*DZ(KKG-1)
   ...
   CASE(71) ! OBUKHOV LENGTH
      SOLID_PHASE_OUTPUT = BP%L_OBUKHOV
...
SELECT CASE(IND)
   ...
   CASE(341) ! Node Sensible Enhtalpy
      ZZ_GET(1:N_TRACKED_SPECIES) = DUCTNODE(NODE_INDEX(1))%ZZ(1:N_TRACKED_SPECIES)
...
SELECT CASE(IND)
   CASE(434)  ! PARTICLE DIAMETER
      PARTICLE_OUTPUT = 2.E6*SUM(BOUNDARY_ONE_D(LP%OD_INDEX)%LAYER_THICKNESS)
   ...
   CASE(437)  ! PARTICLE TEMPERATURE
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%TMP_F - TMPM
   ...
   CASE(450)  ! PARTICLE DRAG FORCE X
      PARTICLE_OUTPUT = LP%MASS * LP%PWT * LP%ACCEL_X
   CASE(451)  ! PARTICLE DRAG FORCE Y
      PARTICLE_OUTPUT = LP%MASS * LP%PWT * LP%ACCEL_Y
   CASE(452)  ! PARTICLE DRAG FORCE Z
      PARTICLE_OUTPUT = LP%MASS * LP%PWT * LP%ACCEL_Z
   CASE(453)  ! PARTICLE DRAG COEFFICIENT
      PARTICLE_OUTPUT = LP%C_DRAG
   CASE(454)  ! PARTICLE DISK AREA
      PARTICLE_OUTPUT = BOUNDARY_ONE_D(LP%OD_INDEX)%AREA/2._EB
...
REAL(EB), INTENT(IN) :: DT
REAL(EB) :: VC,U_N,ZZ_GET(1:N_TRACKED_SPECIES),H_S,H_S_ALPHA,H_S_J_ALPHA,ENTHALPY_SUM_OLD
INTEGER, INTENT(IN) :: NM
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (N_TRACKED_SPECIES > 1) THEN
      DO N=1,N_TRACKED_SPECIES
         CALL GET_SENSIBLE_ENTHALPY_Z(N,ONE_D%TMP_F,H_S_ALPHA)
         H_S_J_ALPHA = H_S_J_ALPHA + 2._EB*H_S_ALPHA*ONE_D%RHO_D_F(N)*(ZZ(IIG,JJG,KKG,N)-ONE_D%ZZ_F(N))*ONE_D%RDN
      ENDDO
   ENDIF
   Q_DOT(3,NM) = Q_DOT(3,NM) - U_N*ONE_D%RHO_F*H_S*ONE_D%AREA
   Q_DOT(4,NM) = Q_DOT(4,NM) - ONE_D%Q_CON_F*ONE_D%AREA
   Q_DOT(5,NM) = Q_DOT(5,NM) - H_S_J_ALPHA*ONE_D%AREA
   IF (.NOT. HRR_GAS_ONLY) Q_DOT(1,NM) = Q_DOT(1,NM) + ONE_D%Q_DOT_O2_PP*ONE_D%AREA
ENDDO WALL_LOOP
...
CFACE_LOOP : DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   IF (N_TRACKED_SPECIES > 1) THEN
      DO N=1,N_TRACKED_SPECIES
         CALL GET_SENSIBLE_ENTHALPY_Z(N,ONE_D%TMP_F,H_S_ALPHA)
         H_S_J_ALPHA = H_S_J_ALPHA + 2._EB*H_S_ALPHA*ONE_D%RHO_D_F(N)*(CUT_CELL(ICC)%ZZ(N,JCC)-ONE_D%ZZ_F(N))*ONE_D%RDN
      ENDDO
   ENDIF
   Q_DOT(3,NM) = Q_DOT(3,NM) - U_N*ONE_D%RHO_F*H_S*ONE_D%AREA
   Q_DOT(4,NM) = Q_DOT(4,NM) - ONE_D%Q_CON_F*ONE_D%AREA
...
IF (CHAR_OXIDATION .AND. .NOT. HRR_GAS_ONLY) THEN
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      ONE_D => BOUNDARY_ONE_D(LP%OD_INDEX)
      Q_DOT(1,NM) = Q_DOT(1,NM) + LP%PWT*ONE_D%Q_DOT_O2_PP*ONE_D%AREA
   ENDDO PARTICLE_LOOP
ENDIF
...
WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   ONE_D => BOUNDARY_ONE_D(WC%OD_INDEX)
   DO N=1,N_TRACKED_SPECIES
      M_DOT(N,NM) = M_DOT(N,NM) + ONE_D%M_DOT_G_PP_ACTUAL(N)*ONE_D%AREA*ONE_D%AREA_ADJUST
   ENDDO
ENDDO WALL_LOOP2

CFACE_LOOP_2 : DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICF)
...
DO IP=1,NLP
   ...
   IF (BC%Z<=ZS) CYCLE
   IF (BC%Z>=ZF) CYCLE
   DROPMASS = LP%PWT*LPC%FTPR*MAXVAL(ONE_D%X)**3
   RVC = RDX(BC%IIG)*RRN(BC%IIG)*RDY(BC%JJG)*RDZ(BC%KKG)
   WFX(BC%IIG,BC%JJG,BC%KKG) = WFX(BC%IIG,BC%JJG,BC%KKG) + DROPMASS*LP%U*RVC
   WFY(BC%IIG,BC%JJG,BC%KKG) = WFY(BC%IIG,BC%JJG,BC%KKG) + DROPMASS*LP%V*RVC
   WFZ(BC%IIG,BC%JJG,BC%KKG) = WFZ(BC%IIG,BC%JJG,BC%KKG) + DROPMASS*LP%W*RVC
ENDDO
