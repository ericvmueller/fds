...
WRITE(LU_CORE(NM)) Q_DOT_SUM(1:N_Q_DOT,NM),M_DOT_SUM(1:N_M_DOT,NM),MASS_DT(0:N_SPECIES+N_TRACKED_SPECIES,NM)
DO N=1,N_DEVC
   DV => DEVICE(N)
...
READ(LU_RESTART(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),SMOKE3D_CLOCK(NM),RADF_CLOCK(NM), &
             ...
             RTE_SOURCE_CORRECTION_FACTOR,RAD_Q_SUM,KFST4_SUM,ENTHALPY_SUM(NM),&
             BC_CLOCK_HT3D,WALL_COUNTER_HT3D
READ(LU_RESTART(NM)) Q_DOT_SUM(1:N_Q_DOT,NM),M_DOT_SUM(1:N_M_DOT,NM),MASS_DT(0:N_SPECIES+N_TRACKED_SPECIES,NM)
ICYC_RESTART = ICYC
DO N=1,N_DEVC
   DV => DEVICE(N)
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(538)  ! PYRO3D IOR
      ...
      ENDIF
   CASE(539)  ! MEAN FORCING CELL
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (ANY(MEAN_FORCING)) THEN
         IF (MEAN_FORCING_CELL(II,JJ,KK)) GAS_PHASE_OUTPUT_RES = 1._EB
      ENDIF
