...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   ELSE
      ...
      DO K=0,KBP1
         DO J=0,JBP1
            DO I=0,IBP1
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,IS%INDEX,IS%Y_INDEX,IS%Z_INDEX,0,0,IS%VELO_INDEX,0,0,0,0)
            ENDDO
         ENDDO
      ENDDO
   ...
   INDEX2_IF: IF ( IS%INDEX2 /= -1 ) THEN
      ...
      ELSE
         ...
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  QUANTITY2(I,J,K) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,IS%INDEX2,IS%Y_INDEX2,IS%Z_INDEX2,0,0,IS%VELO_INDEX2,0,0,0,0)
               ENDDO
            ENDDO
         ENDDO
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            FF(I,J,K)=GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,S3%QUANTITY_INDEX,S3%Y_INDEX,S3%Z_INDEX,0,0,0,0,0,0,0)
         ENDDO
      ENDDO
   ENDDO
...
INTEGER, INTENT(IN) :: I1,I2,J1,J2,K1,K2,NFACES,NFACES_CUTCELLS,&
                       IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,NM
            ...
            IF (CELLTYPE == CC_CUTCFE) THEN
               ...
               DO IFACECF=1,CUT_FACE(ICF)%NFACE
                  CALL GET_GASCUTFACE_SCALAR_SLICE(VAL_CF,X1AXIS,ICF,IFACECF,CC_INTERP2FACES,CC_CELL_CENTERED,&
                         IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,T,DT,NM)
                  NVF=CUT_FACE(ICF)%CFELEM(1,IFACECF)
                  DO IVCF = 1, NVF-2
                     IFACECUT = IFACECUT + 1
...
SUBROUTINE DUMP_SLICE_GEOM_DATA(FUNIT_DATA,CC_INTERP2FACES,CC_CELL_CENTERED,SLICETYPE, &
                                ...
                                IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,T, &
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: X1AXIS,I,J,K,IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,NM
...
IF (CC1( LOW_IND)>TWENTY_EPSILON_EB) THEN
   VAL_CF_LO = GAS_PHASE_OUTPUT(T,DT,NM,II_LO,JJ_LO,KK_LO,&
                                IND,Y_INDEX,Z_INDEX,0,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX)
ENDIF
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: X1AXIS,ICF,IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,NM
REAL(EB),INTENT(OUT):: VAL_CF
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: X1AXIS,ICF,IFACE,IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,NM
LOGICAL, INTENT(IN) :: CC_INTERP2FACES,CC_CELL_CENTERED
REAL(EB),INTENT(OUT):: VAL_CF
...
DO ISIDE=LOW_IND,LOCAL_IND
   ...
   CASE(CC_FTYPE_CFGAS) ! Cut-cell -> use value from CUT_CELL data struct:
      ...
      VAL_LOC(ISIDE) = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,&
                       IND,Y_INDEX,Z_INDEX,0,PART_INDEX,VELO_INDEX,&
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT.AGL_TERRAIN_SLICE) THEN
      DO K=KK1,KK2
         DO J=JJ1,JJ2
            DO I=II1,II2
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,IND,Y_INDEX,Z_INDEX,0,PART_INDEX,VELO_INDEX,0,&
   ...
   ELSE
      ...
      DO I=II1,II2
         DO J=JJ1,JJ2
            ...
            QUANTITY(I,J,K1) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,KTS,IND,Y_INDEX,Z_INDEX,0,PART_INDEX,VELO_INDEX,0,0,0,0)
         ENDDO
      ENDDO
   ENDIF
      ...
      ELSE
         ...
         IF (ABS(STIME-T_BEGIN)<TWENTY_EPSILON_EB) THEN
            ...
            CALL DUMP_SLICE_GEOM_DATA(LU_SLCF(IQ,NM),CC_INTERP2FACES,SL%CELL_CENTERED,SL%SLICETYPE, &
                              ...
                              IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,PROP_INDEX,REAC_INDEX,MATL_INDEX,T,DT,NM, &
                              SLICE_MIN, SLICE_MAX)
            SLICE_MIN_BOUND = SLICE_MIN
            SLICE_MAX_BOUND = SLICE_MAX
            CHANGE_BOUND = 1
      ...
      CASE(1:299,500:N_OUTPUT_QUANTITIES) OUTPUT_INDEX_SELECT ! gas phase
         ...
         GAS_STATS_SELECT: SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE('null') GAS_STATS_SELECT
               ...
               SDV%VALUE_1 = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,DV%QUANTITY_INDEX(1),DV%Y_INDEX,DV%Z_INDEX,DV%ELEM_INDEX,&
                                              DV%PART_CLASS_INDEX,DV%VELO_INDEX,DV%PIPE_INDEX,DV%PROP_INDEX,DV%REAC_INDEX,&
                                              DV%MATL_INDEX)

               IF (DV%N_QUANTITY>1) &
                  SDV%VALUE_2 = GAS_PHASE_OUTPUT(T,DT,NM,DV%I(2),DV%J(2),DV%K(2),DV%QUANTITY_INDEX(2),DV%Y_INDEX,DV%Z_INDEX,&
               ...
               K_DEVICE_CELL_LOOP: DO K=SDV%K1,SDV%K2
                  J_DEVICE_CELL_LOOP: DO J=SDV%J1,SDV%J2
                     I_DEVICE_CELL_LOOP: DO I=SDV%I1,SDV%I2
                        ...
                        VALUE = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,DV%QUANTITY_INDEX(1),DV%Y_INDEX,DV%Z_INDEX,DV%ELEM_INDEX,&
                                                 DV%PART_CLASS_INDEX,DV%VELO_INDEX,DV%PIPE_INDEX,DV%PROP_INDEX,DV%REAC_INDEX,&
                                                 DV%MATL_INDEX)
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: II,JJ,KK,IND,NM,Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX, &
                       MATL_INDEX
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(85)  ! KOLMOGOROV LENGTH SCALE
      SS = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,84,Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,&
   ...
   CASE(86)  ! CELL REYNOLDS NUMBER
      ...
      KKK = MAX(1,MIN(KK,KBAR))
      DELTA = LES_FILTER_WIDTH(III,JJJ,KKK)
      ETA = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,85,Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,&
                             REAC_INDEX,MATL_INDEX)
   ...
   CASE(114) ! BI-DIRECTIONAL PROBE
      ...
      IF (PY%TC) THEN
         PROBE_TMP = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,110,Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,&
   ...
   FILE_LOOP2 : DO NF=1,N_BNDF
      ...
      IF (REAL(T-T_BEGIN,FB)<2._FB*EPSILON(REAL(T,FB))) THEN
         ...
         CALL DUMP_SLICE_GEOM_DATA(LU_BNDG(NF,NM),.FALSE.,.TRUE.,"INBOUND_FACES",1,STIME,I1,I2,J1,J2,K1,K2,BF%DEBUG, &
                                   IND,BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,0,0,BF%PROP_INDEX,0,0,T,DT,NM, &
