...
ALLOCATE(LU_SMOKE3D(N_SMOKE3D*2,NMESHES)) ! and the SMOKE3D_DENSITY files
...
PART_DIST_LOOP: DO I=1,N_LAGRANGIAN_CLASSES
   ...
   IF (.NOT.LPC%CHECK_DISTRIBUTION .OR. LPC%MONODISPERSE .OR. LPC%DIAMETER<TWENTY_EPSILON_EB) CYCLE PART_DIST_LOOP
   LU = GET_FILE_NUMBER()
...
INTEGER, PARAMETER :: SMOKE3D_VERSION=1
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D) THEN
   ...
   DO N=1,N_SMOKE3D
      ...
      IF (.NOT. APPEND) THEN
         ...
         WRITE(LU_SMOKE3D(N,NM)) INTEGER_ONE,SMOKE3D_VERSION,0,M%IBAR,0,M%JBAR,0,M%KBAR
         ...
         WRITE(LU_SMOKE3D(N+N_SMOKE3D,NM),*) SMOKE3D_VERSION
   ...
   CREATE_PATCHES: IF (.NOT.APPEND) THEN
      ...
      DO N=1,M%N_NEIGHBORING_MESHES
         ...
         OBST_LOOP: DO OBST_INDEX=1,M4%N_OBST
            ...
            IF (ABS(OB%X1-M%XF)<TWENTY_EPSILON_EB) IOR = -1
            IF (ABS(OB%X2-M%XS)<TWENTY_EPSILON_EB) IOR =  1
            IF (ABS(OB%Y1-M%YF)<TWENTY_EPSILON_EB) IOR = -2
            IF (ABS(OB%Y2-M%YS)<TWENTY_EPSILON_EB) IOR =  2
            IF (ABS(OB%Z1-M%ZF)<TWENTY_EPSILON_EB) IOR = -3
            IF (ABS(OB%Z2-M%ZS)<TWENTY_EPSILON_EB) IOR =  3
...
! Set ODE solver
   ...
   REACTION_LOOP: DO NR=1,N_REACTIONS
      ...
      DO NN=1,N_SPECIES
         IF (ABS(RN%NU_SPECIES(NN))<=TWENTY_EPSILON_EB) CYCLE
      ...
      ELSE
         ...
         ENDDO
         IF (ABS(RN%N_T)>TWENTY_EPSILON_EB) WRITE(LU_OUTPUT,'(6X,A,50X,F12.6)') 'Temperature',RN%N_T
         IF (RN%THIRD_BODY) THEN
            ...
            IF (RN%N_THIRD > 0) THEN
               ...
               DO NN=1,N_SPECIES
                  IF (ABS(RN%THIRD_EFF(NN)-1._EB)>TWENTY_EPSILON_EB) &
      ...
      IF (COMPUTE_ADIABATIC_FLAME_TEMPERATURE .AND. RN%FAST_CHEMISTRY) THEN
         ...
         ZZ_PROD=0._EB
         DO NN=1,N_TRACKED_SPECIES
            IF (RN%NU(NN) < -TWENTY_EPSILON_EB) ZZ_REAC(NN)=RN%NU(NN)*SPECIES_MIXTURE(NN)%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW
            IF (RN%NU(NN) >  TWENTY_EPSILON_EB) ZZ_PROD(NN)=RN%NU(NN)*SPECIES_MIXTURE(NN)%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW
         ENDDO
         ! add background diluents
         DO NN=1,N_TRACKED_SPECIES
            IF (ABS(RN%NU(NN)) > TWENTY_EPSILON_EB) CYCLE
            IF (SPECIES_MIXTURE(RN%AIR_SMIX_INDEX)%ZZ0>TWENTY_EPSILON_EB) THEN
         ...
         IF (ABS(SUM(ZZ_REAC))>TWENTY_EPSILON_EB) ZZ_REAC = ZZ_REAC/SUM(ZZ_REAC)
         IF (ABS(SUM(ZZ_PROD))>TWENTY_EPSILON_EB) ZZ_PROD = ZZ_PROD/SUM(ZZ_PROD)
...
MATL_LOOP: DO N=1,N_MATL
   ...
   IF (ML%PYROLYSIS_MODEL==PYROLYSIS_SOLID .OR. ML%PYROLYSIS_MODEL==PYROLYSIS_SURFACE_OXIDATION) THEN
      DO NR=1,ML%N_REACTIONS
         ...
         IF (ML%TMP_REF(NR) <= TWENTY_EPSILON_EB) THEN
            ITMP = INT(TMPA)
...
SURFLOOP: DO N=0,N_SURF
   ...
   IF (ABS(SF%VEL)>TWENTY_EPSILON_EB)             WRITE(LU_OUTPUT,'(A,F8.3)')  '     Normal Velocity (m/s)      ', SF%VEL
   IF (ABS(SF%MASS_FLUX_TOTAL)>TWENTY_EPSILON_EB) WRITE(LU_OUTPUT,'(A,ES10.3)') &
      '     Total Mass Flux (kg/m^2/s) ', SF%MASS_FLUX_TOTAL
   IF (ABS(SF%VOLUME_FLOW)>TWENTY_EPSILON_EB)     WRITE(LU_OUTPUT,'(A,ES10.3)') '     Volume Flow     (m^3/s)    ', SF%VOLUME_FLOW

   IF (SF%HRRPUA>0._EB .AND. .NOT.SF%VEG_LSET_SPREAD) &
      WRITE(LU_OUTPUT,'(A,F12.1)') '     HRR Per Unit Area (kW/m2) ', SF%HRRPUA/1000._EB
   DO NN=1,N_TRACKED_SPECIES
      IF (SF%MASS_FRACTION(NN)>TWENTY_EPSILON_EB) WRITE(LU_OUTPUT,'(5X,A,A,8X,F6.3)') &
          TRIM(SPECIES_MIXTURE(NN)%ID),' Mass Fraction',SF%MASS_FRACTION(NN)
      IF (ABS(SF%MASS_FLUX(NN))>TWENTY_EPSILON_EB) WRITE(LU_OUTPUT,'(5X,A,A,2X,ES10.3)') &
   ...
   IF (SF%VEG_LSET_SPREAD) THEN
      ...
      ELSE
         WRITE(LU_OUTPUT,'(A)')        '     Wind Height                 6.1 m + Andrews UMF'
      ENDIF
   ENDIF
...
IF (LEVEL_SET_MODE>0 .AND. LEVEL_SET_TAU_ACCEL>0._EB) WRITE(LU_CORE(NM)) ROS_LAG
IF (LEVEL_SET_MODE>0 .AND. LEVEL_SET_WIND_TAU>0._EB) WRITE(LU_CORE(NM)) U_LS,V_LS
...
IF (LEVEL_SET_MODE>0) THEN
   READ(LU_RESTART(NM)) PHI_LS
   IF (LEVEL_SET_TAU_ACCEL>0._EB .AND. ANY(PHI_LS>=0._EB)) LEVEL_SET_IGNITED = .TRUE.
ENDIF
IF (LEVEL_SET_MODE>0 .AND. LEVEL_SET_TAU_ACCEL>0._EB) READ(LU_RESTART(NM)) ROS_LAG
IF (LEVEL_SET_MODE>0 .AND. LEVEL_SET_WIND_TAU>0._EB) READ(LU_RESTART(NM)) U_LS,V_LS
...
IF (SIM_MODE==DNS_MODE) THEN
   ...
   ELSE
      OUT_DIGITS = 7
   ENDIF
   IF (ABS(TIME_SHRINK_FACTOR-1._EB) > TWENTY_EPSILON_EB) THEN
      STIME = T_BEGIN + (T-T_BEGIN) * TIME_SHRINK_FACTOR
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   ELSE
      ...
      DO K=0,KBP1
         DO J=0,JBP1
            DO I=0,IBP1
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,IS%INDEX,Y_INDEX=IS%Y_INDEX,Z_INDEX=IS%Z_INDEX,&
                                                  VELO_INDEX=IS%VELO_INDEX)
            ENDDO
         ENDDO
      ENDDO
   ...
   INDEX2_IF: IF ( IS%INDEX2 /= -1 ) THEN
      ...
      ELSE
         ...
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  QUANTITY2(I,J,K) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,IS%INDEX2,Y_INDEX=IS%Y_INDEX2,Z_INDEX=IS%Z_INDEX2,&
                                                      VELO_INDEX=IS%VELO_INDEX2)
               ENDDO
            ENDDO
         ENDDO
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            FF(I,J,K)=GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,S3%QUANTITY_INDEX,Y_INDEX=S3%Y_INDEX,Z_INDEX=S3%Z_INDEX)
         ENDDO
      ENDDO
   ENDDO
   ...
   CALL SMOKE3D_TO_FILE(NM,STIME,N,QQ_PACK,IBP1,JBP1,KBP1)
   DEALLOCATE(QQ_PACK)
...
SUBROUTINE GET_GEOMVALS(CC_INTERP2FACES,CC_CELL_CENTERED,SLICETYPE,&
                        ...
                        IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,&
                        PIPE_INDEX,PROP_INDEX,REAC_INDEX,T,DT,NM,OPT_BNDF_INDEX)
...
INTEGER, INTENT(IN) :: I1,I2,J1,J2,K1,K2,NFACES,NFACES_CUTCELLS,&
                       IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,NM
            ...
            IF (CELLTYPE == CC_CUTCFE) THEN
               ...
               DO IFACECF=1,CUT_FACE(ICF)%NFACE
                  CALL GET_GASCUTFACE_SCALAR_SLICE(VAL_CF,X1AXIS,ICF,IFACECF,CC_INTERP2FACES,CC_CELL_CENTERED,&
                         IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,T,DT,NM)
                  NVF=CUT_FACE(ICF)%CFELEM(1,IFACECF)
                  DO IVCF = 1, NVF-2
                     IFACECUT = IFACECUT + 1
...
SUBROUTINE DUMP_SLICE_GEOM_DATA(FUNIT_DATA,CC_INTERP2FACES,CC_CELL_CENTERED,SLICETYPE, &
                                ...
                                IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,T, &
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: X1AXIS,I,J,K,IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,NM
REAL(EB),INTENT(OUT):: VAL_CF

! Local Variables:
INTEGER :: II_LO,II_HI,JJ_LO,JJ_HI,KK_LO,KK_HI,SOLID_LO,SOLID_HI
REAL(EB):: CC1(LOW_IND:HIGH_IND),CCSUM
REAL(EB) :: VAL_CF_LO,VAL_CF_HI
...
IF (CC1( LOW_IND)>TWENTY_EPSILON_EB) THEN
   VAL_CF_LO = GAS_PHASE_OUTPUT(T,DT,NM,II_LO,JJ_LO,KK_LO,IND,Y_INDEX=Y_INDEX,Z_INDEX=Z_INDEX,PART_INDEX=PART_INDEX,&
                                VELO_INDEX=VELO_INDEX,PIPE_INDEX=PIPE_INDEX,PROP_INDEX=PROP_INDEX,REAC_INDEX=REAC_INDEX)
ENDIF

IF (CC1(HIGH_IND)>TWENTY_EPSILON_EB) THEN
   VAL_CF_HI = GAS_PHASE_OUTPUT(T,DT,NM,II_HI,JJ_HI,KK_HI,IND,Y_INDEX=Y_INDEX,Z_INDEX=Z_INDEX,PART_INDEX=PART_INDEX,&
                                VELO_INDEX=VELO_INDEX,PIPE_INDEX=PIPE_INDEX,PROP_INDEX=PROP_INDEX,REAC_INDEX=REAC_INDEX)
ENDIF
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: X1AXIS,ICF,IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,NM
REAL(EB),INTENT(OUT):: VAL_CF
...
IF (FOUND) THEN
   VAL_CF = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,IND,Y_INDEX=Y_INDEX,Z_INDEX=Z_INDEX,PART_INDEX=PART_INDEX,VELO_INDEX=VELO_INDEX,&
                             PIPE_INDEX=PIPE_INDEX,PROP_INDEX=PROP_INDEX,REAC_INDEX=REAC_INDEX)
ENDIF
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: X1AXIS,ICF,IFACE,IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,NM
LOGICAL, INTENT(IN) :: CC_INTERP2FACES,CC_CELL_CENTERED
REAL(EB),INTENT(OUT):: VAL_CF
...
DO ISIDE=LOW_IND,LOCAL_IND
   ...
   CASE(CC_FTYPE_CFGAS) ! Cut-cell -> use value from CUT_CELL data struct:
      ...
      VAL_LOC(ISIDE) = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,IND,Y_INDEX=Y_INDEX,Z_INDEX=Z_INDEX,PART_INDEX=PART_INDEX,&
                       VELO_INDEX=VELO_INDEX,PIPE_INDEX=PIPE_INDEX,PROP_INDEX=PROP_INDEX,REAC_INDEX=REAC_INDEX,&
                       ICC_IN=ICC,JCC_IN=JCC)
   END SELECT
ENDDO
...
INTEGER :: I,J,K,NQT,I1,I2,J1,J2,K1,K2,ITM,ITM1,IQ,IQ2,IQ3,IQQ,IND,II1,II2,JJ1,JJ2,KK1,KK2, &
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT.AGL_TERRAIN_SLICE) THEN
      DO K=KK1,KK2
         DO J=JJ1,JJ2
            DO I=II1,II2
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,IND,Y_INDEX=Y_INDEX,Z_INDEX=Z_INDEX,PART_INDEX=PART_INDEX,&
                                                  VELO_INDEX=VELO_INDEX,PROP_INDEX=PROP_INDEX,REAC_INDEX=REAC_INDEX)
            ENDDO
         ENDDO
      ENDDO
   ELSE
      ...
      DO I=II1,II2
         DO J=JJ1,JJ2
            ...
            QUANTITY(I,J,K1) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,KTS,IND,Y_INDEX=Y_INDEX,Z_INDEX=Z_INDEX,PART_INDEX=PART_INDEX,&
                                                VELO_INDEX=VELO_INDEX)
         ENDDO
      ENDDO
   ENDIF
      ...
      ELSE
         ...
         IF (ABS(STIME-T_BEGIN)<TWENTY_EPSILON_EB) THEN
            ...
            CALL DUMP_SLICE_GEOM_DATA(LU_SLCF(IQ,NM),CC_INTERP2FACES,SL%CELL_CENTERED,SL%SLICETYPE, &
                              ...
                              IND,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,PROP_INDEX,REAC_INDEX,T,DT,NM, &
                              SLICE_MIN, SLICE_MAX)
            SLICE_MIN_BOUND = SLICE_MIN
            SLICE_MAX_BOUND = SLICE_MAX
            CHANGE_BOUND = 1
...
IF (ABS(S(I,J,K))<=TWENTY_EPSILON_EB) THEN
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   IF (ABS(T-T_BEGIN)>TWENTY_EPSILON_EB) THEN
         ...
         SOLID_STATS_SELECT: SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE('null') SOLID_STATS_SELECT
               ...
               IF (DV%WALL_INDEX>0) THEN
                  SDV%VALUE_1 = SOLID_PHASE_OUTPUT(ABS(DV%QUANTITY_INDEX(1)),T,NM,DV%Y_INDEX,DV%Z_INDEX,DV%PART_CLASS_INDEX,&
            ...
            CASE DEFAULT SOLID_STATS_SELECT
               ...
               WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
                  ...
                  VALUE = SOLID_PHASE_OUTPUT(ABS(DV%QUANTITY_INDEX(1)),T,NM,DV%Y_INDEX,DV%Z_INDEX,DV%PART_CLASS_INDEX,&
      ...
      CASE(1:299,500:N_OUTPUT_QUANTITIES) OUTPUT_INDEX_SELECT ! gas phase
         ...
         GAS_STATS_SELECT: SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE('null') GAS_STATS_SELECT
               ...
               SDV%VALUE_1 = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,DV%QUANTITY_INDEX(1),Y_INDEX=DV%Y_INDEX,Z_INDEX=DV%Z_INDEX,&
               ...
               IF (DV%N_QUANTITY>1) &
                  SDV%VALUE_2 = GAS_PHASE_OUTPUT(T,DT,NM,DV%I(2),DV%J(2),DV%K(2),DV%QUANTITY_INDEX(2),Y_INDEX=DV%Y_INDEX,&
                                                 Z_INDEX=DV%Z_INDEX,ELEM_INDX=DV%ELEM_INDEX,PART_INDEX=DV%PART_CLASS_INDEX,&
                                                 VELO_INDEX=DV%VELO_INDEX,PIPE_INDEX=DV%PIPE_INDEX,PROP_INDEX=DV%PROP_INDEX,&
                                                 REAC_INDEX=DV%REAC_INDEX,DEVC_INDEX=N)
               ...
               K_DEVICE_CELL_LOOP: DO K=SDV%K1,SDV%K2
                  J_DEVICE_CELL_LOOP: DO J=SDV%J1,SDV%J2
                     I_DEVICE_CELL_LOOP: DO I=SDV%I1,SDV%I2
                        ...
                        VALUE = GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,DV%QUANTITY_INDEX(1),Y_INDEX=DV%Y_INDEX,Z_INDEX=DV%Z_INDEX,&
                                                 ELEM_INDX=DV%ELEM_INDEX,PART_INDEX=DV%PART_CLASS_INDEX,VELO_INDEX=DV%VELO_INDEX,&
                                                 PIPE_INDEX=DV%PIPE_INDEX,PROP_INDEX=DV%PROP_INDEX,REAC_INDEX=DV%REAC_INDEX,&
                                                 DEVC_INDEX=N)
      ...
      CASE(400:454) OUTPUT_INDEX_SELECT  ! Particle-specific output
         ...
         SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE('null')
               ...
               IF (LP_INDEX>0) THEN
                  SDV%VALUE_1 = PARTICLE_OUTPUT(NM,T,ABS(DV%QUANTITY_INDEX(1)),LP_INDEX)
               ENDIF
            ...
            CASE DEFAULT
               ...
               PARTICLE_LOOP2: DO IP=1,NLP
                  ...
                  VALUE = PARTICLE_OUTPUT(NM,T,ABS(DV%QUANTITY_INDEX(1)),IP)
                  B1 => BOUNDARY_PROP1(LP%B1_INDEX)
                  CALL SELECT_SPATIAL_STATISTIC(OPT_LP_INDEX=IP)
...
IF (SUM_WALL_WGT_LOC>TWENTY_EPSILON_EB) THEN
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   SELECT CASE (DV%SPATIAL_STATISTIC)
      ...
      CASE('MASS MEAN','VOLUME MEAN','MEAN','INTERPOLATION','CENTROID X','CENTROID Y','CENTROID Z')
         IF (ABS(DV%VALUE_2)>TWENTY_EPSILON_EB) THEN
   ...
   IF (DV%QUANTITY(1)=='LAYER HEIGHT' .OR. DV%QUANTITY(1)=='UPPER TEMPERATURE' .OR. DV%QUANTITY(1)=='LOWER TEMPERATURE') THEN
      ...
      IF (ABS(DV%VALUE_3)<=TWENTY_EPSILON_EB) THEN  ! VALUE_3 is Integral_z_int^H T(z) dz
      ...
      IF (ABS(Z_INT_DENOM)<=TWENTY_EPSILON_EB) THEN
         DV%Z_INT = DV%VALUE_4
      ELSE
   ...
   IF (DV%SMOOTHING_TIME>TWENTY_EPSILON_EB) DV%SMOOTHING_FACTOR = MAX(0._EB,1._EB - DT/DV%SMOOTHING_TIME)
...
REAL(EB) RECURSIVE FUNCTION GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,IND,Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,&
                                             PROP_INDEX,REAC_INDEX,DEVC_INDEX,ICC_IN,JCC_IN) RESULT(GAS_PHASE_OUTPUT_RES)
...
INTEGER, INTENT(IN), OPTIONAL :: Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,DEVC_INDEX,&
                                 ICC_IN,JCC_IN
REAL(EB) :: H_TC,TMP_TC,RE_D,NUSSELT,VEL,K_G,MU_G,COSTHETA,FAC,&
            ...
            VELSR,WATER_VOL_FRAC,WATER_TEMP,RHS,DT_C,DT_E,T_RATIO,Y_E_LAG, H_G,H_G_SUM,CPBAR,CP,ZZ_GET(1:N_TRACKED_SPECIES),RCON,&
...
IF (PRESENT(PROP_INDEX)) THEN
   PY => PROPERTY(PROP_INDEX)
ELSE
   PY => PROPERTY(0)
ENDIF

IF (PRESENT(DEVC_INDEX)) THEN
   DV => DEVICE(DEVC_INDEX)
   IF (DV%N_SUBDEVICES>0) THEN
      IF (DV%SUBDEVICE_INDEX(NM)>0) SDV => DV%SUBDEVICE(DV%SUBDEVICE_INDEX(NM))
   ENDIF
ENDIF
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(15)  ! MIXING TIME
      ...
      MIX_TIME(II,JJ,KK) = 0._EB ! reset MIX_TIME
   ...
   CASE(85)  ! KOLMOGOROV LENGTH SCALE
      SS = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,84)
   ...
   CASE(86)  ! CELL REYNOLDS NUMBER
      ...
      KKK = MAX(1,MIN(KK,KBAR))
      DELTA = LES_FILTER_WIDTH(III,JJJ,KKK)
      ETA = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,85)
   ...
   CASE(98)  ! OPTICAL DENSITY
      ...
      ELSEIF (Y_INDEX>0) THEN
         MEC = SPECIES(Y_INDEX)%MASS_EXTINCTION_COEFFICIENT
      ENDIF
      GAS_PHASE_OUTPUT_RES = Y_SPECIES*RHO(II,JJ,KK)*MEC/LOGTEN
   ...
   CASE(114) ! BI-DIRECTIONAL PROBE
      ...
      IF (PY%TC) THEN
         PROBE_TMP = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,110,PROP_INDEX=PROP_INDEX,DEVC_INDEX=DEVC_INDEX) + TMPM
      ELSE
   ...
   CASE(156) ! SPRINKLER LINK TEMPERATURE
      ...
      IF (H2O_INDEX > 0) THEN
         DO NN = 1,N_LAGRANGIAN_CLASSES
            ...
            ENDIF
         ENDDO
         IF (WATER_VOL_FRAC > TWENTY_EPSILON_EB) WATER_TEMP = WATER_TEMP / WATER_VOL_FRAC
      ENDIF
      RHS      = (VELSR*(TMP_G-DV%TMP_L) - PY%C_FACTOR*(DV%TMP_L-PY%INITIAL_TEMPERATURE) - &
                 C_DIMARZO*VEL*WATER_VOL_FRAC*(DV%TMP_L - WATER_TEMP))/PY%RTI
   ...
   CASE(185,187) ! NUMBER OF PARTICLES, TOTAL NUMBER OF PARTICLES
...
INTEGER, INTENT(IN), OPTIONAL :: OPT_WALL_INDEX,OPT_LP_INDEX,OPT_CFACE_INDEX,OPT_BNDF_INDEX,OPT_DEVC_INDEX,OPT_CUT_FACE_INDEX,&
                                 OPT_NODE_INDEX,OPT_PROF_INDEX,OPT_PROP_INDEX
...
IF (PRESENT(OPT_PROP_INDEX)) THEN
   PY => PROPERTY(OPT_PROP_INDEX)
ELSE
   PY => PROPERTY(0)
ENDIF
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(83) ! INSIDE AVERAGE TEMPERATURE
      IF (SF%THERMAL_BC_INDEX/=THERMALLY_THICK) RETURN
      SOLID_PHASE_OUTPUT = SURFACE_DENSITY(4,SF,ONE_D) - TMPM
...
REAL(EB) FUNCTION PARTICLE_OUTPUT(NM,T,IND,LP_INDEX,Y_INDEX,Z_INDEX)

! Assign particle output quantities to devices

INTEGER, INTENT(IN) :: IND,LP_INDEX,NM
...
SELECT CASE(IND)
   ...
   CASE(:-1)  ! SOLID PHASE QUANTITY
      PARTICLE_OUTPUT = SOLID_PHASE_OUTPUT(ABS(IND),T,NM,Y_INDEX,Z_INDEX,LP%CLASS_INDEX,OPT_LP_INDEX=LP_INDEX)
END SELECT
   ...
   DO J=1,N_DEVC
      ...
      IF (PY%HISTOGRAM) THEN
         ...
         DO NN =1,MAX_HISTOGRAM_NBINS
            ...
            IF (PY%HISTOGRAM_NORMALIZE .AND. CONST>TWENTY_EPSILON_EB) VALUE = VALUE / CONST
...
PROF_LOOP: DO N=1,N_PROF
   ...
   DO I = 1, NWP
      IF (PF%WALL_INDEX>0) THEN
         PF_TEMP(I) = SOLID_PHASE_OUTPUT(ABS(PF%QUANTITY_INDEX),T,NM,1,1,LPC_INDEX,&
   ...
   IF (PF%QUANTITY == 'INSIDE WALL TEMPERATURE') THEN
      IF (PF%WALL_INDEX>0) THEN
         PF_TEMP(0) = SOLID_PHASE_OUTPUT(ABS(PF%QUANTITY_INDEX),T,NM,1,1,LPC_INDEX,&
                                          OPT_WALL_INDEX=PF%WALL_INDEX,OPT_NODE_INDEX=0,OPT_PROF_INDEX=N)
         PF_TEMP(NWP+1) = SOLID_PHASE_OUTPUT(ABS(PF%QUANTITY_INDEX),T,NM,1,1,LPC_INDEX,&
...
FILE_LOOP: DO NF=1,N_BNDF
            ...
            DO I=PA%IG1,PA%IG2
               ...
               IF (WALL(IW)%BOUNDARY_TYPE/=NULL_BOUNDARY .AND. &
                  ...
                  PP(L,N)  = REAL(SOLID_PHASE_OUTPUT(IND,T,NM,BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,OPT_WALL_INDEX=IW,&
                                                     OPT_BNDF_INDEX=NF,OPT_PROP_INDEX=BF%PROP_INDEX),FB)
               ENDIF
            ENDDO
         ENDDO
      ENDDO
   ...
   CHANGE_BOUND = 0
   IF (REAL(T-T_BEGIN,FB)<2._FB*EPSILON(REAL(T,FB))) THEN
...
IF (CC_IBM) THEN
   FILE_LOOP2 : DO NF=1,N_BNDF
      ...
      IF (REAL(T-T_BEGIN,FB)<2._FB*EPSILON(REAL(T,FB))) THEN
         ...
         CALL DUMP_SLICE_GEOM_DATA(LU_BNDG(NF,NM),.FALSE.,.TRUE.,"INBOUND_FACES",1,STIME,I1,I2,J1,J2,K1,K2,BF%DEBUG, &
                                   IND,BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,0,0,BF%PROP_INDEX,0,T,DT,NM, &
      ...
      ELSE
         ...
         CALL DUMP_SLICE_GEOM_DATA(LU_BNDG(NF,NM),.FALSE.,.TRUE.,"INBOUND_FACES",0,STIME,I1,I2,J1,J2,K1,K2,BF%DEBUG, &
                         IND,BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,0,0,BF%PROP_INDEX,0,T,DT,NM, &
                         BOUND_MIN, BOUND_MAX, OPT_BNDF_INDEX=NF)
...
SUBROUTINE EXTRAPOLATE_EXTREMA(DV)
...
IF (ITEM=='DEVC') THEN
   DO N = 1,N_DEVC
      IF (ABS(DEVICE(N)%T_CHANGE-T) < TWENTY_EPSILON_EB) THEN
...
IF (ITEM=='CTRL') THEN
   DO N = 1,N_CTRL
      IF (ABS(CONTROL(N)%T_CHANGE-T) < TWENTY_EPSILON_EB) THEN
