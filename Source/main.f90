...
USE VELO
USE RAD
USE RADCONS, ONLY : DLN,TIME_STEP_INCREMENT
...
USE TURBULENCE, ONLY: NS_ANALYTICAL_SOLUTION,INIT_TURB_ARRAYS,COMPRESSION_WAVE,&
                      ...
                      SYNTHETIC_TURBULENCE,SYNTHETIC_EDDY_SETUP,SYNTHETIC_EDDY_SETUP_FINALIZE,SANDIA_DAT
...
IF(RADIATION .AND. .NOT. ALLOW_RANDOM_RADIATION_ROTATION) THEN
   ...
   CALL ALLOCATE_RADIAITON_SEND_PKG()
ENDIF
...
ENDDO
! If any rank has SYNTHETIC_EDDY_METHOD=.TRUE., then all ranks will have SYNTHETIC_EDDY_METHOD=.TRUE.
! to ensure that all ranks have the same value of SYNTHETIC_EDDY_METHOD for the final setup.
IF (N_MPI_PROCESSES>1) CALL MPI_ALLREDUCE(MPI_IN_PLACE,SYNTHETIC_EDDY_METHOD,&
   INTEGER_ONE,MPI_LOGICAL,MPI_LOR,MPI_COMM_WORLD,IERR)
IF (SYNTHETIC_EDDY_METHOD) CALL SYNTHETIC_EDDY_SETUP_FINALIZE
...
IF (.NOT.RESTART) THEN
   ...
   DO ITER=1,INITIAL_RADIATION_ITERATIONS
      IF (RADIATION .AND. ALLOW_RANDOM_RADIATION_ROTATION .AND. ITER==1) THEN
         START_NEW_ANGLE_CYCLE = MOD(MESHES(LOWER_MESH_INDEX)%RAD_CALL_COUNTER,TIME_STEP_INCREMENT)==0 
...
MAIN_LOOP: DO
   ...
   IF ((UNFREEZE_TIME > 0._EB).AND.(T>UNFREEZE_TIME).AND.FREEZE_VELOCITY) THEN
   ...
   IF (SYNTHETIC_EDDY_METHOD) CALL SYNTHETIC_TURBULENCE(DT,T)
...
DO NOM=1,NMESHES
   ...
   IF (M%OMESH(NOM)%NIC_R>0) THEN
      ...
      ALLOCATE(M%OMESH(NOM)%IL_R_OLD(M%OMESH(NOM)%NIC_R,NUMBER_RADIATION_ANGLES,NUMBER_SPECTRAL_BANDS))
...
INTEGER :: RNODE,SNODE,IJK_SIZE,NN,NOM
REAL(EB) :: TNOW
TYPE (STORAGE_TYPE), POINTER :: OS
...
INTEGER :: NNN,NN1,NN2,IPC,II1,II2,JJ1,JJ2,KK1,KK2,NQT2,NN,IOR,AIC,OBST_INDEX,IP,IW
...
SUBROUTINE ALLOCATE_RADIAITON_RECV_PKG()
INTEGER :: I,NM,NN,NOM,NRA,NRA_MAX,AIC,LL,N,SNODE,RNODE
REAL(EB):: TNOW
TNOW = CURRENT_TIME()
DO I=1,N_REQ5  ; CALL MPI_REQUEST_FREE(REQ5(I) ,IERR) ; ENDDO
N_REQ5 = 0
RECEIVING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SENDING_MESH_LOOP: DO NN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (M3%NIC_R>0) THEN
         ...
         IF(ALLOCATED(M3%REAL_RECV_PKG5))DEALLOCATE(M3%REAL_RECV_PKG5)
...
ENDDO RECEIVING_MESH_LOOP
T_USED(11)=T_USED(11) + CURRENT_TIME() - TNOW
END SUBROUTINE ALLOCATE_RADIAITON_RECV_PKG


! Determine the maximum number of radiation angles that are to be sent
SUBROUTINE ALLOCATE_RADIAITON_SEND_PKG()
INTEGER :: NM,NNN,NOM,NRA,NRA_MAX,AIC,LL,N,SNODE,RNODE,IERR
REAL(EB):: TNOW
TNOW = CURRENT_TIME()
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   M =>MESHES(NM)
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (M3%NIC_S>0 .AND. RNODE/=SNODE) THEN
         ...
         DO AIC=1,ANGLE_INCREMENT
            ...
            ENDDO
         ENDDO
         IF(ALLOCATED(M3%REAL_SEND_PKG5))DEALLOCATE(M3%REAL_SEND_PKG5)
...
ENDDO SENDING_MESH_LOOP
T_USED(11)=T_USED(11) + CURRENT_TIME() - TNOW
END SUBROUTINE ALLOCATE_RADIAITON_SEND_PKG
