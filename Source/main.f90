...
REAL(EB), DIMENSION(NMESHES) :: CFL_VALUES,VN_VALUES,PART_CFL_VALUES,DRAG_CFL_VALUES
INTEGER :: NM_CFL_MAX,NM_VN_MAX,NM_PART_MAX,NM_DRAG_MAX
CHARACTER(512) :: CFL_LINE
CHARACTER(64) :: CFL_APPEND
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   REAL_BUFFER_20(19,NM)= DT*MESHES(NM)%PART_UVWMAX
   REAL_BUFFER_20(20,NM)= DT*MESHES(NM)%DRAG_STABILITY_MAX
ENDDO
...
IF (MY_RANK==0) THEN
   ...
   PART_CFL_VALUES(:)  = REAL_BUFFER_20(19,:)
   DRAG_CFL_VALUES(:)  = REAL_BUFFER_20(20,:)
   NM_CFL_MAX  = MAXLOC(CFL_VALUES,DIM=1)
   NM_VN_MAX   = MAXLOC(VN_VALUES,DIM=1)
   NM_PART_MAX = MAXLOC(PART_CFL_VALUES,DIM=1)
   NM_DRAG_MAX = MAXLOC(DRAG_CFL_VALUES,DIM=1)
   WRITE(CFL_LINE,'(I7,A,2(ES12.4,A),F6.3,A,4(I4,A),6(F7.3,A),4(ES12.4,A),F6.3,A,4(I4,A))') &
      ICYC,',',T,',',DT,',',&
      ...
      REAL_BUFFER_20(18,NM_CFL_MAX),',',MAXVAL(VN_VALUES),',',NM_VN_MAX,',',&
      NINT(REAL_BUFFER_20(6,NM_VN_MAX)),',',NINT(REAL_BUFFER_20(7,NM_VN_MAX)),',',NINT(REAL_BUFFER_20(8,NM_VN_MAX))
   IF (PARTICLE_CFL) THEN
      WRITE(CFL_APPEND,'(A,ES12.4,A,I4)') ',',REAL_BUFFER_20(19,NM_PART_MAX),',',NM_PART_MAX
      CFL_LINE = TRIM(CFL_LINE)//TRIM(CFL_APPEND)
   ENDIF
   IF (DRAG_CFL) THEN
      WRITE(CFL_APPEND,'(A,ES12.4,A,I4)') ',',REAL_BUFFER_20(20,NM_DRAG_MAX),',',NM_DRAG_MAX
      CFL_LINE = TRIM(CFL_LINE)//TRIM(CFL_APPEND)
   ENDIF
   WRITE(LU_CFL,'(A)') TRIM(CFL_LINE)
ENDIF
