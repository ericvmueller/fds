...
! When using MEAN_FORCING, ensure that all meshes are forcing toward the same target velocity, e.g, U_MEAN_GLOBAL

IF(MEAN_FORCING(1) .AND. I_RAMP_U0_Z==0) THEN
   IF (N_MPI_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, U_MEAN_LOC, NMESHES, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, IERR)
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, M_VOLU_LOC, NMESHES, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, IERR)
   ENDIF
   IF (SUM(M_VOLU_LOC)>TWO_EPSILON_EB) THEN
      U_MEAN_GLOBAL = SUM(U_MEAN_LOC)/SUM(M_VOLU_LOC)
   ELSE
      U_MEAN_GLOBAL = 0._EB
   ENDIF
   U_MEAN_LOC = 0._EB
...
IF(MEAN_FORCING(2) .AND. I_RAMP_V0_Z==0) THEN
   IF (N_MPI_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, V_MEAN_LOC, NMESHES, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, IERR)
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, M_VOLV_LOC, NMESHES, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, IERR)
   ENDIF
   IF (SUM(M_VOLV_LOC)>TWO_EPSILON_EB) THEN
      V_MEAN_GLOBAL = SUM(V_MEAN_LOC)/SUM(M_VOLV_LOC)
   ELSE
      V_MEAN_GLOBAL = 0._EB
   ENDIF
   V_MEAN_LOC = 0._EB
...
IF(MEAN_FORCING(3) .AND. I_RAMP_W0_Z==0) THEN
   IF (N_MPI_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, W_MEAN_LOC, NMESHES, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, IERR)
      CALL MPI_ALLREDUCE(MPI_IN_PLACE, M_VOLW_LOC, NMESHES, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, IERR)
   ENDIF
   IF (SUM(M_VOLW_LOC)>TWO_EPSILON_EB) THEN
      W_MEAN_GLOBAL = SUM(W_MEAN_LOC)/SUM(M_VOLW_LOC)
   ELSE
      W_MEAN_GLOBAL = 0._EB
   ENDIF
   W_MEAN_LOC = 0._EB
      ...
      DEVICE_LOOP_2: DO N=1,N_DEVC
         ...
         IF (OP_INDEX>1 .AND. (DV%SPATIAL_STATISTIC(1:6)=='MINLOC'.OR.DV%SPATIAL_STATISTIC(1:6)=='MAXLOC')) THEN
            ...
            DO NN=1,DV%N_SUBDEVICES
               ...
               ENDIF
            ENDDO
            IF (MYID==0 .AND. .NOT.NO_NEED_TO_RECV) &
