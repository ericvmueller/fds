...
USE COMPLEX_GEOMETRY, ONLY: INIT_CUTCELL_DATA, CCIBM_SET_DATA, CCIBM_END_STEP, FINISH_CCIBM, &
                            ...
                            CCIBM_TARGET_VELOCITY,ROTATED_CUBE_ANN_SOLN,MESH_CC_EXCHANGE
USE OPENMP
USE MPI
...
DO ITER=1,INITIAL_RADIATION_ITERATIONS
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      IF (EVACUATION_ONLY(NM)) THEN
         IF (MY_RANK==EVAC_PROCESS .AND. RADIATION .AND..NOT.ALL(EVACUATION_ONLY)) EXCHANGE_RADIATION=.TRUE.
         CYCLE
      ENDIF
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   CALL INSERT_ALL_PARTICLES(T,DT,NM)
...
IF (ANY(EVACUATION_ONLY).OR.EVACUATION_WRITE_FED) THEN
...
MAIN_LOOP: DO
   ...
   IF (ANY(EVACUATION_ONLY).OR.EVACUATION_WRITE_FED) CALL EVAC_MAIN_LOOP
...
INTEGER :: N_REQ0,SNODE,MEMBERS(0:NMESHES-1),GROUP_WORLD,NN,NOM,SUBGROUP,N_COMMUNICATIONS
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(1)
      ...
      ALLOCATE(REAL_BUFFER_QM(N_Q_DOT+N_M_DOT,NMESHES))
      ...
      COUNTS_QM = COUNTS*(N_Q_DOT+N_M_DOT)
      DISPLS_QM = DISPLS*(N_Q_DOT+N_M_DOT)
      COUNTS_10 = COUNTS*10
      DISPLS_10 = DISPLS*10
      COUNTS_20 = COUNTS*20
      DISPLS_20 = DISPLS*20
...
PRESSURE_ITERATION_LOOP: DO
   ...
   IF (MY_RANK==0 .AND. VELOCITY_ERROR_FILE .AND. .NOT.ALL(EVACUATION_ONLY)) THEN
...
IF (MY_RANK==0 .AND. .NOT.ALL(EVACUATION_ONLY)) THEN
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (EVACUATION_ONLY(NM) .OR. EVACUATION_ONLY(NOM)) CYCLE OTHER_MESH_LOOP
...
DO NOM=1,NMESHES
   SNODE = PROCESS(NOM)
   IF (CODE==6 .AND. EXCHANGE_EVACUATION .AND. MY_RANK==MAX(0,EVAC_PROCESS) .AND. .NOT.EVACUATION_ONLY(NOM)) THEN
      M4=>MESHES(NOM)
...
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      RNODE = PROCESS(NOM)
      IF (CODE>0 .AND. (EVACUATION_SKIP(NM) .OR. EVACUATION_SKIP(NOM))) CYCLE RECEIVING_MESH_LOOP
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   IF (CODE==6 .AND. EXCHANGE_EVACUATION.AND.MY_RANK/=MAX(0,EVAC_PROCESS) .AND..NOT.EVACUATION_ONLY(NM)) THEN
      M => MESHES(NM)
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (CODE>0 .AND. (EVACUATION_SKIP(NM).OR.EVACUATION_SKIP(NOM))) CYCLE SEND_MESH_LOOP
...
EVAC_ONLY: IF (ANY(EVACUATION_ONLY) .AND. MY_RANK==MAX(0,EVAC_PROCESS)) THEN
...
REAL_BUFFER_QM(N_Q_DOT+1:N_Q_DOT+N_M_DOT,1:NMESHES) = M_DOT_SUM(1:N_M_DOT,1:NMESHES)
IF (MY_RANK>0) THEN
...
IF (MY_RANK==0) THEN
   ...
   M_DOT_SUM(1:N_M_DOT,1:NMESHES) = REAL_BUFFER_QM(N_Q_DOT+1:N_Q_DOT+N_M_DOT,1:NMESHES)
ENDIF
...
IF (.NOT.ANY(EVACUATION_ONLY)) THEN
   ...
   T_EVAC_SAVE = T_BEGIN
   T_TMP       = T_BEGIN
   IF (MY_RANK==0) CALL INITIALIZE_EVAC_DUMPS(T_TMP,T_EVAC_SAVE)
   RETURN
ENDIF
...
IF (N_MPI_PROCESSES==1 .OR. (N_MPI_PROCESSES>1 .AND. MY_RANK==EVAC_PROCESS))  CALL INITIALIZE_EVAC_DUMPS(T_TMP,T_EVAC_SAVE)
...
IF (ICYC <= 0) THEN
   DO NM = 1, NMESHES
      ...
      IF (.NOT.ALL(EVACUATION_ONLY)) EVACUATION_SKIP(NM) = .TRUE.  ! Fire meshes => do just a FED write, no EFF calculation
   END DO
ENDIF
      ...
      IF (EVACUATION_ONLY(NM)) THEN
         ...
         IF (ICYC <= 1 .AND. .NOT.BTEST(I_EVAC, 2)) THEN
            IF (ICYC <= 0 .AND. EMESH_INDEX(NM)>0) THEN
               ...
               END IF
               DIAGNOSTICS = .FALSE.
               IF (.NOT.ALL(EVACUATION_ONLY)) EVACUATION_SKIP(NM) = .TRUE.
            END IF
            !
