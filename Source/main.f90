...
INTEGER :: N_REQ,N_REQ1=0,N_REQ2=0,N_REQ3=0,N_REQ4=0,N_REQ5=0,N_REQ7=0,N_REQ6=0,N_REQ14=0,N_REQ15=0,N_COMMUNICATIONS
...
INTEGER :: N_REQ0,SNODE,MEMBERS(0:NMESHES-1),GROUP_WORLD,NN,SUBGROUP
...
MESH_LOOP_2: DO NM=1,NMESHES
   ...
   SEARCH_LOOP: DO WHILE(FOUND_UNASSIGNED_CELL)
      ...
      IF (FOUND_UNASSIGNED_CELL) THEN
         ...
         N_ZONE = N_ZONE + 1
         IF (MYID==PROCESS(NM)) CALL ASSIGN_PRESSURE_ZONE(NM,M%XC(I),M%YC(J),M%ZC(K),N_ZONE,N_OVERLAP)
         CALL ZONE_BOUNDARY_EXCHANGE
      ENDIF
   ENDDO SEARCH_LOOP
ENDDO MESH_LOOP_2
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NOM=1,NMESHES
      ...
      M2 => M%OMESH(NOM)
      IF (M2%NIC_S==0 .AND. M2%NIC_R==0) CYCLE SEND_MESH_LOOP
      ...
      IF_RECEIVE_PARTICLES: IF (CODE==6 .AND. OMESH_PARTICLES) THEN
         ...
         DO IPC=1,N_LAGRANGIAN_CLASSES
            ...
            DO N=M%NLP+1,M%NLP+M2%N_PART_ADOPT(IPC)
               ...
               CALL ALLOCATE_STORAGE(NM,IBC,LPC_INDEX=IPC,LP_INDEX=N,TAG=-1)
