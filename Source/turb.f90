...
LOGICAL, PARAMETER :: DFSEM_SPLIT_DEBUG = .TRUE.
...
IF (N_MPI_PROCESSES>1) THEN
   ...
   ENDIF
   IF (.NOT.ALLOCATED(SEM_RANK_HAS_VENT)) THEN
      ALLOCATE(SEM_RANK_HAS_VENT(N_VENT_TOTAL))
...
VENT_LOOP: DO NV=1,MESHES(NM)%N_VENT
   ...
   IF (VT%DFSEM) THEN
      ...
      SIGMA_BOX = SIGMA_MAX
      IF (DFSEM_SPLIT_DEBUG) THEN
         WRITE(LU_ERR,'(A,5(I0,A),3ES11.3,A,9ES11.3)') 'DFSEM_SETUP rank=',MY_RANK,', NM=',NM,', NV=',NV,', NT=',VT%TOTAL_INDEX, &
            ', IOR=',VT%IOR,', sigma=',VT%SIGMA_DFSEM(1),VT%SIGMA_DFSEM(2),VT%SIGMA_DFSEM(3),', rot=', &
            VT%DFSEM_ROT(1,1),VT%DFSEM_ROT(1,2),VT%DFSEM_ROT(1,3),VT%DFSEM_ROT(2,1),VT%DFSEM_ROT(2,2),VT%DFSEM_ROT(2,3), &
            VT%DFSEM_ROT(3,1),VT%DFSEM_ROT(3,2),VT%DFSEM_ROT(3,3)
      ENDIF
   ELSE
...
IF (N_MPI_PROCESSES>1) THEN
   ...
   IF (.NOT.ALLOCATED(SEM_COMM_BY_TOTAL)) ALLOCATE(SEM_COMM_BY_TOTAL(N_VENT_TOTAL))
   ! Ranks without local SEM vents still participate in world collectives with neutral values.
...
INTEGER :: NV2
INTEGER :: N_OWNED,N_NONZERO_LOCAL,N_NONZERO_MERGED
TYPE(VENTS_TYPE), POINTER :: VT
...
MESH_ADVECT_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   VENT_ADVECT_LOOP: DO NV=1,N_VENT
      ...
      IF (DFSEM_SPLIT_DEBUG .AND. VT%DFSEM .AND. ICYC<=5) THEN
         WRITE(LU_ERR,'(A,5(I0,A),I0)') 'DFSEM_OWN rank=',MY_RANK,', ICYC=',ICYC,', NM=',NM,', NV=',NV,', NT=',VT%TOTAL_INDEX, &
            ', owned=',N_OWNED
      ENDIF
   ...
   VENT_COMBINE_LOOP: DO NT=1,N_VENT_TOTAL
      ...
      DO NM2=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         DO NV2=1,MESHES(NM2)%N_VENT
            ...
            IF (VT2%DFSEM) NT_HAS_DFSEM = .TRUE.
         ...
         ENDDO
      ENDDO
      IF (DFSEM_SPLIT_DEBUG .AND. NT_HAS_DFSEM .AND. ICYC<=5) THEN
         N_NONZERO_LOCAL = COUNT(ABS(EDDY_BUFFER(3*N_EDDY_TOTAL+1:4*N_EDDY_TOTAL))+&
         ABS(EDDY_BUFFER(4*N_EDDY_TOTAL+1:5*N_EDDY_TOTAL))+ &
                                ABS(EDDY_BUFFER(5*N_EDDY_TOTAL+1:6*N_EDDY_TOTAL)) > TWO_EPSILON_EB)
      ENDIF

      ! If split across ranks, combine rank-local totals on the cached communicator.
      IF (N_MPI_PROCESSES>1) THEN
         IF (SEM_COMMS_INITIALIZED) THEN
            IF (SEM_SPLIT_VENT(NT) .AND. SEM_RANK_HAS_VENT(NT)) THEN
      ...
      IF (DFSEM_SPLIT_DEBUG .AND. NT_HAS_DFSEM .AND. ICYC<=5) THEN
         N_NONZERO_MERGED = &
         COUNT(ABS(EDDY_BUFFER(3*N_EDDY_TOTAL+1:4*N_EDDY_TOTAL))+ABS(EDDY_BUFFER(4*N_EDDY_TOTAL+1:5*N_EDDY_TOTAL))+ &
                                 ABS(EDDY_BUFFER(5*N_EDDY_TOTAL+1:6*N_EDDY_TOTAL)) > TWO_EPSILON_EB)
         WRITE(LU_ERR,'(A,3(I0,A),2(I0,A),I0)') 'DFSEM_COMB rank=',MY_RANK,', ICYC=',ICYC,', NT=',NT, &
            ', local_nonzero=',N_NONZERO_LOCAL,', merged_nonzero=',N_NONZERO_MERGED,', N_EDDY=',N_EDDY_TOTAL
      ENDIF
