...
VENT_COMBINE_LOOP: DO NT=1,N_VENT_TOTAL
      ...
      DO NM2=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         ENDDO
      ENDDO
      NEED_NT_COMBINE = N_LOCAL_SEC>1
      IF (N_MPI_PROCESSES>1 .AND. SEM_COMMS_INITIALIZED) THEN
         IF (ALLOCATED(SEM_SPLIT_VENT) .AND. ALLOCATED(SEM_RANK_HAS_VENT)) THEN
            IF (SEM_SPLIT_VENT(NT) .AND. SEM_RANK_HAS_VENT(NT)) NEED_NT_COMBINE = .TRUE.
         ENDIF
      ENDIF
      IF (.NOT. NEED_NT_COMBINE .OR. N_EDDY==0) CYCLE VENT_COMBINE_LOOP
