...
VENT_LOOP: DO NV=1,MESHES(NM)%N_VENT
   ...
   EDDY_LOOP: DO NE=1,VT%N_EDDY
      IERROR=1; CALL EDDY_POSITION(NE,NV,NM,IERROR,SETUP=.TRUE.)
...
INTEGER :: NE,NV,NT,II,JJ,KK,IERROR,NM
INTEGER :: IN,IA,IC
TYPE(VENTS_TYPE), POINTER :: VT
...
! OPEN+WIND: height-mean of the global wind formula over Z1_ORIG…Z2_ORIG (no MPI).
...
MESH_ADVECT_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   VENT_ADVECT_LOOP: DO NV=1,N_VENT
      ...
      EDDY_LOOP: DO NE=1,VT%N_EDDY
         ...
         IF (IERROR==1) CALL EDDY_AMPLITUDE(NE,NV,NM,T)
      ENDDO EDDY_LOOP
...
SUBROUTINE EDDY_POSITION(NE,NV,NM,IERROR,SETUP)
...
IF (IERROR==1) THEN
   ...
   ! Setup: fill the full eddy box so the inlet starts statistically loaded.
   ! Runtime: pin the normal coordinate to the upstream face (based on signed IOR)
...
! (Eddy advection uses the vent-mean velocity separately.)
...
REAL(EB), INTENT(IN) :: T
TYPE(SURFACE_TYPE), POINTER :: SF
REAL(EB) :: XE,YE,ZE,SPEED,UW,VW,WW
...
SELECT CASE(SF%PROFILE)
   CASE(ATMOSPHERIC_PROFILE)
      IF (ZE>GROUND_LEVEL .AND. SF%Z0>TWO_EPSILON_EB) &
         SPEED = SPEED*((ZE-GROUND_LEVEL)/SF%Z0)**SF%PLE
   CASE(RAMP_PROFILE)
      SELECT CASE(ABS(VT%IOR))
         CASE(1)
            IF (SF%RAMP(VELO_PROF_Y)%INDEX>0) SPEED = SPEED*EVALUATE_RAMP(YE,SF%RAMP(VELO_PROF_Y)%INDEX,TAU=1._EB)
            IF (SF%RAMP(VELO_PROF_Z)%INDEX>0) SPEED = SPEED*EVALUATE_RAMP(ZE,SF%RAMP(VELO_PROF_Z)%INDEX,TAU=1._EB)
         CASE(2)
            IF (SF%RAMP(VELO_PROF_X)%INDEX>0) SPEED = SPEED*EVALUATE_RAMP(XE,SF%RAMP(VELO_PROF_X)%INDEX,TAU=1._EB)
            IF (SF%RAMP(VELO_PROF_Z)%INDEX>0) SPEED = SPEED*EVALUATE_RAMP(ZE,SF%RAMP(VELO_PROF_Z)%INDEX,TAU=1._EB)
         CASE(3)
            IF (SF%RAMP(VELO_PROF_X)%INDEX>0) SPEED = SPEED*EVALUATE_RAMP(XE,SF%RAMP(VELO_PROF_X)%INDEX,TAU=1._EB)
            IF (SF%RAMP(VELO_PROF_Y)%INDEX>0) SPEED = SPEED*EVALUATE_RAMP(YE,SF%RAMP(VELO_PROF_Y)%INDEX,TAU=1._EB)
...
LOCAL_VELOCITY_LOOKUP = SPEED

END FUNCTION LOCAL_VELOCITY_LOOKUP


SUBROUTINE DFSEM_VELOCITY_CONTRIB(VT,NE,XG,YG,ZG,U_ADD,V_ADD,W_ADD)
...
! Compact DFSEM kernel is defined locally here: f(r)=1-r^2 for r<1, else 0.
SHAPE_FACTOR = 1._EB - R2
UPR(1) = VT%SIGMA_DFSEM(1)*SHAPE_FACTOR*(XP(2)*VT%CW_EDDY(NE)-XP(3)*VT%CV_EDDY(NE))
UPR(2) = VT%SIGMA_DFSEM(2)*SHAPE_FACTOR*(XP(3)*VT%CU_EDDY(NE)-XP(1)*VT%CW_EDDY(NE))
UPR(3) = VT%SIGMA_DFSEM(3)*SHAPE_FACTOR*(XP(1)*VT%CV_EDDY(NE)-XP(2)*VT%CU_EDDY(NE))
...
END SUBROUTINE DFSEM_VELOCITY_CONTRIB
