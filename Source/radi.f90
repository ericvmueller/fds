...
REAL(EB) :: RAP, AX, AXU, AXD, AY, AYU, AYD, AZ, AZU, AZD, VC, RU, RD, RP, AFD, &
            ...
            FWXD,FWYD,FWZD,CELL_TOTAL,LOCAL_QP
...
REAL(EB), ALLOCATABLE, DIMENSION(:,:,:) :: QP_CORRECTION
...
IF (RAD_DIFF_SCHEME>1 .OR. INTERPOLATE_PART) THEN
         ...
         ANGLE_LOOP: DO N = NSTART,NEND,NSTEP  ! Sweep through control angles
            ...
            ELSE GEOMETRY2  ! Sweep in 3D cartesian geometry
               ...
               IPROP_LOOP: DO N_SLICE = ISTEP*ISTART + JSTEP*JSTART + KSTEP*KSTART, &
                  ...
                  SLICE_LOOP: DO IJK = 1, M_IJK
                     ...
                     ! Higher-order schemes only with particles present; STEP in cut-cells
                     APPLY_ALT_SCHEME = (RAD_DIFF_SCHEME>1 .AND. KAPPA_PART(I,J,K)>0._EB)
                     IF (CC_IBM) THEN
            ...
            WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
                WC => WALL(IW)
                   ...
                   ! Use downwind face value if relevant (cut-cells / no particles store ILD=IL)
                   IF (RAD_DIFF_SCHEME>1) THEN
                      SELECT CASE(ABS(IOR))
                        CASE(1)
            ...
            IF (INTERPOLATE_PART) THEN
               PARTICLE_INTERPOLATION_LOOP: DO IP=1,NLP
                  LP => LAGRANGIAN_PARTICLE(IP)
                  LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
                  IF (.NOT. LPC%SUBGRID_INTERPOLATION) CYCLE PARTICLE_INTERPOLATION_LOOP
                  BC => BOUNDARY_COORD(LP%BC_INDEX)
                  BR => BOUNDARY_RADIA(LP%BR_INDEX)                  
                  BR%BAND(IBND)%ILW(N) = RSA(N) * &
                     INTENSITY_INTERPOLATION(BC%IIG,BC%JJG,BC%KKG,BC%X,BC%Y,BC%Z,DLANG(:,N),IL,ILDX,ILDY,ILDZ)
...
IF (SOLID_PARTICLES .AND. UPDATE_INTENSITY) THEN
   IF (INTERPOLATE_PART) THEN
      ALLOCATE(QP_CORRECTION(1:IBAR,1:JBAR,1:KBAR))
      QP_CORRECTION = 0._EB
   ENDIF
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN
         ...
         IF (LP%ORIENTATION_INDEX>0) THEN
            ...
            ENDDO
         ELSEIF (LPC%SUBGRID_INTERPOLATION) THEN
            BR => BOUNDARY_RADIA(LP%BR_INDEX)
            B1%Q_RAD_IN = 0._EB
            DO IBND=1,NUMBER_SPECTRAL_BANDS
            ...
            ENDDO
            BC => BOUNDARY_COORD(LP%BC_INDEX)
            LOCAL_QP = 0._EB; CELL_TOTAL = 0._EB
            ! Assume cylinders for now
            CELL_TOTAL = PI*SUM(AVG_DROP_AREA(BC%IIG,BC%JJG,BC%KKG,LP_INTERP_INDICES(:)))*UII(BC%IIG,BC%JJG,BC%KKG)            
            DO IBND=1,NUMBER_SPECTRAL_BANDS
               LOCAL_QP = LOCAL_QP + WEIGH_CYL*SUM(BR%BAND(IBND)%ILW(1:NUMBER_RADIATION_ANGLES))*B1%AREA*LP%PWT*LP%RVC
            ENDDO
            QP_CORRECTION(BC%IIG,BC%JJG,BC%KKG) = &
               QP_CORRECTION(BC%IIG,BC%JJG,BC%KKG) + LOCAL_QP/(CELL_TOTAL+TWO_EPSILON_EB)
         ELSE
            BC => BOUNDARY_COORD(LP%BC_INDEX)
   ...
   IF (INTERPOLATE_PART) THEN
      PARTICLE_LOOP2: DO IP=1,NLP
         LP => LAGRANGIAN_PARTICLE(IP)
         IF (.NOT. (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET)) CYCLE PARTICLE_LOOP2
         LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
         IF (.NOT. LPC%SUBGRID_INTERPOLATION) CYCLE PARTICLE_LOOP2
         B1 => BOUNDARY_PROP1(LP%B1_INDEX)
         BC => BOUNDARY_COORD(LP%BC_INDEX)
         B1%Q_RAD_IN = B1%Q_RAD_IN / QP_CORRECTION(BC%IIG,BC%JJG,BC%KKG)
      ENDDO PARTICLE_LOOP2
      DEALLOCATE(QP_CORRECTION)
   ENDIF
ENDIF
...
REAL(EB) FUNCTION INTENSITY_INTERPOLATION(IIG,JJG,KKG,PX,PY,PZ,L_DIR,IL,ILDX,ILDY,ILDZ)

INTEGER, INTENT(IN) :: IIG,JJG,KKG
REAL(EB), INTENT(IN) :: PX,PY,PZ
REAL(EB), INTENT(IN), DIMENSION(1:3) :: L_DIR
REAL(EB), INTENT(IN), DIMENSION(0:IBP1,0:JBP1,0:KBP1) :: IL,ILDX,ILDY,ILDZ
REAL(EB) :: GX,GY,GZ, DX_CELL, DY_CELL, DZ_CELL
REAL(EB) :: DX_P, DY_P, DZ_P, MIN_IL, MAX_IL, IL_P(7)
...
IL_P(1) = IL(IIG,JJG,KKG)
IF (L_DIR(1)>0._EB) THEN
   IL_P(2) = ILDX(IIG-1,JJG,KKG)
   IL_P(3) = ILDX(IIG,JJG,KKG)
ELSE
   IL_P(2) = ILDX(IIG,JJG,KKG)
   IL_P(3) = ILDX(IIG+1,JJG,KKG)
ENDIF
IF (L_DIR(2)>0._EB) THEN
   IL_P(4) = ILDY(IIG,JJG-1,KKG)
   IL_P(5) = ILDY(IIG,JJG,KKG)
ELSE
   IL_P(4) = ILDY(IIG,JJG,KKG)
   IL_P(5) = ILDY(IIG,JJG+1,KKG)
ENDIF
IF (L_DIR(3)>0._EB) THEN
   IL_P(6) = ILDZ(IIG,JJG,KKG-1)
   IL_P(7) = ILDZ(IIG,JJG,KKG)
ELSE
   IL_P(6) = ILDZ(IIG,JJG,KKG)
   IL_P(7) = ILDZ(IIG,JJG,KKG+1)
ENDIF

GX = 0._EB; GY = 0._EB; GZ = 0._EB
IF (DX_CELL > TWO_EPSILON_EB) GX = (IL_P(3) - IL_P(2)) / DX_CELL
IF (DY_CELL > TWO_EPSILON_EB) GY = (IL_P(5) - IL_P(4)) / DY_CELL
IF (DZ_CELL > TWO_EPSILON_EB) GZ = (IL_P(7) - IL_P(6)) / DZ_CELL

DX_P = PX - XC(IIG)
DY_P = PY - YC(JJG)
DZ_P = PZ - ZC(KKG)

INTENSITY_INTERPOLATION = IL_P(1) + GX*DX_P + GY*DY_P + GZ*DZ_P
