...
REAL(EB) :: UBAR,VBAR,WBAR,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_FILM, &
            ...
            GX_LOC,GY_LOC,GZ_LOC,K_SGS,U_P,KN,M_DOT,&
...
REAL(EB) :: RDT,UODT,VODT,WODT,STABILITY_X,STABILITY_Y,STABILITY_Z,DRAG_STABILITY_MAX_BC
INTEGER :: I,J,K,IW
TYPE(WALL_TYPE), POINTER :: WC
...
! Save carried boundary-fuel contribution, then rebuild fresh particle contribution
DRAG_STABILITY_MAX_BC = DRAG_STABILITY_MAX
...
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
         ...
         STABILITY_X = SQRT(ABS(FVX_D(I,J,K))/MAX(DXN(I),TWO_EPSILON_EB))
         STABILITY_Y = SQRT(ABS(FVY_D(I,J,K))/MAX(DYN(J),TWO_EPSILON_EB))
         STABILITY_Z = SQRT(ABS(FVZ_D(I,J,K))/MAX(DZN(K),TWO_EPSILON_EB))
         IF (I==0) THEN 
            IW = CELL(CELL_INDEX(I+1,J,K))%WALL_INDEX(-IAXIS)
         ELSE  
            IW = CELL(CELL_INDEX(I,J,K))%WALL_INDEX(IAXIS)
         ENDIF
         IF (IW>0) THEN
            WC => WALL(IW)
            IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) STABILITY_X = 0._EB
         ENDIF
         IF (J==0) THEN
            IW = CELL(CELL_INDEX(I,J+1,K))%WALL_INDEX(-JAXIS)
         ELSE
            IW = CELL(CELL_INDEX(I,J,K))%WALL_INDEX(JAXIS)
         ENDIF
         IF (IW>0) THEN
            WC => WALL(IW)
            IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) STABILITY_Y = 0._EB
         ENDIF
         IF (K==0) THEN
            IW = CELL(CELL_INDEX(I,J,K+1))%WALL_INDEX(-KAXIS)
         ELSE
            IW = CELL(CELL_INDEX(I,J,K))%WALL_INDEX(KAXIS)
         ENDIF
         IF (IW>0) THEN
            WC => WALL(IW)
            IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) STABILITY_Z = 0._EB
         ENDIF
         DRAG_STABILITY_MAX = MAX(DRAG_STABILITY_MAX,MAX(STABILITY_X,STABILITY_Y,STABILITY_Z))
         ...
         FVX(I,J,K) = FVX(I,J,K) + FVX_D(I,J,K)
         FVY(I,J,K) = FVY(I,J,K) + FVY_D(I,J,K)
         FVZ(I,J,K) = FVZ(I,J,K) + FVZ_D(I,J,K)
      ENDDO
   ENDDO
ENDDO

DRAG_STABILITY_MAX = MAX(DRAG_STABILITY_MAX,DRAG_STABILITY_MAX_BC)
