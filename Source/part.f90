...
! Generate size distribution statistics for all particles classes
...
WALL_INSERT_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT. WC%ONE_D%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)>0) CYCLE WALL_INSERT_LOOP
   ...
   GPR=DX(II)*DY(JJ)*WC%ONE_D%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)*DT*100
   SF%NPPC=FLOOR(GPR)
   CALL RANDOM_NUMBER(RN)
...
! Place holder for inserting particles for duct heat transfer
...
IF (LPC%SOLID_PARTICLE) THEN
   ...
   !disk radius for firebrand particles
   IF(FBRAND) THEN
      CALL RANDOM_NUMBER(RN)
...
REAL(EB) :: UBAR,VBAR,WBAR,RVC,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_AIR, &
            ...
            GX_LOC,GY_LOC,GZ_LOC,DUMMY,DRAG_MAX(3),D_HY
