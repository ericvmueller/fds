...
ELSE PARTICLE_NON_STATIC_IF ! Drag calculation for stationary, airborne particles
   ...
   IF (TWO_D) THEN
      ...
      DRAG_MAX(2) = 0._EB
   ENDIF
   ! IF (ANY(ABS(DRAG_MAX)>PART_UVWMAX)) WRITE(LU_ERR,*) IIG_OLD,JJG_OLD,KKG_OLD,MAXVAL(DRAG_MAX),BETA,QREL
...
DO AXIS=IAXIS,KAXIS
   ...
   ! IF (IIG_OLD==20 .AND. JJG_OLD==20 .AND. KKG_OLD==1 .AND. AXIS==IAXIS) WRITE(LU_ERR,*) IL,IIX,FLOOR(XI)
   ! IF (IIG_OLD==20 .AND. JJG_OLD==20 .AND. KKG_OLD==1 .AND. AXIS==IAXIS) &
   !    WRITE(LU_ERR,*) FV_D(IIG_OLD-1:IIG_OLD, JJG_OLD, KKG_OLD),UBAR,QREL
ENDDO
...
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
         ...
         ! IF (I==21 .AND. J==20 .AND. K==1) WRITE(LU_ERR,*) 'x',FVX_D(I,J,K),UODT,MIN(UODT,MAX(-UODT,FVX_D(I,J,K)))
         ! IF (I==20 .AND. J==20 .AND. K==1) WRITE(LU_ERR,*) 'y',FVY_D(I,J,K),VODT,MIN(VODT,MAX(-VODT,FVY_D(I,J,K)))
         ! IF (I==20 .AND. J==20 .AND. K==1) WRITE(LU_ERR,*) 'z',FVZ_D(I,J,K),WODT,MIN(WODT,MAX(-WODT,FVZ_D(I,J,K)))
      ENDDO
   ENDDO
ENDDO
