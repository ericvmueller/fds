...
REAL(EB) :: DELKDELT,VC,VC1,DTDX,DTDY,DTDZ,TNOW,DZDX,DZDY,DZDZ,TMP_G,DIV_DIFF_HEAT_FLUX,H_S,XHAT,ZHAT,TT,Q_Z,&
            D_Z_TEMP,D_Z_N(0:I_MAX_TEMP),RHO_D_DZDN_GET(1:N_TRACKED_SPECIES),UN_P,TMP_F_GAS,R_PFCT,RHO_D_DZDN,STRAT_FAC
...
IF (STRATIFICATION) THEN
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            DP(I,J,K) = DP(I,J,K) + STRAT_FAC*RTRM(I,J,K)*0.5_EB*(WW(I,J,K)+WW(I,J,K-1))*RHO_0(K)*GVEC(3)
         ENDDO
      ENDDO
   ENDDO
   !$OMP END PARALLEL DO
...
SPECIES_LOOP: DO N=1,N_TOTAL_SCALARS
   ...
   CALL GET_SCALAR_FACE_VALUE(UU,RHO_Z_P,FX_P,0,IBAR,1,JBAR,1,KBAR,1,I_FLUX_LIMITER)
   CALL GET_SCALAR_FACE_VALUE(VV,RHO_Z_P,FY_P,1,IBAR,0,JBAR,1,KBAR,2,I_FLUX_LIMITER)
   CALL GET_SCALAR_FACE_VALUE(WW,RHO_Z_P,FZ_P,1,IBAR,1,JBAR,0,KBAR,3,I_FLUX_LIMITER)

   !$OMP PARALLEL PRIVATE(IW,WC,BC,B1,U_TEMP,Z_TEMP,F_TEMP,U_WORK,Z_WORK,F_WORK)
   U_TEMP => U_WORK
   F_TEMP => F_WORK
   Z_TEMP => Z_WORK
   !$OMP DO
...
FACE_CORRECTION_IF: IF (FLUX_LIMITER_MW_CORRECTION) THEN
   ...
   CALL GET_SCALAR_FACE_VALUE(UU,RHO_RMW,FX_P,0,IBAR,1,JBAR,1,KBAR,1,I_FLUX_LIMITER)
   CALL GET_SCALAR_FACE_VALUE(VV,RHO_RMW,FY_P,1,IBAR,0,JBAR,1,KBAR,2,I_FLUX_LIMITER)
   CALL GET_SCALAR_FACE_VALUE(WW,RHO_RMW,FZ_P,1,IBAR,1,JBAR,0,KBAR,3,I_FLUX_LIMITER)

   !$OMP PARALLEL PRIVATE(IW,WC,BC,B1,U_TEMP,Z_TEMP,F_TEMP,U_WORK,Z_WORK,F_WORK,ZZ_GET)
   U_TEMP => U_WORK
   F_TEMP => F_WORK
   Z_TEMP => Z_WORK
   !$OMP DO
