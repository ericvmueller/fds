...
COUNT_MESH_LOOP: DO
   ...
   ENDIF
   IF (.NOT.DO_EVACUATION .AND. EVACUATION) CYCLE COUNT_MESH_LOOP ! skip evacuation meshes
   IF (DO_EVACUATION .AND. .NOT.EVACUATION) CYCLE COUNT_MESH_LOOP ! skip fire meshes
...
IF (DO_EVACUATION) NMESHES = NMESHES + N_DOOR_MESHES + NEVAC_MESHES
IF (DO_EVACUATION) NMESHES = NMESHES + N_STRS
...
MESH_LOOP: DO N=1,NMESHES_READ
   ...
   IF (.NOT.DO_EVACUATION .AND. EVACUATION) CYCLE MESH_LOOP ! skip evacuation meshes
   IF (DO_EVACUATION .AND. .NOT.EVACUATION) CYCLE MESH_LOOP ! skip fire meshes
...
DO I = 1, NM
   EV_IF: IF (EVACUATION_SKIP(I) .AND. EVACUATION_ONLY(I)) THEN
      ...
      IF (EMESH_NFIELDS(NN)==0) THEN
         WRITE(MESSAGE,'(A,I0,3A)') 'ERROR: FDS+Evac: Emesh ',NN,' ',TRIM(EMESH_ID(NN)),' needs at least one DOOR/EXIT.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ELSE
         WRITE(LU_ERR,FMT='(A,I0,3A,I0,A)') ' FDS+Evac: Emesh ',NN,' ',TRIM(EMESH_ID(NN)),' has ',&
...
T_END                = 1._EB
T_END_GEOM           = T_END
IF (DO_EVACUATION) DT = EVAC_DT_STEADY_STATE
...
LOGICAL :: DNS,RESEARCH_MODE,NO_EVACUATION,EVACUATION_ALL_STEPS
CHARACTER(LABEL_LENGTH) :: ASSUMED_GAS_TEMPERATURE_RAMP,RAMP_GX,RAMP_GY,RAMP_GZ,TURBULENCE_MODEL,&
                           NEAR_WALL_TURBULENCE_MODEL,SIMULATION_MODE,FLUX_LIMITER,LES_FILTER_TYPE
CHARACTER(CHID_LENGTH)  :: CHID_EVAC
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE,ALLOW_SURFACE_PARTICLES, &
                ...
                BNDF_DEFAULT,CC_IBM,CC_SLIPIBM_VELO,CC_STRESS_METHOD,CCVOL_LINK,CC_ZEROIBM_VELO,C_DEARDORFF,&
                ...
                EVACUATION_WRITE_FED,EVACUATION_INITIALIZATION, EVACUATION_ALL_STEPS, &
                EVAP_MODEL,&
                ...
                HUMIDITY,HVAC_LOCAL_PRESSURE,HVAC_MASS_TRANSPORT,HVAC_PRES_RELAX,IBLANK_SMV,I_MAX_TEMP,&
                ...
                P_INF,QFAN_BETA_TEST,RAMP_GX,RAMP_GY,RAMP_GZ,RESTART,RESTART_CHID,&
                ...
                BAROCLINIC,DNS,RADIATION,RESEARCH_MODE,STRATIFICATION,SUPPRESSION,SURF_DEFAULT
...
!
IF (NO_EVACUATION .OR. .NOT.(EVACUATION_DRILL.OR.EVACUATION_MC_MODE.OR.EVACUATION_WRITE_FED.OR.&
     EVACUATION_INITIALIZATION.OR.EVACUATION_ALL_STEPS)) THEN
   ! If no EVACUATION keywords on the MISC namelist (all are false by default) => Do fire calculation
   ! NO_EVACAUTION parameter on the MISC namelist overrides all other EVACUATION parameters
   EVACUATION_DRILL          = .FALSE.
   EVACUATION_MC_MODE        = .FALSE.
   EVACUATION_WRITE_FED      = .FALSE.
   EVACUATION_INITIALIZATION = .FALSE.
...
IF (EVACUATION_ALL_STEPS) THEN
   ! Check the files that exists in the curren directory to figure out the step of the 3 step fire+evacuation calc.
   ! Inquire if CHID_evac.xyz, CHID_evac.eff, CHID_evac.fed exist or not
   IF (LEN_TRIM(CHID) > CHID_LENGTH-5) THEN
      WRITE(MESSAGE,'(A,I0,A)')  'ERROR: CHID TOO LONG FOR EVACUATION, CHID SHOULD BE SHORTER THAN ', &
   ...
   IF (EX) THEN
      IF (EVACUATION_DRILL) THEN
         EVACUATION_MC_MODE        = .TRUE.   ! fire drill, read EFF
         EVACUATION_WRITE_FED      = .FALSE.
         EVACUATION_INITIALIZATION = .FALSE.
         WRITE(MESSAGE,'(A)') ' FDS+Evac: EVACUATION DRILL IN MC MODE.'
         IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
      ELSE
         INQUIRE(FILE=TRIM(TRIM(CHID_EVAC)//'.fed'),EXIST=EX)
         IF (EX) THEN
            EVACUATION_MC_MODE        = .TRUE. ! fire+evacuation, read EFF and FED
            EVACUATION_WRITE_FED      = .FALSE.
            EVACUATION_INITIALIZATION = .FALSE.
            WRITE(MESSAGE,'(A)') ' FDS+Evac: FIRE+EVACUATION IN MC MODE.'
            IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
         ELSE
            INQUIRE(FILE=TRIM(TRIM(CHID_EVAC)//'.xyz'),EXIST=EX)
            IF (EX) THEN
               EVACUATION_MC_MODE        = .FALSE.
            ...
            ELSE
               EVACUATION_MC_MODE        = .FALSE.
               EVACUATION_WRITE_FED      = .FALSE.
               EVACUATION_INITIALIZATION = .TRUE. ! initialization (get EFF and xyz files)
               WRITE(MESSAGE,'(A)') ' FDS+Evac: EVACUATION INITIALIZATION.'
               IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
            ENDIF
         ENDIF
      ENDIF
   ELSE ! Step 1 needed (EFF file needed)
      EVACUATION_MC_MODE        = .FALSE.
      EVACUATION_WRITE_FED      = .FALSE.
      EVACUATION_INITIALIZATION = .TRUE. ! Write EFF (and xyz) files
      IF (EVACUATION_DRILL) THEN
         WRITE(MESSAGE,'(A)') ' FDS+Evac: EVACUATION INITIALIZATION (DRILL).'
      ELSE
         WRITE(MESSAGE,'(A)') ' FDS+Evac: EVACUATION INITIALIZATION.'
      ENDIF
      IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
...
IF (EVACUATION_INITIALIZATION) THEN ! No fire meshes, do not use FED, write FEDxyz file
   ...
   ! This parameter forces an evacuation initialization calculation (if NO_EVACUATION is not given)
   DO_EVACUATION        = .TRUE.
   EVACUATION_WRITE_FED = .FALSE.
   EVACUATION_MC_MODE   = .FALSE.
...
IF (EVACUATION_WRITE_FED) THEN ! No evacuation meshes, read FEDxyz file and write FED file
   IF (EVACUATION_MC_MODE) THEN
      WRITE(MESSAGE,'(A)')  'ERROR on MISC: Do not set true both EVACUATION_WRITE_FED and EVACUATION_MC_MODE'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   DO_EVACUATION             = .FALSE.  ! no evacuation meshes in a fire calculation
   EVACUATION_DRILL          = .FALSE.
   EVACUATION_MC_MODE        = .FALSE.
   EVACUATION_INITIALIZATION = .FALSE.
ENDIF

IF (EVACUATION_MC_MODE) THEN ! Read FED and EFF (if not a drill) files
   IF (EVACUATION_WRITE_FED) THEN
      WRITE(MESSAGE,'(A)')  'ERROR on MISC: Do not set true both EVACUATION_MC_MODE and EVACUATION_WRITE_FED'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   EVACUATION_WRITE_FED      = .FALSE.
   DO_EVACUATION             = .TRUE.
...
REAL(EB) :: CORIOLIS_VECTOR(3)=0._EB,FORCE_VECTOR(3)=0._EB,L,ZZZ,ZETA,Z_0,SPEED,DIRECTION,&
            ...
            LCC,DT_THETA,TAU_THETA,SIGMA_THETA
INTEGER :: N
...
NAMELIST /WIND/ CORIOLIS_VECTOR,DIRECTION,FORCE_VECTOR,FYI,GEOSTROPHIC_WIND,GROUND_LEVEL,L,LAPSE_RATE,LATITUDE,&
                POTENTIAL_TEMPERATURE_CORRECTION,RAMP_DIRECTION,RAMP_FVX_T,RAMP_FVY_T,RAMP_FVZ_T,RAMP_SPEED,&
                RAMP_TMP0_Z,RAMP_U0_T,RAMP_U0_Z,RAMP_V0_T,RAMP_V0_Z,RAMP_W0_T,RAMP_W0_Z,SIGMA_THETA,&
                SPEED,STRATIFICATION,TAU_THETA,THETA_STAR,TMP_REF,U_STAR,U0,USE_ATMOSPHERIC_INTERPOLATION,&
                V0,W0,Z_0,Z_REF

IF(DO_EVACUATION) RETURN ! No wind input for evacuation calculation
...
IF (Z_0<0._EB) THEN
   ! Z_0 has not been specified on the WIND line, therefore, pick up Z_0 specified on first SURF line using ABL model.
   DO N=0,N_SURF
      SF=>SURFACE(N)
...
ELSE
   ! Z_0 has been specified on WIND line. Make sure this does not conflict with any SURF lines with ABL model.
   DO N=0,N_SURF
      SF=>SURFACE(N)
...
IF (SIGMA_THETA>0._EB) THEN
   ...
   RAMP_DIRECTION = 'RSRVD FLUCTUATING WIND DIRECTION'
   RRP%ID = RAMP_DIRECTION
ENDIF
...
IF (I_RAMP_DIRECTION/=0 .OR. I_RAMP_SPEED/=0) THEN
   ...
   DO I=0,N_PTS
      IF (I_RAMP_DIRECTION/=0) THEN
         THETA = RAMPS(I_RAMP_DIRECTION)%INTERPOLATED_DATA(I)*DEG2RAD
         SIN_THETA = -SIN(THETA)
         COS_THETA = -COS(THETA)
      ELSE
         SIN_THETA = 1._EB
...
NAMELIST /DUMP/ BNDF_TIMER,CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,&
                DEVC_COLUMN_LIMIT,DT_CPU,DT_CTRL,DT_BNDF,DT_DEVC,DT_DEVC_LINE,DT_FLUSH,&
                DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RESTART,DT_RADF,DT_SL3D,DT_SLCF,DT_SMOKE3D,EB_PART_FILE,&
                ...
                SMOKE3D,SMOKE3D_QUANTITY,SMOKE3D_SPEC_ID,SLCF_TIMER,SL3D_TIMER, &
                STATUS_FILES,STORE_SPECIES_FLUX,SUPPRESS_DIAGNOSTICS,&
                T_DEVC_LINE_END,T_RADF_BEGIN,T_RADF_END,UVW_TIMER,VELOCITY_ERROR_FILE,WRITE_XYZ,REMOVE_FIREBRANDS
...
IF (DT_SMOKE3D < 0._EB) THEN ; DT_SMOKE3D = DT_DEFAULT       ; ELSE ; DT_SMOKE3D = DT_SMOKE3D /TIME_SHRINK_FACTOR ; ENDIF
...
IF (ANY(SLCF_TIMER<1.E10_EB)) DT_SLCF = 1.E10_EB
IF (ANY(BNDF_TIMER<1.E10_EB)) DT_BNDF = 1.E10_EB
IF (ANY(SL3D_TIMER<1.E10_EB)) DT_SL3D = 1.E10_EB
...
INTEGER  :: N_SPEC_READ,N,NN,NNN,NS2,NR,N_SPEC_READ_2,N_SUB_SPECIES,NS,N_BINS,N_COPY,N_FOUND,SPECIES_ARRAY_DIMENSION,Y_S,Z_S
...
IF (SIMPLE_CHEMISTRY) THEN
   SPECIES_ARRAY_DIMENSION = 10
ELSE
   SPECIES_ARRAY_DIMENSION = 5
ENDIF
REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
N_SPECIES=0
...
COUNT_SPEC_LINES: DO
   ...
   SPECIES_ARRAY_DIMENSION=SPECIES_ARRAY_DIMENSION+1
20 IF (IOS>0) THEN
...
ALLOCATE(PREDEFINED(SPECIES_ARRAY_DIMENSION))
ALLOCATE(PREDEFINED_SMIX(1:SPECIES_ARRAY_DIMENSION))
ALLOCATE(NEW_PRIMITIVE(0:SPECIES_ARRAY_DIMENSION))
NEW_PRIMITIVE=.FALSE.
ALLOCATE(PREDEFINED_SPEC_ID(SPECIES_ARRAY_DIMENSION))
ALLOCATE(SPEC_ID_READ(SPECIES_ARRAY_DIMENSION))
...
DO N=1,N_SPECIES
   ...
   IF(SS%RAMP_CP_INDEX > 0) THEN
      IF (SS%H_F > -1.E20_EB) THEN
         CP2 = EVALUATE_RAMP(1._EB,SS%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
         H2 = 0._EB
         DO J=1,INT(H_F_REFERENCE_TEMPERATURE)+1
            H1 = H2
            CP1 = CP2
            CP2 = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
   ...
   IF(SM%RAMP_CP_INDEX > 0) THEN
      IF (SM%H_F > -1.E20_EB) THEN
         ...
         DO J=1,INT(H_F_REFERENCE_TEMPERATURE)+1
            H1 = H2
            CP1 = CP2
            CP2 = EVALUATE_RAMP(REAL(J,EB),SM%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
            H2 = H2 + 0.5_EB*(CP1+CP2)
         ENDDO
...
TABLE_LOOP: DO J=1,I_MAX_TEMP
   ...
   DO N=1,N_SPECIES
      ...
      IF (SS%RAMP_CP_INDEX>0) THEN
         CP_TMP(N) = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_CP_INDEX)*1000._EB
         H_TMP(N) = SS%REFERENCE_ENTHALPY
      ENDIF
      IF (SS%RAMP_D_INDEX>0)   D_TMP(N)   = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_D_INDEX,TAU=1._EB)
      IF (SS%RAMP_G_F_INDEX>0) G_F_TMP(N) = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_G_F_INDEX,TAU=1._EB)
      IF (SS%RAMP_K_INDEX>0)   K_TMP(N)   = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_K_INDEX,TAU=1._EB)/SQRT(SS%MW)
      IF (SS%RAMP_MU_INDEX>0)  MU_TMP(N)  = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_MU_INDEX,TAU=1._EB)/SQRT(SS%MW)
   ENDDO
   ...
   DO N=1,N_TRACKED_SPECIES
      ...
      IF (SM%RAMP_CP_INDEX>0) THEN
         CP_TMP_Z(N) = EVALUATE_RAMP(REAL(J,EB),SM%RAMP_CP_INDEX)*1000._EB
         H_TMP_Z(N) = SM%REFERENCE_ENTHALPY
      ENDIF
      IF (SM%RAMP_D_INDEX>0)   D_TMP_Z(N)   = EVALUATE_RAMP(REAL(J,EB),SM%RAMP_D_INDEX,TAU=1._EB)
      IF (SM%RAMP_G_F_INDEX>0) G_F_TMP_Z(N) = EVALUATE_RAMP(REAL(J,EB),SM%RAMP_G_F_INDEX,TAU=1._EB)
      IF (SM%RAMP_K_INDEX>0)   K_TMP_Z(N)   = EVALUATE_RAMP(REAL(J,EB),SM%RAMP_K_INDEX,TAU=1._EB)*RSQ_MW_Z(N)
      IF (SM%RAMP_MU_INDEX>0)  MU_TMP_Z(N)  = EVALUATE_RAMP(REAL(J,EB),SM%RAMP_MU_INDEX,TAU=1._EB)*RSQ_MW_Z(N)
   ENDDO
...
SPEC_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   IZERO = 0
   IF (.NOT.ALLOCATED(SS%C_P_L)) ALLOCATE(SS%C_P_L(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('PROC_SPEC','SS%C_P_L',IZERO)
   SS%C_P_L=SS%SPECIFIC_HEAT_LIQUID
   IF (.NOT.ALLOCATED(SS%C_P_L_BAR)) ALLOCATE(SS%C_P_L_BAR(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('PROC_SPEC','SS%C_P_L_BAR',IZERO)
   IF (.NOT.ALLOCATED(SS%H_L)) ALLOCATE(SS%H_L(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('PROC_SPEC','SS%H_L',IZERO)
   IF (.NOT.ALLOCATED(SS%H_V)) ALLOCATE(SS%H_V(0:I_MAX_TEMP),STAT=IZERO)
   CALL ChkMemErr('PROC_SPEC','SS%H_V',IZERO)

   TMP_REF = -1._EB
   TMP_MELT = -1._EB
   TMP_V = -1._EB
   TMP_LOOP: DO J = 1, I_MAX_TEMP
      ...
      ELSE
         CALL JANAF_TABLE_LIQUID (J,SS%C_P_L(J),H_V,H_L,TMP_REF,TMP_MELT,TMP_V,SS%ID,FUEL,DENSITY,&
                                    MU_LIQUID,K_LIQUID,BETA_LIQUID)
         IF (SS%RAMP_CP_L_INDEX>0) SS%C_P_L(J) = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_CP_L_INDEX,TAU=1._EB)*1000._EB
         IF (J==1) THEN
            IF (SS%H_V_REFERENCE_TEMPERATURE < 0._EB) SS%H_V_REFERENCE_TEMPERATURE=TMP_REF
            IF (SS%TMP_V < 0._EB) SS%TMP_V = TMP_V
            IF (SS%TMP_V < 0._EB) THEN
...
NAMELIST /COMB/ AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,CHECK_REALIZABILITY,EXTINCTION_MODEL,&
                FIXED_MIX_TIME,FREE_BURN_TEMPERATURE,FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,&
...
USE RADCONS, ONLY : MIE_NDG
INTEGER :: SAMPLING_FACTOR,N,NN,ILPC,IPC,RGB(3),N_STRATA,N_LAGRANGIAN_CLASSES_READ,ADHERE_TO_SOLID
...
LOGICAL :: TARGET_ONLY,MASSLESS,STATIC,MONODISPERSE,BREAKUP,CHECK_DISTRIBUTION,&
           TURBULENT_DISPERSION,EMBER_PARTICLE
...
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
            PEAK_REACTION_RATE,REFRACTIVE_INDEX,DIFFUSIVITY_SPEC(MAX_SPECIES),POROSITY,PERMEABILITY,MW
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ALLOW_SHRINKING,ALLOW_SWELLING,BETA_CHAR,BOILING_TEMPERATURE,&
                ...
                MW,N_O2,N_REACTIONS,N_S,N_T,NU_MATL,NU_O2_CHAR,NU_PART,NU_SPEC,PART_ID,PCR,PERMEABILITY,&
                POROSITY,PYROLYSIS_RANGE,REFERENCE_RATE,REFERENCE_TEMPERATURE,REFRACTIVE_INDEX,RGB,&
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   IF (ANY(ML%RAMP_H_R/='null')) THEN
      DO NNN=1,MAX_REACTIONS
         IF (ML%RAMP_H_R(NNN)/='null') CALL GET_RAMP_INDEX(ML%RAMP_H_R(NNN),'TEMPERATURE',ML%I_RAMP_H_R(NNN))
      ENDDO
   ENDIF
...
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
INTEGER :: I,N,NLPC,NLPC2,NS,NS2,NR,NR2,Z_INDEX(N_TRACKED_SPECIES,MAX_REACTIONS)
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   ! If RAMPs are present, populate arrays for C_S, K_S, and H_R
   ...
   DO NR=1,ML%N_REACTIONS
      IF (ML%I_RAMP_H_R(NR)>0) THEN
         IF (.NOT.RAMPS(ML%I_RAMP_H_R(NR))%DEP_VAR_UNITS_CONVERTED) THEN
            RAMPS(ML%I_RAMP_H_R(NR))%INTERPOLATED_DATA(:) = RAMPS(ML%I_RAMP_H_R(NR))%INTERPOLATED_DATA(:)*1000._EB
            RAMPS(ML%I_RAMP_H_R(NR))%DEP_VAR_UNITS_CONVERTED = .TRUE.
         ENDIF
         DO I=0,I_MAX_TEMP
            ML%H_R(NR,I)=EVALUATE_RAMP(REAL(I,EB),ML%I_RAMP_H_R(NR))
         ENDDO
      ENDIF
   ENDDO
   ...
   ! Define the Heat of Gasification for a liquid

   IF (ML%TMP_BOIL < 50000._EB) &
      ML%HEAT_OF_GASIFICATION = ML%H(MIN(I_MAX_TEMP,NINT(ML%TMP_BOIL))) - ML%H(NINT(TMPA)) + ML%H_R(1,NINT(ML%TMP_BOIL))
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            TAU_EF,E_COEFFICIENT,VOLUME_FLOW,VOLUME_FLUX,&
            ...
            Z_0,PARTICLE_EXTRACTION_VELOCITY,RENODE_DELTA_T(MAX_LAYERS)
...
REAL(EB) :: VEG_LSET_IGNITE_TIME,VEG_LSET_QCON,VEG_LSET_ROS_HEAD,VEG_LSET_ROS_FLANK,VEG_LSET_ROS_BACK, &
            ...
            VEG_LSET_CHAR_FRACTION,VEL_PART
...
! Firebrand generating variables
REAL(EB) :: FIREBRAND_GENERATION_HEIGHT,FIREBRAND_GENERATION_RATE


NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BLOWING,BLOWING_2,BURN_AWAY,BURN_DURATION,&
                CELL_SIZE_FACTOR,&
                ...
                FIREBRAND_GENERATION_HEIGHT,FIREBRAND_GENERATION_RATE,&
                FSK_A,FSK_K,FSK_W,&
                ...
                RAMP_PART,RAMP_Q,RAMP_T,RAMP_T_B,RAMP_T_I,RAMP_V,RAMP_V_X,RAMP_V_Y,RAMP_V_Z,&
                ...
                VOLUME_FLUX ! Backwards compatability??
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   ! Firebrand generation variables

   SF%FIREBRAND_GENERATION_HEIGHT = FIREBRAND_GENERATION_HEIGHT
   SF%FIREBRAND_GENERATION_RATE   = FIREBRAND_GENERATION_RATE
   ...
   SF%TAU(TIME_EFLUX)      = TAU_EF/TIME_SHRINK_FACTOR
   ...
   IF (SF%CONE_HEAT_FLUX > 0._EB) THEN
      IF (SF%TMP_IGN>=50000._EB .OR. SF%RAMP_Q=='null' .OR. SF%HRRPUA <=0._EB) THEN
         WRITE (MESSAGE,'(A,A,A)') 'ERROR: Problem with SURF: ',TRIM(SF%ID),&
...
FIREBRAND_GENERATION_HEIGHT = 1.0_EB
FIREBRAND_GENERATION_RATE   = -1.0_EB
...
INTEGER :: ILPC,N,NN,NNN,NL,NLPC,I_CONE_RAMP=0, NR,N_LIST,N_LIST2,INDEX_LIST(MAX_LPC**2)
REAL(EB) :: R_L(0:MAX_LAYERS),VOLSUM,VOLFRAC
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   BURNING_IF: IF (BURNING .AND. .NOT. DO_EVACUATION) THEN
      RN => REACTION(1)
...
REAL(EB) :: K_S_0,C_S_0,SWELL_RATIO,DENSITY_MAX,DENSITY_MIN
...
SURF_GRID_LOOP: DO SURF_INDEX=0,N_SURF
   ...
   LAYER_LOOP: DO NL=1,SF%N_LAYERS
      ...
      CALL GET_N_LAYER_CELLS(SF%MIN_DIFFUSIVITY(NL),SWELL_RATIO*SF%LAYER_THICKNESS(NL),SF%STRETCH_FACTOR(NL), &
                             SF%CELL_SIZE_FACTOR,SF%N_LAYER_CELLS_MAX(NL),SF%N_LAYER_CELLS(NL),SF%SMALLEST_CELL_SIZE(NL),&
                             SF%DDSUM(NL))
...
USE SCRC, ONLY: SCARC_METHOD, SCARC_GRID, SCARC_MATRIX, SCARC_ERROR_FILE, SCARC_ACCURACY, SCARC_COARSENING, &
                SCARC_KRYLOV_ITERATIONS, SCARC_KRYLOV_ACCURACY, SCARC_KRYLOV_INTERPOL, SCARC_MGM_BC, SCARC_MGM_INTERPOLATION, &
                SCARC_MGM_ITERATIONS, SCARC_MGM_ACCURACY, SCARC_MGM_EXACT_INITIAL, SCARC_MGM_LAPLACE_SOLVER, SCARC_MGM_USE_FFT, &
                SCARC_MGM_CHECK_LAPLACE, SCARC_MULTIGRID, SCARC_MULTIGRID_ITERATIONS, SCARC_MULTIGRID_ACCURACY, SCARC_TWOLEVEL, &
                SCARC_VERBOSE, SCARC_MULTIGRID_CYCLE, SCARC_MULTIGRID_LEVEL, SCARC_MULTIGRID_INTERPOL, SCARC_MULTIGRID_RELAXING, &
...
NAMELIST /PRES/ BAROCLINIC,CHECK_POISSON,FISHPAK_BC,ITERATION_SUSPEND_FACTOR,INSEPARABLE_POISSON, &
                ...
                SOLVER,SUSPEND_PRESSURE_ITERATIONS,TUNNEL_PRECONDITIONER,VELOCITY_TOLERANCE
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               ENDIF
               IF (DO_EVACUATION) OB%SHOW_BNDF(:) = .FALSE.
...
CHARACTER(LABEL_LENGTH) :: MESH_ID
CHARACTER(25) :: COLOR
LOGICAL :: EVACUATION_HOLE,EVACUATION
...
READ_HOLE_LOOP: DO N=1,N_HOLE_O
   ...
   EVACUATION_HOLE = .FALSE.
   IF (DO_EVACUATION) CALL DEFINE_EVACUATION_HOLES(3)
...
IF(DO_EVACUATION) DEALLOCATE(TEMP_HOLE_EVAC)
...
CHARACTER(LABEL_LENGTH) :: ID,CTRL_ID,DEVC_ID,PART_ID,SHAPE,MULT_ID,SPEC_ID(1:MAX_SPECIES),PATH_RAMP(3),RAMP_Q,RAMP_PART
...
INIT_LOOP: DO N=1,N_INIT_READ+N_INIT_RESERVED
   ...
   IF (ANY(XYZ>-100000._EB)) THEN
      ...
      MESH_LOOP: DO NM=1,NMESHES
         IF (DO_EVACUATION) CYCLE MESH_LOOP
         M=>MESHES(NM)
...
INTEGER  :: N,NN,NM,MESH_NUMBER=0,N_DEVC_READ,IOR,TRIP_DIRECTION,VELO_INDEX,POINTS,I_POINT,PIPE_INDEX,ORIENTATION_INDEX, &
            ORIENTATION_NUMBER,N_INTERVALS,MOVE_INDEX,IIG,JJG,KKG
...
NAMELIST /DEVC/ ABSOLUTE_VALUE,BYPASS_FLOWRATE,CONVERSION_ADDEND,CONVERSION_FACTOR,COORD_FACTOR,CELL_L,CTRL_ID,DB,DELAY,DEPTH,&
                DEVC_ID,D_ID,DRY,DUCT_ID,DX,DY,DZ,EVACUATION,FLOWRATE,FORCE_DIRECTION,FYI,HIDE_COORDINATES,ID,&
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (BAD) THEN
         IF (CTRL_ID/='null' .OR. DUCT_ID/='null' .OR. NODE_ID(1)/='null' .OR. INIT_ID/='null' .OR. CELL_L>-1._EB) THEN
            ...
            ENDIF
         ELSE
            IF (DO_EVACUATION) CYCLE READ_DEVC_LOOP
...
LOGICAL :: VECTOR,CELL_CENTERED,FACE_CENTERED,EVACUATION
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   COUNT_RADF_LOOP: DO
      ...
      IF (DO_EVACUATION) CYCLE COUNT_RADF_LOOP
      N_RADF = N_RADF + 1
      10 IF (IOS>0) THEN
   ...
   RADF_LOOP: DO NN=1,N_RADF
      ...
      READ(LU_INPUT,RADF)
      IF (DO_EVACUATION) CYCLE RADF_LOOP
...
REAL(EB) :: H_G_1,H_G_2,H_L_1,H_L_2
LOGICAL :: FUEL
...
DO I=0,I_MAX_TEMP
   ...
   IF (INT(T_R)==I) THEN
      H_L_1 = H_L
      H_G_1 = H_G
   ELSEIF(INT(T_R)+1==I) THEN
      H_L_2 = H_L
      H_G_2 = H_G
   ENDIF
END DO

H_L = H_L_1+(T_R-INT(T_R))*(H_L_2-H_L_1)
H_G = H_G_1+(T_R-INT(T_R))*(H_G_2-H_G_1)
H_V_H2O = H_V_H2O + H_V - (H_G-H_L)
