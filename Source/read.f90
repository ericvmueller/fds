...
CHARACTER(25) :: COLOR
CHARACTER(LABEL_LENGTH) :: MULT_ID,PERIODIC_MESH_IDS(3)
NAMELIST /MESH/ COLOR,CYLINDRICAL,EVACUATION,EVAC_HUMANS,EVAC_Z_OFFSET, FYI,ID,IJK,LEVEL,MPI_PROCESS,MULT_ID,PERIODIC_MESH_IDS,&
                RGB,XB,N_THREADS
...
ALLOCATE(PERIODIC_MESH_NAMES(NMESHES,3),STAT=IZERO)
CALL ChkMemErr('READ','PERIODIC_MESH_NAMES',IZERO)
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            PERIODIC_MESH_NAMES(NM,:) = PERIODIC_MESH_IDS(:)
...
DO NM=1,NMESHES
   ...
   DO NM2=1,NMESHES
      ...
      M2 => MESHES(NM2)
      IF ((M2%XS>M%XF+NANOMETER .OR. M2%XF<M%XS-NANOMETER .OR. &
           M2%YS>M%YF+NANOMETER .OR. M2%YF<M%YS-NANOMETER .OR. &
           M2%ZS>M%ZF+NANOMETER .OR. M2%ZF<M%ZS-NANOMETER) .AND. &
           PERIODIC_MESH_NAMES(NM,1)/=MESH_NAME(NM2).AND. &
           PERIODIC_MESH_NAMES(NM,2)/=MESH_NAME(NM2).AND. &
           PERIODIC_MESH_NAMES(NM,3)/=MESH_NAME(NM2)) CYCLE
...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                BAROCLINIC,BNDF_DEFAULT,CC_IBM,CC_RADIATION,CCVOL_LINK,CC_ZEROIBM_VELO,CHECK_MASS_CONSERVE, &
                ...
                COMPUTE_VISCOSITY_TWICE,COMPUTE_ZETA_SOURCE_TERM,CONSTANT_H_SOLID,CONSTANT_SPECIFIC_HEAT_RATIO,CORIOLIS_VECTOR,&
                CORRECT_SUBGRID_TEMPERATURE,COUPLED_1D3D_HEAT_TRANSFER,C_DEARDORFF,C_RNG,C_RNG_CUTOFF,C_SMAGORINSKY,C_VREMAN,&
                C_WALE,DNS,DO_IMPLICIT_CCREGION,DRAG_CFL_MAX,DT_MEAN_FORCING,ENTHALPY_TRANSPORT,&
                ...
                GRAVITATIONAL_SETTLING,GROUND_LEVEL,GVEC,DT_HVAC,H_F_REFERENCE_TEMPERATURE,&
                ...
                RAMP_GX,RAMP_GY,RAMP_GZ,RAMP_TMP0_Z,RAMP_U0_T,RAMP_V0_T,RAMP_W0_T,RAMP_U0_Z,RAMP_V0_Z,RAMP_W0_Z,&
                RADIATION,RESEARCH_MODE,RESTART,RESTART_CHID,RICHARDSON_ERROR_TOLERANCE,&
                SC,SHARED_FILE_SYSTEM,SLIP_CONDITION,SMOKE_ALBEDO,SOLID_PHASE_ONLY,SOOT_OXIDATION,SPONGE_LAYER_DISTANCE,&
                ...
                TMPA,TURBULENCE_MODEL,TURBULENT_DEPOSITION,TURB_INIT_CLOCK,U0,UVW_FILE,V0,&
                VEG_LEVEL_SET_COUPLED,VEG_LEVEL_SET_UNCOUPLED,VERBOSE,VISIBILITY_FACTOR,VN_MAX,VN_MIN,Y_CO2_INFTY,Y_O2_INFTY,W0,&
                WD_PROPS,WIND_ONLY,FBRAND
...
IF (RAMP_U0_T/='null') CALL GET_RAMP_INDEX(RAMP_U0_T,'TIME',I_RAMP_U0_T)
IF (RAMP_V0_T/='null') CALL GET_RAMP_INDEX(RAMP_V0_T,'TIME',I_RAMP_V0_T)
...
IF (STRATIFICATION) THEN
   IF (RAMP_TMP0_Z=='null') RAMP_TMP0_Z = 'ASSUMED TEMPERATURE PROFILE'
   CALL GET_RAMP_INDEX(RAMP_TMP0_Z,'PROFILE',I_RAMP_TMP0_Z)
   CALL GET_RAMP_INDEX('COMPUTED PRESSURE PROFILE','PROFILE',I_RAMP_P0_Z)
ENDIF
...
REAL(EB) :: MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            ...
            CONVERSION,PR_GAS,CONDUCTIVITY_LIQUID,VISCOSITY_LIQUID,BETA_LIQUID,H_F_IN
REAL(EB):: MASS_FRACTION(MAX_SPECIES),VOLUME_FRACTION(MAX_SPECIES),MIN_DIAMETER,MAX_DIAMETER
INTEGER  :: N_SPEC_READ,N,NN,NNN,NS2,NR,N_SPEC_READ_2,N_SUB_SPECIES,NS,N_BINS,N_COPY,N_FOUND
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   IF (N_BINS > 0) THEN
      IF (AGGLOMERATION_INDEX>0) THEN
         WRITE(MESSAGE,'(A,I0,A)') 'ERROR: SPEC ',N_SPEC_READ,': Can only specify N_BINS for one &SPEC input'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (N_BINS < 2) THEN
...
IF (TRANSPORT_UNMIXED_FRACTION) THEN
   N_PASSIVE_SCALARS=1
   ZETA_INDEX=N_TRACKED_SPECIES+1
ENDIF
...
USE PROPERTY_DATA, ONLY : ELEMENT,GET_FORMULA_WEIGHT,MAKE_PERIODIC_TABLE,SIMPLE_SPECIES_MW,GAS_PROPS,LOOKUP_CHI_R
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX,GET_TABLE_INDEX
CHARACTER(LABEL_LENGTH) :: FUEL,RADCAL_ID='null',SPEC_ID_NU(MAX_SPECIES),SPEC_ID_N_S(MAX_SPECIES),RAMP_FS,TABLE_FS,&
                           RAMP_CHI_R
CHARACTER(100) :: FORMULA
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   RN%FLAME_SPEED               = FLAME_SPEED
   RN%FLAME_SPEED_EXPONENT      = FLAME_SPEED_EXPONENT
   RN%FLAME_SPEED_TEMPERATURE   = FLAME_SPEED_TEMPERATURE + TMPM
   IF (RN%FLAME_SPEED_TEMPERATURE <=0._EB) RN%FLAME_SPEED_TEMPERATURE = TMPA
   RN%FUEL                      = FUEL
   RN%FWD_ID                    = FWD_ID
   RN%FYI                       = FYI
   RN%H                         = H
   ...
   RN%TURBULENT_FLAME_SPEED_ALPHA    = TURBULENT_FLAME_SPEED_ALPHA
   RN%TURBULENT_FLAME_SPEED_EXPONENT = TURBULENT_FLAME_SPEED_EXPONENT

   IF (RN%RAMP_FS/='null' .AND. RN%TABLE_FS/='null') THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Can only specify one of RAMP_FS or TABLE_FS'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   ...
   IF (RADIATIVE_FRACTION < 0._EB) THEN
      IF (DNS) RN%CHI_R = 0._EB
      IF (LES) CALL LOOKUP_CHI_R(FUEL,RN%CHI_R)
   ELSE
...
CO_YIELD                    = 0._EB
CRITICAL_FLAME_TEMPERATURE  = 1427._EB   ! C (See C. Beyler, Ch 7, Eq 6, SFPE Handbook, 4th Ed.)
E                           = -1._EB     ! kJ/kmol
...
FLAME_SPEED                 = -1._EB
FLAME_SPEED_EXPONENT        = 0._EB
FLAME_SPEED_TEMPERATURE     = -273.15_EB
FORMULA                     = 'null'
FUEL                        = 'null'
FWD_ID                      = 'null'
FYI                         = 'null'
...
TABLE_FS                    = 'null'
THIRD_BODY                  = .FALSE.
TURBULENT_FLAME_SPEED_ALPHA    = 1._EB ! see O. Gulder. 23rd Int. Symp. on Comb. 1990.
TURBULENT_FLAME_SPEED_EXPONENT = 2._EB
...
IF (TRIM(ODE_SOLVER)/='null') THEN
   SELECT CASE (TRIM(ODE_SOLVER))
      ...
      CASE ('RK2')
         COMBUSTION_ODE_SOLVER = RK2
      CASE ('RK3')
         COMBUSTION_ODE_SOLVER = RK3
      CASE ('RK2 RICHARDSON')
...
LOGICAL :: TARGET_ONLY,MASSLESS,STATIC,MONODISPERSE,BREAKUP,CHECK_DISTRIBUTION,TURBULENT_DISPERSION,&
           PERIODIC_X,PERIODIC_Y,PERIODIC_Z,SECOND_ORDER_PARTICLE_TRANSPORT
   ...
   DO NR=1,ML%N_REACTIONS
      DO NS=1,MAX_SPECIES
         ...
         IF (TRIM(ML%SPEC_ID(NS,NR))=='null' .AND. ML%NU_SPEC(NS,NR)>TWO_EPSILON_EB) THEN
            WRITE(MESSAGE,'(A,A,A,I0,A,I0)') 'ERROR: MATL ',TRIM(MATL_NAME(N)),' requires a SPEC_ID for the',&
...
NAMELIST /SURF/ ADIABATIC,AUTO_IGNITION_TEMPERATURE,&
...
PERIODIC_WIND_SURF_INDEX     = N_SURF + 11
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   SF%AUTO_IGNITION_TEMPERATURE = AUTO_IGNITION_TEMPERATURE + TMPM
   SELECT CASE(BACKING)
      CASE('VOID')
         SF%BACKING        = VOID
      CASE('INSULATED')
   ...
   ! Determine if REIGNITION_MODEL is to be used

   IF (SF%AUTO_IGNITION_TEMPERATURE < 1.E20_EB ) REIGNITION_MODEL = .TRUE.
...
ELSE
   ...
   IF (PRESSURE_TOLERANCE<TWO_EPSILON_EB) PRESSURE_TOLERANCE = 20.0_EB/CHARACTERISTIC_CELL_SIZE**2
ENDIF
...
USE RADCONS
REAL(EB) :: BAND_LIMITS(MAX_NUMBER_SPECTRAL_BANDS+1), RADIATIVE_FRACTION=-1._EB
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,INITIAL_RADIATION_ITERATIONS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,&
                PATH_LENGTH,&
                QR_CLIP,RADIATION,RADIATION_ITERATIONS,RADIATIVE_FRACTION,RADTMP,RTE_SOURCE_CORRECTION,TIME_STEP_INCREMENT,&
...
IF (RADIATIVE_FRACTION >=0._EB) THEN
   CALL SHUTDOWN('ERROR: RADIATIVE_FRACTION is now specified on REAC.')
   RETURN
ENDIF
...
ELSE
   RHOMAX = 3.0_EB*P_INF*MW_MAX/(R0*MAX(TMPMIN,1._EB))
ENDIF
...
COUNT_RAMP_POINTS: DO N=1,N_RAMP
   ...
   IF (RAMP_ID(N)=='ASSUMED TEMPERATURE PROFILE' .OR. RAMP_ID(N)=='COMPUTED PRESSURE PROFILE') THEN
      RP%RESERVED = .TRUE.
      RP%NUMBER_DATA_POINTS = 2
   ELSE
      REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
...
READ_RAMP_LOOP: DO N=1,N_RAMP
   ...
   IF (RP%RESERVED) THEN
      ...
      RP%INDEPENDENT_DATA(1) = ZS_MIN
      RP%INDEPENDENT_DATA(2) = ZF_MAX
      RP%DEPENDENT_DATA(1)   = (TMPA+LAPSE_RATE*(ZS_MIN-GROUND_LEVEL))/TMPA
      RP%DEPENDENT_DATA(2)   = (TMPA+LAPSE_RATE*(ZF_MAX-GROUND_LEVEL))/TMPA
...
COUNT_TABLE_POINTS: DO N=1,N_TABLE
   ...
   SELECT CASE (TABLE_TYPE(N))
      ...
      CASE (PART_RADIATIVE_PROPERTY)
         TA%NUMBER_COLUMNS = 3
      CASE (TABLE_2D_TYPE,FLAME_SPEED_TABLE)
         TA%NUMBER_COLUMNS = 3
   END SELECT
   SEARCH_LOOP: DO
   ...
   IF (TA%NUMBER_ROWS<=0) THEN
      ...
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (TABLE_TYPE(N) == TABLE_2D_TYPE .OR. TABLE_TYPE(N) == FLAME_SPEED_TABLE) THEN
      IF (TA%NUMBER_ROWS<=1) THEN
...
READ_TABL_LOOP: DO N=1,N_TABLE
   ...
   TABLE_2D_IF: IF (TABLE_TYPE(N)==TABLE_2D_TYPE) THEN
      ...
      DO I = 1, TA%NUMBER_ROWS
         ...
         ENDDO
      ENDDO
      IF (TABLE_TYPE(N)==FLAME_SPEED_TABLE) TA%Y=TA%Y+TMPM
   ENDIF TABLE_2D_IF
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               IF (WIND) THEN
                  IF (VT%SURF_INDEX==OPEN_SURF_INDEX) THEN
                     VT%IS_WIND_BOUNDARY = .TRUE.
                  ELSEIF (VT%SURF_INDEX==PERIODIC_WIND_SURF_INDEX) THEN
                  ELSE
                     IF (ID=='null') WRITE(MESSAGE,'(A,I0,A,I0)') "ERROR: VENT ",NN, &
                        " WIND=T requires SURF_ID='OPEN' or 'PERIODIC', line number ",INPUT_FILE_LINE_NUMBER
                     IF (ID/='null') WRITE(MESSAGE,'(A,A,A)')     "ERROR: VENT ",TRIM(ID), &
                        " WIND=T requires SURF_ID='OPEN' or 'PERIODIC', line number ",INPUT_FILE_LINE_NUMBER
                     CALL SHUTDOWN(MESSAGE) ; RETURN
                  ENDIF
               ENDIF
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IN%AIT           = AUTO_IGNITION_TEMPERATURE + TMPM
...
READ_ZONE_LOOP: DO N=1,N_ZONE-N_EVAC_ZONE
   ...
   P_ZONE(N)%LEAK_REFERENCE_PRESSURE(0:N_ZONE) = LEAK_REFERENCE_PRESSURE(0:N_ZONE)
   P_ZONE(N)%LEAK_PRESSURE_EXPONENT(0:N_ZONE) = LEAK_PRESSURE_EXPONENT(0:N_ZONE)
   P_ZONE(N)%X1 = XB(1)
   P_ZONE(N)%X2 = XB(2)
   P_ZONE(N)%Y1 = XB(3)
   P_ZONE(N)%Y2 = XB(4)
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   LINE_DEVICE = .FALSE.
   IF (POINTS>1 .AND. .NOT.TIME_HISTORY) LINE_DEVICE = .TRUE.
   ...
   IF (POINTS>1 .AND. ANY(XB<-1.E5_EB) .AND. INIT_ID=='null') THEN
      ...
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (LINE_DEVICE .AND. TRIM(STATISTICS)/='null'     .AND. &
      ...
      WRITE(MESSAGE,'(A,A,A)')  'ERROR: DEVC ',TRIM(ID),' cannot use a steady-state line device and STATISTICS at the same time'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (.NOT.LINE_DEVICE .AND. (TRIM(STATISTICS)=='TIME MIN' .OR. &
                               TRIM(STATISTICS)=='TIME MAX') ) THEN
...
INTEGER  :: N,NN,NNN,NM,QUANTITY_INDEX,QUANTITY2_INDEX,MAXCELLS,I,J,K
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   SPECIAL_QUANTITIES: SELECT CASE (DV%QUANTITY)
      ...
      CASE ('THERMOCOUPLE')
         ...
         IF (DV%STATISTICS=='MAX'.OR.DV%STATISTICS=='MIN'.OR.DV%STATISTICS=='MASS MEAN'.OR.DV%STATISTICS=='VOLUME MEAN'.OR.&
             DV%STATISTICS=='MASS INTEGRAL'.OR.DV%STATISTICS=='VOLUME INTEGRAL'.OR.DV%STATISTICS=='MEAN'.OR.&
             DV%STATISTICS=='AREA INTEGRAL') THEN
            WRITE(MESSAGE,'(A,A,A)') 'ERROR: DEVC ',TRIM(DV%ID),' cannot use volume STATISTICS'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         DV%TMP_L = PROPERTY(DV%PROP_INDEX)%INITIAL_TEMPERATURE
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (TRIM(QUANTITY)=='H PRIME' .AND. .NOT.EXTERNAL_BOUNDARY_CORRECTION) THEN
         WRITE(MESSAGE,'(A,I0,A)') 'ERROR: Promblem with SCLF ',NN,', H PRIME requires EXTERNAL_BOUNDARY_CORRECTION=T on MISC'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF

      IF (TRIM(QUANTITY)=='SOLID CELL Q_S') STORE_Q_DOT_PPP_S = .TRUE.

      IF (TRIM(QUANTITY)=='SUBGRID TEMPERATURE CORRECTION' .AND. .NOT.CORRECT_SUBGRID_TEMPERATURE) THEN
         WRITE(MESSAGE,'(A,I0,A)') 'ERROR: Promblem with SCLF ',NN,', requires CORRECT_SUBGRID_TEMPERATURE=T on MISC'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
   ...
   QUANTITY_IF:IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      IF (TRIM(QUANTITY)=='DIFFUSIVITY' .AND. DNS .AND. Z_INDEX < 0) THEN
