...
NAMELIST /WIND/ CORIOLIS_VECTOR,DIRECTION,DT_MEAN_FORCING,FORCE_VECTOR,FYI,GROUND_LEVEL,INITIALIZATION_ONLY,L,&
                LAPSE_RATE,MEAN_FORCING,NEW_MOMENTUM_NUDGING,OBUKHOV_LENGTH,&
                ...
                SPEED,SPONGE_LAYER_DISTANCE,STRATIFICATION,THETA_STAR,TMP_REF,U_STAR,U0,V0,W0,Z_0,Z_REF
...
REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
WIND_LOOP: DO
   CALL CHECKREAD('WIND',LU_INPUT,IOS)
...
IF (U0/=0._EB .AND. .NOT. INITIALIZATION_ONLY) MEAN_FORCING(1) = .TRUE.
IF (V0/=0._EB .AND. .NOT. INITIALIZATION_ONLY) MEAN_FORCING(2) = .TRUE.
IF (W0/=0._EB .AND. .NOT. INITIALIZATION_ONLY) MEAN_FORCING(3) = .TRUE.
...
IF (I_RAMP_DIRECTION/=0) THEN
   DO I=0,RAMPS(I_RAMP_DIRECTION)%NUMBER_INTERPOLATION_POINTS+1
      THETA = RAMPS(I_RAMP_DIRECTION)%INTERPOLATED_DATA(I)*PI/180._EB
      RAMPS(I_RAMP_U0_T)%INTERPOLATED_DATA(I) = -SIN(THETA)
      RAMPS(I_RAMP_V0_T)%INTERPOLATED_DATA(I) = -COS(THETA)
   ENDDO
ENDIF
