...
INTEGER :: IJK(3),NM,NM2,CURRENT_MPI_PROCESS,MPI_PROCESS,RGB(3),LEVEL,N_MESH_NEW,N,II,JJ,KK,NMESHES_READ,NNN, &
           N_THREADS, JBAR_OLD_VALUE
...
NAMELIST /MESH/ CHECK_MESH_ALIGNMENT,COLOR,CYLINDRICAL,FYI,ID,IJK,LEVEL,MPI_PROCESS,MULT_ID,&
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE,ALLOW_SURFACE_PARTICLES, &
                ...
                NOISE,NOISE_VELOCITY,NUCLEATION_SITES,ORIGIN_LAT,ORIGIN_LON,&
...
CHARACTER(LABEL_LENGTH) :: RAMP_FVX_T,RAMP_FVY_T,RAMP_FVZ_T,RAMP_U0_T,RAMP_V0_T,RAMP_W0_T,&
                           RAMP_U0_Z,RAMP_V0_Z,RAMP_W0_Z,RAMP_TMP0_Z,&
...
NAMELIST /WIND/ CORIOLIS_VECTOR,DIRECTION,FORCE_VECTOR,FYI,GEOSTROPHIC_WIND,GROUND_LEVEL,L,LAPSE_RATE,LATITUDE,&
                POTENTIAL_TEMPERATURE_CORRECTION,RAMP_DIRECTION_T,RAMP_DIRECTION_Z,RAMP_FVX_T,RAMP_FVY_T,&
                RAMP_FVZ_T,RAMP_SPEED_T,RAMP_SPEED_Z,RAMP_TMP0_Z,RAMP_U0_T,RAMP_U0_Z,RAMP_V0_T,RAMP_V0_Z,&
                RAMP_W0_T,RAMP_W0_Z,SIGMA_THETA,SPEED,STRATIFICATION,TAU_THETA,THETA_STAR,TMP_REF,U_STAR,U0,&
...
IF (RAMP_DIRECTION_T/='null') THEN  ! create dummy time RAMPs for U0 and V0 and fill in later
   RAMP_U0_T = RAMP_DIRECTION_T
   RAMP_V0_T = RAMP_DIRECTION_T
ENDIF
IF (RAMP_DIRECTION_Z/='null') THEN  ! create dummy time RAMPs for U0 and V0 and fill in later
   RAMP_U0_Z = RAMP_DIRECTION_Z
   RAMP_V0_Z = RAMP_DIRECTION_Z
ENDIF
IF (RAMP_SPEED_T/='null') THEN  ! create dummy time RAMPs for U0 and V0 and fill in later
   RAMP_U0_T = RAMP_SPEED_T
   RAMP_V0_T = RAMP_SPEED_T
ENDIF
IF (RAMP_SPEED_Z/='null') THEN  ! create dummy time RAMPs for U0 and V0 and fill in later
   RAMP_U0_Z = RAMP_SPEED_Z
   RAMP_V0_Z = RAMP_SPEED_Z
ENDIF
...
IF (RAMP_U0_Z/='null') CALL GET_RAMP_INDEX(RAMP_U0_Z,'PROFILE',I_RAMP_U0_Z,DUPLICATE_RAMP=.TRUE.)
IF (RAMP_V0_Z/='null') CALL GET_RAMP_INDEX(RAMP_V0_Z,'PROFILE',I_RAMP_V0_Z,DUPLICATE_RAMP=.TRUE.)
...
IF (I_RAMP_DIRECTION_T/=0 .OR. I_RAMP_SPEED_T/=0) THEN
   IF (I_RAMP_DIRECTION_T/=0) N_PTS = RAMPS(I_RAMP_DIRECTION_T)%NUMBER_INTERPOLATION_POINTS+1
   IF (I_RAMP_SPEED_T    /=0) N_PTS = RAMPS(I_RAMP_SPEED_T    )%NUMBER_INTERPOLATION_POINTS+1
   DO I=0,N_PTS
      IF (I_RAMP_DIRECTION_T/=0) THEN
         THETA = RAMPS(I_RAMP_DIRECTION_T)%INTERPOLATED_DATA(I)*DEG2RAD
         SIN_THETA = -SIN(THETA)
         COS_THETA = -COS(THETA)
      ELSE
         SIN_THETA = 1._EB
         COS_THETA = 1._EB
      ENDIF
      IF (I_RAMP_SPEED_T/=0) THEN
         SPEED_FACTOR = RAMPS(I_RAMP_SPEED_T)%INTERPOLATED_DATA(I)
      ELSE
         SPEED_FACTOR = 1._EB
      ENDIF
...
IF (I_RAMP_DIRECTION_Z/=0 .OR. I_RAMP_SPEED_Z/=0) THEN
   IF (I_RAMP_DIRECTION_Z/=0) N_PTS = RAMPS(I_RAMP_DIRECTION_Z)%NUMBER_INTERPOLATION_POINTS+1
   IF (I_RAMP_SPEED_Z    /=0) N_PTS = RAMPS(I_RAMP_SPEED_Z    )%NUMBER_INTERPOLATION_POINTS+1
   DO I=0,N_PTS
      IF (I_RAMP_DIRECTION_Z/=0) THEN
         THETA = RAMPS(I_RAMP_DIRECTION_Z)%INTERPOLATED_DATA(I)*DEG2RAD
         SIN_THETA = -SIN(THETA)
         COS_THETA = -COS(THETA)
      ELSE
         SIN_THETA = 1._EB
         COS_THETA = 1._EB
      ENDIF
      IF (I_RAMP_SPEED_Z/=0) THEN
         SPEED_FACTOR = RAMPS(I_RAMP_SPEED_Z)%INTERPOLATED_DATA(I)
      ELSE
         SPEED_FACTOR = 1._EB
      ENDIF
      RAMPS(I_RAMP_U0_Z)%INTERPOLATED_DATA(I) = SPEED_FACTOR*SIN_THETA
      RAMPS(I_RAMP_V0_Z)%INTERPOLATED_DATA(I) = SPEED_FACTOR*COS_THETA
   ENDDO
ENDIF
...
NAMELIST /DUMP/ BNDF_TIMER,CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,&
                ...
                STATUS_FILES,SUPPRESS_DIAGNOSTICS,&
...
LOGICAL :: DEFAULT,VEG_LSET_SPREAD,VEG_LSET_TAN2,TGA_ANALYSIS,COMPUTE_EMISSIVITY,&
           COMPUTE_EMISSIVITY_BACK,HT3D,THERM_THICK,BLOWING,ABL_MODEL
...
NAMELIST /HOLE/ COLOR,CTRL_ID,DEVC_ID,FYI,ID,MESH_ID,MULT_ID,RGB,TRANSPARENCY,XB
...
NAMELIST /CTRL/  CONSTANT,DELAY,DIFFERENTIAL_GAIN,FUNCTION_TYPE,ID,INITIAL_STATE,&
...
LOGICAL :: VECTOR,CELL_CENTERED,FACE_CENTERED
...
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,DB,FACE_CENTERED,FYI,ID,MAXIMUM_VALUE,MATL_ID,MESH_NUMBER,&
