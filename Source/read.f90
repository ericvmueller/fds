   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (XB(1)>-1.E5_EB) THEN
         ...
         CHECK_MESH_LOOP: DO NM=1,NMESHES
            ...
            IF (XB(3)/=XB(4) .AND. (MIN(XB(4),M%YF)-MAX(XB(3),M%YS)<TWENTY_EPSILON_EB)) OVERLAPPING_Y = .FALSE.
            IF (XB(5)==XB(6) .AND. (XB(5)> M%ZF .OR. XB(6)< M%ZS)) OVERLAPPING_Z = .FALSE.
            IF (XB(5)/=XB(6) .AND. (MIN(XB(6),M%ZF)-MAX(XB(5),M%ZS)<TWENTY_EPSILON_EB)) OVERLAPPING_Z = .FALSE.
