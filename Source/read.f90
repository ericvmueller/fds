...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   IF (ML%H_R_I(1)>0) THEN
      H_R = EVALUATE_RAMP(ML%TMP_BOIL,0._EB,ML%H_R_I(1))
   ELSE
      H_R = ML%H_R(1)
   ENDIF

   ML%HEAT_OF_GASIFICATION = ML%H(MIN(I_MAX_TEMP,NINT(ML%TMP_BOIL))) - ML%H(NINT(TMPA)) + H_R
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (POINTS>1) THEN
         XYZ(1) = XBP(1) + (XBP(2)-XBP(1))*REAL(I_POINT-1,EB)/REAL(MAX(POINTS-1,1),EB)
         XYZ(2) = XBP(3) + (XBP(4)-XBP(3))*REAL(I_POINT-1,EB)/REAL(MAX(POINTS-1,1),EB)
         XYZ(3) = XBP(5) + (XBP(6)-XBP(5))*REAL(I_POINT-1,EB)/REAL(MAX(POINTS-1,1),EB)
