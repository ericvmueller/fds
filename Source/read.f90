...
LOGICAL :: DNS,RESEARCH_MODE,NO_EVACUATION
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE,ALLOW_SURFACE_PARTICLES, &
                ...
                STORE_DIVERGENCE_CORRECTION,TAU_DEFAULT,TERRAIN_IMAGE,&
                TEST_NEW_WAKE_REDUCTION,TEXTURE_ORIGIN,&
...
IF (NO_EVACUATION .OR. .NOT.(EVACUATION_DRILL.OR.EVACUATION_MC_MODE.OR.EVACUATION_WRITE_FED.OR.EVACUATION_INITIALIZATION)) THEN
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,INITIAL_RADIATION_ITERATIONS,KAPPA0,&
                ...
                TIME_STEP_INCREMENT,TRI_MODEL,WIDE_BAND_MODEL,WSGG_MODEL
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   MESH_LOOP_2: DO NM=1,NMESHES
      ...
      IF (DV%X1>-1.E5_EB) THEN
         ...
         SDV%I1 =   FLOOR( GINV(SDV%X1-M%XS,1,NM)*M%RDXI ) + 1
         SDV%I2 = CEILING( GINV(SDV%X2-M%XS,1,NM)*M%RDXI )
         SDV%J1 =   FLOOR( GINV(SDV%Y1-M%YS,2,NM)*M%RDETA ) + 1
         SDV%J2 = CEILING( GINV(SDV%Y2-M%YS,2,NM)*M%RDETA )
         SDV%K1 =   FLOOR( GINV(SDV%Z1-M%ZS,3,NM)*M%RDZETA ) + 1
         SDV%K2 = CEILING( GINV(SDV%Z2-M%ZS,3,NM)*M%RDZETA )
