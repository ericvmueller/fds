...
USE MISC_FUNCTIONS, ONLY: SEARCH_CONTROLLER,WRITE_SUMMARY_INFO,GET_RAMP_INDEX,GET_SPEC_OR_SMIX_INDEX,DEFINE_PRES_METHOD
...
IF (DT<=0._EB) THEN
   ...
   IF (ABS(U0)>TWENTY_EPSILON_EB .OR. ABS(V0)>TWENTY_EPSILON_EB .OR. ABS(W0)>TWENTY_EPSILON_EB) &
   ...
   ICLOOP_2: DO IC=1,3
      ...
      SELECT CASE (T%ITRAN(IC))
         ...
         CASE (2)  ! linear transformation
            ...
            DO N=1,T%NOC(IC)+1
               IF (T%C1(N,IC)-T%C1(N-1,IC)<TWENTY_EPSILON_EB) THEN
...
IF (T_END>TWENTY_EPSILON_EB) CHECK_MESH_ALIGNMENT=.FALSE. ! overwrite user-specified value
...
IF (DT_EXTERNAL_HEARTBEAT <= TWENTY_EPSILON_EB .NEQV. EXTERNAL_HEARTBEAT_FILENAME=='null') THEN
   WRITE(MESSAGE,'(A)') &
...
COUNT_MULT_LOOP: DO
   ...
   N_MULT = N_MULT + 1
   10 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I0,A,I0)') 'ERROR(101): Problem with MULT number ',N_MULT+1,', line number ',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
READ_MULT_LOOP: DO N=0,N_MULT
   ...
   MR%DY0     = DY0
   MR%DZ0     = DZ0
   IF (ABS(DX)>TWENTY_EPSILON_EB) MR%DXB(1:2) = DX
   IF (ABS(DY)>TWENTY_EPSILON_EB) MR%DXB(3:4) = DY
   IF (ABS(DZ)>TWENTY_EPSILON_EB) MR%DXB(5:6) = DZ
   ...
   IF ( (I_UPPER-I_LOWER>0 .AND. ABS(DX)<TWENTY_EPSILON_EB) .OR. &
        (J_UPPER-J_LOWER>0 .AND. ABS(DY)<TWENTY_EPSILON_EB) .OR. &
        (K_UPPER-K_LOWER>0 .AND. ABS(DZ)<TWENTY_EPSILON_EB)      ) THEN
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE, &
                ...
                TAU_DEFAULT,TENSOR_DIFFUSIVITY,TERRAIN_IMAGE,TEST_NEW_CHAR_MODEL,TEST_NEW_KSGS_MODEL,TEXTURE_ORIGIN,&
...
IF (Z_0<-TWENTY_EPSILON_EB) Z_0 = 0.03_EB
...
IF (U_STAR>0._EB .AND. ABS(L)>TWENTY_EPSILON_EB) THEN
...
IF (STRATIFICATION) THEN
   ...
   IF (ABS(L)>1.E-10_EB) THEN
      ...
      DO I=1,N_MO_PTS
         ...
         IF (SPEED>TWENTY_EPSILON_EB) THEN
...
IF (ANY(ABS(GEOSTROPHIC_WIND)>TWENTY_EPSILON_EB)) THEN
   IF (ALL(ABS(OVEC)<TWENTY_EPSILON_EB)) THEN
   ...
   IF (SPEC_ID(1)=='null') THEN
      ...
      IF (N_BINS>0) THEN
         ...
         IF (.NOT. AEROSOL) THEN
            ...
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         IF (MEAN_DIAMETER > TWENTY_EPSILON_EB) THEN
...
IF (SIMPLE_CHEMISTRY) THEN
   DO NR=1,N_REACTIONS
      ...
      IF (ANY(SPECIES_MIXTURE(N_PREDEFINED_SMIX+1:N_TRACKED_SPECIES)%ID==REACTION(NR)%FUEL)) THEN
         ...
         ENDIF
      ELSE
         IF (REACTION(NR)%C<=TWENTY_EPSILON_EB .AND. REACTION(NR)%H<=TWENTY_EPSILON_EB) THEN
...
DO N1 = 1, N_TRACKED_SPECIES
   ...
   SPECIES_MIXTURE(N1)%EQUIV = SPECIES_MIXTURE(N1)%OXR/(SPECIES_MIXTURE(N1)%OXA+TWENTY_EPSILON_EB)
ENDDO
      ...
      IF (TRIM(SPECIES(NS2)%ID) == TRIM(SPEC_ID(NS))) THEN
         ...
         IF (SPECIES(NS2)%MODE == AEROSOL_SPECIES) THEN
            IF (N_SUB_SPECIES == 1) THEN
               ...
               ENDIF
               IF (ABS(DENSITY_SOLID-SOOT_DENSITY) <=TWENTY_EPSILON_EB .AND. &
                     ABS(DENSITY_SOLID-SPECIES(NS2)%DENSITY_SOLID) <=TWENTY_EPSILON_EB) THEN
...
DO NS = 1,N_SPECIES
   IF (SM%MASS_FRACTION(NS) <TWENTY_EPSILON_EB) CYCLE
...
ELSE BACKGROUND_IF ! Mixture is fuel or products
   ...
   N_SIMPLE_CHEM_RXN: SELECT CASE (RN%N_SIMPLE_CHEMISTRY_REACTIONS)
      CASE (1) N_SIMPLE_CHEM_RXN
         ...
         IF (ABS(RN%NU_H2O) < TWENTY_EPSILON_EB) RN%NU_H2O = 0._EB
         RN%NU_CO2          = RN%C - RN%NU_CO - RN%NU_HCN - RN%NU_SOOT*SOOT_C_FRACTION
         IF (ABS(RN%NU_CO2) < TWENTY_EPSILON_EB) RN%NU_CO2 = 0._EB
      ...
      CASE (2) N_SIMPLE_CHEM_RXN
         ...
         IF (ABS(RN%NU_CO)<TWENTY_EPSILON_EB) RN%NU_CO = 0._EB
         RN%NU_SOOT         = (RN%C-RN%NU_CO-RN%NU_HCN)/SOOT_C_FRACTION
         IF (ABS(RN%NU_SOOT)<TWENTY_EPSILON_EB) RN%NU_SOOT = 0._EB
         RN%NU_H2           = RN%H*RN%FUEL_H_TO_H2_FRACTION*0.5_EB
         IF (ABS(RN%NU_H2)<TWENTY_EPSILON_EB) RN%NU_H2 = 0._EB
         ...
         IF (ABS(RN%NU_N2)<TWENTY_EPSILON_EB) RN%NU_N2 = 0._EB
         IF (RN%NU_N2 <0._EB) THEN
         ...
         IF (ABS(RN2%NU_H2O) < TWENTY_EPSILON_EB) RN%NU_H2O = 0._EB
         RN2%NU_CO2          = RN%C - RN2%NU_CO - RN2%NU_HCN - RN2%NU_SOOT*SOOT_C_FRACTION
         IF (ABS(RN2%NU_CO2) < TWENTY_EPSILON_EB) RN2%NU_CO2 = 0._EB
...
INTEGER :: NR,NS,NS2,PRIORITY,PROD_COUNTER=0, NEW_REAC=0,N_SIMPLE_CHEMISTRY_REACTIONS,IZ,FUEL_INDEX,N_REVERSE=0
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   A_E_IF: IF (A < 0._EB .AND. E < 0._EB .AND. TRIM(SPEC_ID_NU(1))=='null' .AND. TRIM(EQUATION)=='null') THEN
      ...
      C_H_IF: IF(C<=TWENTY_EPSILON_EB .AND. H<=TWENTY_EPSILON_EB) THEN
   ...
   SELECT CASE (REACTYPE)
      ...
      CASE('FALLOFF-TROE')
         ...
         ENDIF
         RN%A_TROE = A_TROE
         RN%RT1_TROE = 1._EB/(T1_TROE+TWENTY_EPSILON_EB)
         RN%T2_TROE = T2_TROE
         RN%RT3_TROE = 1._EB/(T3_TROE+TWENTY_EPSILON_EB)
         RN%THIRD_BODY = .TRUE.
   ...
   IF (RN%A_IN<0._EB .AND. RN%E<0._EB .AND. .NOT.RN%REVERSE) RN%FAST_CHEMISTRY=.TRUE.
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (TRIM(RN%EQUATION)/='null') THEN
      IF(ANY(ABS(RN%NU_READ)>TWENTY_EPSILON_EB)) THEN
      ...
      RN%N_SMIX = 0
      DO NS=1,MAX_SPECIES
         IF(ABS(RN%NU_READ(NS))>TWENTY_EPSILON_EB) THEN
   ...
   RN%N_SMIX_R = 0._EB
   DO NS=1,N_TRACKED_SPECIES
      IF (ABS(RN%NU(NS)) > TWENTY_EPSILON_EB) RN%N_SMIX_FR = RN%N_SMIX_FR + 1
   ...
   IF (.NOT. RN%SIMPLE_CHEMISTRY) THEN
      IF (RN%FUEL=='null') THEN
         FIND_FUEL: DO NS=1,RN%N_SMIX
            IF (RN%NU_READ(NS)<-TWENTY_EPSILON_EB) THEN
               DO NS2=1,N_TRACKED_SPECIES
                  IF (RN%NU(NS2) < -TWENTY_EPSILON_EB .AND. &
   ...
   RN%N_SPEC=0
   DO NS=1,N_SPECIES
      IF (ABS(NU_Y(NS)) > TWENTY_EPSILON_EB) RN%N_SPEC = RN%N_SPEC + 1
   ENDDO
   ...
   NS2 = 0
   DO NS=1,N_SPECIES
      IF (ABS(NU_Y(NS)) > TWENTY_EPSILON_EB) THEN
         NS2 = NS2 + 1
         RN%N_S_INDEX(NS2) = NS
         IF (ABS(REAL(INT(NU_Y(NS)),EB)-NU_Y(NS)) < TWENTY_EPSILON_EB) THEN
   ...
   IF (.NOT. RN%SIMPLE_CHEMISTRY .AND. RN%CHECK_ATOM_BALANCE) THEN
      ...
      DO NS=1,N_TRACKED_SPECIES
         IF (ABS(RN%NU(NS))>TWENTY_EPSILON_EB .AND. .NOT. SPECIES_MIXTURE(NS)%VALID_ATOMS) SKIP_ATOM_BALANCE = .TRUE.
   ...
   DO NS=1,N_TRACKED_SPECIES
      IF (RN%NU(NS) < -TWENTY_EPSILON_EB) MASS_REACTANT = MASS_REACTANT + RN%NU(NS)*SPECIES_MIXTURE(NS)%MW
      IF (RN%NU(NS) >  TWENTY_EPSILON_EB) MASS_PRODUCT  = MASS_PRODUCT  + RN%NU(NS)*SPECIES_MIXTURE(NS)%MW
   ENDDO
   IF (ABS(MASS_PRODUCT) < TWENTY_EPSILON_EB .OR. ABS(MASS_REACTANT) < TWENTY_EPSILON_EB) THEN
      IF (ABS(MASS_PRODUCT) <TWENTY_EPSILON_EB) WRITE(MESSAGE,'(A,I0,A)') 'ERROR(205): REAC ',NR,'. Products not specified.'
      IF (ABS(MASS_REACTANT)<TWENTY_EPSILON_EB) WRITE(MESSAGE,'(A,I0,A)') 'ERROR(206): REAC ',NR,'. Reactants not specified.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   ...
   NS2 = 0
   DO NS=1,N_TRACKED_SPECIES
      IF (ABS(RN%NU_MW_O_MW_F(NS)) > TWENTY_EPSILON_EB) THEN
         NS2 = NS2 + 1
         RN%NU_INDEX(NS2) = NS
...
USE DEVICE_VARIABLES, ONLY : PROPERTY_TYPE,N_PROP,PROPERTY
USE RADCONS, ONLY : MIE_NDG
INTEGER :: SAMPLING_FACTOR,N,NN,NR,ILPC,IPC,RGB(3),N_STRATA,N_LAGRANGIAN_CLASSES_READ,ADHERE_TO_SOLID, &
           NEW_PARTICLE_INCREMENT,N_LP_INTERP_INDICES
...
LOGICAL :: TARGET_ONLY,MASSLESS,STATIC,MONODISPERSE,BREAKUP,CHECK_DISTRIBUTION,DEBUG,&
           TURBULENT_DISPERSION,EMBER_PARTICLE,TRACK_EMBERS,FOUND_IT,SUBGRID_INTERPOLATION
...
NAMELIST /PART/ ADHERE_TO_SOLID,AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                HORIZONTAL_VELOCITY,ID,INITIAL_TEMPERATURE,SUBGRID_INTERPOLATION,KILL_DIAMETER,MASSLESS,&
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (PROP_ID/='null') THEN
      FOUND_IT = .FALSE.
      DO NN=1,N_PROP
         IF (TRIM(PROP_ID)==TRIM(PROPERTY(NN)%ID)) FOUND_IT = .TRUE.
      ENDDO
      IF (.NOT.FOUND_IT) THEN
         WRITE(MESSAGE,'(5A)') 'ERROR(226): PROP_ID ',TRIM(PROP_ID),' for PART ',TRIM(ID),' not found.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
   ...
   IF (ANY(ABS(ORIENTATION(1:3))>TWENTY_EPSILON_EB)) LPC%N_ORIENTATION = LPC%N_ORIENTATION + 1
   ...
   LPC%SUBGRID_INTERPOLATION = SUBGRID_INTERPOLATION
   IF (LPC%SUBGRID_INTERPOLATION) THEN
      INTERPOLATE_PART = .TRUE.
      LPC%INCLUDE_BOUNDARY_RADIA_TYPE = .TRUE.
      N_LP_INTERP_INDICES = N_LP_INTERP_INDICES + 1
   ENDIF
   ...
   SELECT CASE(DRAG_LAW)
      ...
      CASE('POROUS MEDIA')
         IF (ANY(DRAG_COEFFICIENT < 0._EB) .OR. ANY(PERMEABILITY < TWENTY_EPSILON_EB)) THEN
...
! Setup global index for particle radiation interpolation
IF (INTERPOLATE_PART) THEN
   ALLOCATE(LP_INTERP_INDICES(N_LP_INTERP_INDICES),STAT=IZERO)
   N = 1
   DO ILPC=1,N_LAGRANGIAN_CLASSES
      LPC=>LAGRANGIAN_PARTICLE_CLASS(ILPC)
      IF (LPC%SUBGRID_INTERPOLATION) THEN
         LP_INTERP_INDICES(N) = LPC%ARRAY_INDEX
         N = N + 1
      ENDIF
   ENDDO
ENDIF
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   ENDIF
   IF (ABS(SPRAY_ANGLE(1,1)-SPRAY_ANGLE(2,1))<TWENTY_EPSILON_EB .OR. ABS(SPRAY_ANGLE(1,2)-SPRAY_ANGLE(2,2))<TWENTY_EPSILON_EB) THEN
   ...
   IF (PART_ID/='null' .AND. SPRAY_PATTERN_TABLE == 'null' .AND. PDPA_RADIUS<TWENTY_EPSILON_EB) THEN
   ...
   IF (HISTOGRAM) THEN
      IF (HISTOGRAM_NBINS<2) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
       IF (ABS(HISTOGRAM_LIMITS(1)-HISTOGRAM_LIMITS(2)) < TWENTY_EPSILON_EB) THEN
   ...
   IF (PART_ID /='null' .AND. ABS(PDPA_RADIUS) <= TWENTY_EPSILON_EB) THEN
      IF (MASS_FLOW_RATE > 0._EB) THEN
         PY%MASS_FLOW_RATE = MASS_FLOW_RATE
         IF (ABS(PARTICLE_VELOCITY) <= TWENTY_EPSILON_EB) THEN
      ...
      ELSE
         ...
         IF (PARTICLE_VELOCITY<=TWENTY_EPSILON_EB .AND. ORIFICE_DIAMETER<=TWENTY_EPSILON_EB .AND. &
   ...
   ENDIF
   IF (ABS(FLOW_TAU) > TWENTY_EPSILON_EB) THEN
   ...
   IF (PY%VIEW_ANGLE<TWENTY_EPSILON_EB .OR. PY%VIEW_ANGLE>180._EB) THEN
...
PROP_LOOP: DO N=0,N_PROP
   ...
   IF (PY%PART_ID/='null') THEN
      ...
      IF (PY%PARTICLE_VELOCITY>TWENTY_EPSILON_EB) LPC%PRIMARY_BREAKUP_TIME = LPC%PRIMARY_BREAKUP_TIME/PY%PARTICLE_VELOCITY
...
LOGICAL :: ADJUST_H,ALLOW_SHRINKING, ALLOW_SWELLING,ADD_MATL,FOUND,SURFACE_OXIDATION_MODEL,&
           SURFACE_OXIDATION_VF_SCALING,ITERATE
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ADJUST_H,ALLOW_SHRINKING,ALLOW_SWELLING,BOILING_TEMPERATURE,&
                ...
                SURFACE_OXIDATION_VF_SCALING,X_O2_PYRO
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   NOT_BOILING: IF (BOILING_TEMPERATURE>4000._EB) THEN
      ...
      DO NR=1,N_REACTIONS
         ...
         IF (REFERENCE_TEMPERATURE(NR)<-TMPM  .AND. (E(NR)< 0._EB .OR. A(NR)<0._EB)) THEN
            ...
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         IF (ABS(SUM(NU_MATL(:,NR)))<=TWENTY_EPSILON_EB .AND. ABS(SUM(NU_SPEC(:,NR)))<=TWENTY_EPSILON_EB &
             .AND. ABS(SUM(NU_PART(:,NR)))<=TWENTY_EPSILON_EB) THEN
   ...
   ELSE NOT_BOILING ! Is liquid
      ...
      N_REACTIONS = 1
      IF (HEAT_OF_REACTION(1)<=TWENTY_EPSILON_EB) THEN
   ...
   IF (ABS(DENSITY) <=TWENTY_EPSILON_EB ) THEN
      WRITE(MESSAGE,'(A,A,A)') 'ERROR(253): Problem with MATL ',TRIM(ID),': DENSITY=0.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (ABS(CONDUCTIVITY) <=TWENTY_EPSILON_EB .AND. CONDUCTIVITY_RAMP == 'null' ) THEN
      WRITE(MESSAGE,'(A,A,A)') 'ERROR(254): Problem with MATL ',TRIM(ID),': CONDUCTIVITY = 0.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (ABS(SPECIFIC_HEAT)<=TWENTY_EPSILON_EB .AND. SPECIFIC_HEAT_RAMP == 'null' ) THEN
   ...
   ML%SURFACE_OXIDATION_VF_SCALING     = SURFACE_OXIDATION_VF_SCALING
...
SURFACE_OXIDATION_VF_SCALING = .FALSE.
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   DO NR=1,ML%N_REACTIONS
      DO NS=1,MAX_SPECIES
         ...
         IF (TRIM(ML%SPEC_ID(NS,NR))=='null' .AND. ABS(ML%NU_SPEC(NS,NR))>TWENTY_EPSILON_EB) THEN
         ...
         IF (ML%HEAT_OF_COMBUSTION(NS,NR) > 0._EB) THEN
            REAC_DO: DO NR2 = 1,N_REACTIONS
               IF(TRIM(ML%SPEC_ID(NS,NR))==TRIM(REACTION(NR2)%FUEL) .AND. REACTION(NR2)%HOC_COMPLETE > 0._EB) THEN
                  ...
                  IF (ABS(ML%ADJUST_BURN_RATE(Z_INDEX(NS,NR),NR)-1._EB) > TWENTY_EPSILON_EB) THEN
                     DO NS2=1,N_SPECIES
         ...
         ENDIF
      ENDDO
      IF (ANY(ABS(ML%NU_GAS(:,NR))>TWENTY_EPSILON_EB)) THEN
         DO NS=1,N_TRACKED_SPECIES
...
DO N=1,N_MATL
   ...
   DO NR=1,ML%N_REACTIONS
      ...
      IF (SUM_NU(N,NR)-1._EB < -TWENTY_EPSILON_EB) THEN
...
DO N=1,N_MATL
   ...
   DO NR=1,ML%N_REACTIONS
      DO NN=1,ML%N_RESIDUE(NR)
         IF (ABS(ML%NU_RESIDUE(NN,NR))>TWENTY_EPSILON_EB .AND. MATERIAL(ML%RESIDUE_MATL_INDEX(NN,NR))%ADJUST_H) THEN
...
DO N=1,N_MATL
   ...
   DO NR=1,ML%N_REACTIONS
      ...
      DO NS=1,N_TRACKED_SPECIES
         IF (ABS(ML%NU_GAS(NS,NR))>TWENTY_EPSILON_EB) THEN
      ...
      ENDDO
      DO NN=1,ML%N_RESIDUE(NR)
         IF (ABS(ML%NU_RESIDUE(NN,NR))>TWENTY_EPSILON_EB) THEN
            IF (ML%ADJUST_H) &
            ...
            ELSE LIQUID_IF
               ...
               DO I=1,SF%N_LAYERS
                  ...
                  MATL_RHO: DO NN=1,SF%N_MATL
                     IF (SF%MATL_MASS_FRACTION(I,NN)<=TWENTY_EPSILON_EB) CYCLE MATL_RHO
      ...
      DO NR=1,ML%N_REACTIONS
         ...
         DO I=0,I_MAX_TEMP
           H_R_CALC(I) = -(1._EB-NU_INERT)*ML%H(I)
            ...
            ENDDO
            DO NN=1,ML%N_RESIDUE(NR)
               IF (ABS(ML%NU_RESIDUE(NN,NR))>TWENTY_EPSILON_EB) &
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF ((VARIABLE_THICKNESS .OR. HT3D) .AND. THICKNESS(1)>TWENTY_EPSILON_EB .AND. MATL_ID(1,1)/='null') SF%LINING = .TRUE.
   ...
   IF (ANY(MOISTURE_CONTENT>TWENTY_EPSILON_EB) .OR. &
       ANY(MASS_PER_VOLUME>TWENTY_EPSILON_EB)   .OR. &
       ANY(SF%SURFACE_VOLUME_RATIO>TWENTY_EPSILON_EB)) THEN

      ! Determine convective heat transfer coefficient based on element, not surface geometry

      IF (ANY(MASS_PER_VOLUME>0._EB) .AND. SF%SURFACE_VOLUME_RATIO(1)>TWENTY_EPSILON_EB) &
      ...
      LAYER_LOOP_2: DO NL=1,MAX_LAYERS
         ...
         IF (SF%SURFACE_VOLUME_RATIO(NL)>TWENTY_EPSILON_EB .AND. THICKNESS(NL)<0._EB .AND. &
         ...
         IF (MOISTURE_CONTENT(NL)>TWENTY_EPSILON_EB) THEN
            DO NN=1,MAX_MATERIALS
               IF (MATL_ID(NL,NN) == 'null') EXIT
               IF (MATL_MASS_FRACTION(NL,NN)<TWENTY_EPSILON_EB) EXIT
               DO NNN=1,N_MATL
         ...
         IF (MASS_PER_VOLUME(NL)>TWENTY_EPSILON_EB) THEN
   ...
   LAYER_LOOP: DO IL=1,MAX_LAYERS
      ...
      IF ((ADIABATIC.OR.NET_HEAT_FLUX<1.E12_EB.OR.ABS(CONVECTIVE_HEAT_FLUX)>TWENTY_EPSILON_EB) &
      ...
      IF (RAMP_Q(NN)/='null' .AND. REFERENCE_HEAT_FLUX(NN) > 0._EB) THEN
         IF (NN>1) THEN
            ...
            IF (REFERENCE_THICKNESS(NN) > 0._EB .AND. REFERENCE_THICKNESS(NN-1) > 0._EB .AND. &
               ...
               ABS(1._EB-REFERENCE_THICKNESS(NN-1)/REFERENCE_THICKNESS(NN)) > TWENTY_EPSILON_EB) THEN
            ...
            IF (REFERENCE_HEAT_FLUX(NN) < REFERENCE_HEAT_FLUX(NN-1)) THEN
               IF (REFERENCE_THICKNESS(NN) > 0._EB .AND. &
                   (REFERENCE_THICKNESS(NN) - REFERENCE_THICKNESS(NN-1)) <=  TWENTY_EPSILON_EB) THEN
   ...
   IF (SF%N_QDOTPP_REF > 0) THEN
      ...
      DO NN = 1,SF%N_QDOTPP_REF
         IF (NN > 1) THEN
            IF (REFERENCE_THICKNESS(NN) - REFERENCE_THICKNESS(NN-1) > TWENTY_EPSILON_EB) THEN
               NNN = NNN + 1
               NNNN = 0
            ENDIF
         ENDIF
   ...
   SELECT CASE(GEOMETRY)
      ...
      CASE('CYLINDRICAL')
         ...
         IF (SF%INNER_RADIUS<TWENTY_EPSILON_EB) SF%BACKING = INSULATED
         SF%I_GRAD = 2
      CASE('INNER CYLINDRICAL')
         SF%GEOMETRY       = SURF_INNER_CYLINDRICAL
         IF (SF%INNER_RADIUS<TWENTY_EPSILON_EB) THEN
   ...
   IF (ANY(MASS_FLUX>0._EB) .AND. ABS(VEL)>TWENTY_EPSILON_EB)  THEN
      WRITE (MESSAGE,'(A,A,A)') 'ERROR(322): SURF ',TRIM(SF%ID),' cannot use both MASS_FLUX and VEL.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF

   IF (ANY(MASS_FLUX>0._EB) .AND. ABS(MASS_FLUX_TOTAL)>TWENTY_EPSILON_EB)  THEN
      WRITE (MESSAGE,'(A,A,A)') 'ERROR(323): SURF ',TRIM(SF%ID),' cannot use both MASS_FLUX and MASS_FLUX_TOTAL.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF

   IF (ABS(MASS_FLUX_TOTAL)>TWENTY_EPSILON_EB .AND. ABS(VEL)>TWENTY_EPSILON_EB)  THEN
   ...
   IF (SPEC_ID(1)/='null' .AND. SPEC_ID(2)=='null' .AND. MASS_FRACTION(1)<TWENTY_EPSILON_EB .AND. &
   ...
   IF (ANY(MASS_FLUX/=0._EB) .OR. ANY(MASS_FRACTION>0._EB)) THEN
      ...
      ELSE
         ...
         ENDDO
      ENDIF
      IF (SUM(SF%MASS_FRACTION) > TWENTY_EPSILON_EB) THEN
   ...
   IF (ABS(SF%VEL_T(1))>TWENTY_EPSILON_EB .OR. ABS(SF%VEL_T(2))>TWENTY_EPSILON_EB) SF%SPECIFIED_TANGENTIAL_VELOCITY = .TRUE.
   ...
   IF (ANY(ABS(SF%HEAT_SOURCE)>TWENTY_EPSILON_EB)) THEN
      DO NL=1,SF%N_LAYERS
   ...
   IF (SF%ADIABATIC .AND. (SF%NET_HEAT_FLUX < 1.E12_EB .OR. ABS(SF%CONVECTIVE_HEAT_FLUX)>TWENTY_EPSILON_EB)) THEN
         WRITE(MESSAGE,'(3A)') 'ERROR(336): SURF ',TRIM(SF%ID),' cannot use both ADIABATIC and NET or CONVECTIVE_HEAT_FLUX.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (SF%NET_HEAT_FLUX < 1.E12_EB .AND. ABS(SF%CONVECTIVE_HEAT_FLUX)>TWENTY_EPSILON_EB) THEN
   ...
   IF (SF%NET_HEAT_FLUX < 1.E12_EB .AND. TMP_FRONT >= -TMPM) THEN
      ...
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (ABS(SF%CONVECTIVE_HEAT_FLUX)>TWENTY_EPSILON_EB .AND. TMP_FRONT >= -TMPM) SF%SET_H =.TRUE.
   ...
   IF (ABS(SF%CONVECTIVE_HEAT_FLUX)>TWENTY_EPSILON_EB) SF%THERMAL_BC_INDEX = CONVECTIVE_FLUX_BC
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
         ...
         H_OR_M_IF: IF (SF%HRRPUA>0._EB) THEN
            IF (ANY(SF%MASS_FRACTION > 0._EB)) THEN
               ...
               DO NS=1,N_TRACKED_SPECIES
                  ...
                  ENDIF
               ENDDO
               IF (SUM(SF%MASS_FLUX) < TWENTY_EPSILON_EB) THEN
               ...
               DO NS=1,N_TRACKED_SPECIES
                  IF (SF%MASS_FLUX(NS) > TWENTY_EPSILON_EB) SF%RAMP(NS) = SF%RAMP(TIME_HEAT)
               ENDDO
         ...
         ELSEIF (SF%MLRPUA>0._EB) THEN H_OR_M_IF
            IF (ANY(SF%MASS_FRACTION > 0._EB)) THEN
               ...
               DO NS=1,N_TRACKED_SPECIES
                  ...
                  ENDIF
               ENDDO
               IF (FUEL_MF < TWENTY_EPSILON_EB) THEN
   ...
   IF_QDOTPP_REF: IF (SF%N_QDOTPP_REF > 0) THEN
      ...
      SF%Y_S_EFF = 0._EB
      SF%CHI_R_EFF = 0._EB
      MF_SUM = TWENTY_EPSILON_EB
      HR_SUM = TWENTY_EPSILON_EB
      IF (SOOT_INDEX > 0) THEN
         DO NR=1,N_REACTIONS
            RN => REACTION(NR)
   ...
   IF ((ABS(SF%SURFACE_DENSITY) <= TWENTY_EPSILON_EB) .AND. SF%BURN_AWAY) THEN
   ...
   IF (ANY(SF%MASS_FRACTION>0._EB) .AND. (ANY(ABS(SF%MASS_FLUX)>TWENTY_EPSILON_EB) .OR. SF%PYROLYSIS_MODEL/= PYROLYSIS_NONE)) THEN
   ...
   IF (ANY(SF%LEAK_PATH>=0) .AND. (BLOWING .OR. SUCKING .OR. SF%PYROLYSIS_MODEL/= PYROLYSIS_NONE)) THEN
      ...
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   IF (ANY(ABS(SF%MASS_FLUX)>TWENTY_EPSILON_EB) .AND. (BLOWING .OR. SUCKING)) THEN
   ...
   IF (ANY(ABS(SF%MASS_FLUX)>TWENTY_EPSILON_EB) .OR. &
   ...
   IF (SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FRACTION) THEN
      IF (ALL(ABS(SF%MASS_FRACTION)< TWENTY_EPSILON_EB)) &
...
SURF_GRID_LOOP: DO SURF_INDEX=0,N_SURF
   ...
   LAYER_LOOP: DO NL=1,SF%N_LAYERS
      ...
      IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
         ...
         IF (DENSITY_MIN>TWENTY_EPSILON_EB .AND. CAN_SWELL) SF%SWELL_RATIO(NL) = MAX(1._EB,SF%LAYER_DENSITY(NL)/DENSITY_MIN)
...
CALL DEFINE_PRES_METHOD(SOLVER)
...
ELSE
   ...
   IF (VELOCITY_TOLERANCE<TWENTY_EPSILON_EB) VELOCITY_TOLERANCE =  0.5_EB*CHARACTERISTIC_CELL_SIZE
   IF (PRESSURE_TOLERANCE<TWENTY_EPSILON_EB) PRESSURE_TOLERANCE = 20.0_EB/MIN(1._EB,CHARACTERISTIC_CELL_SIZE)**2
ENDIF
...
CHARACTER(LABEL_LENGTH) :: DIFFERENCING_SCHEME
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,DIFFERENCING_SCHEME,INITIAL_RADIATION_ITERATIONS,KAPPA0,&
                MIE_MINIMUM_DIAMETER,MIE_MAXIMUM_DIAMETER,MIE_NDG,NMIEANG,NUMBER_RADIATION_ANGLES,&
                OPTICALLY_THIN,PATH_LENGTH,QR_CLIP,RADIATION,RADIATION_ITERATIONS,RADTMP,RANDOMIZE_RADIATION_DIRECTIONS,&
...
! Spatial differencing scheme for the FVM radiation solver

SELECT CASE (TRIM(DIFFERENCING_SCHEME))
   CASE ('STEP')
      RAD_DIFF_SCHEME = 1
   CASE ('DIAMOND')
      RAD_DIFF_SCHEME = 2
   CASE ('EXPONENTIAL')
      RAD_DIFF_SCHEME = 3
   CASE DEFAULT
      WRITE(MESSAGE,'(A,A,A)') 'ERROR: DIFFERENCING_SCHEME, ',TRIM(DIFFERENCING_SCHEME),&
                               ', is not recognized. Use STEP, DIAMOND, or EXPONENTIAL.'
      CALL SHUTDOWN(MESSAGE) ; RETURN
END SELECT
...
LOGICAL :: EXTERNAL_FILE,DT_WARNING,CYCLING
TYPE(RAMPS_TYPE), POINTER :: RP
TYPE(RESERVED_RAMPS_TYPE), POINTER :: RRP
NAMELIST /RAMP/ CTRL_ID,CTRL_ID_DEP,CYCLING,DEVC_ID,DEVC_ID_DEP,EXTERNAL_FILE,F,FYI,ID,INITIAL_VALUE,NUMBER_INTERPOLATION_POINTS,&
                T,X,Z
...
READ_RAMP_LOOP: DO N=1,N_RAMP
   ...
   ELSE
      ...
      SEARCH_LOOP2: DO
         ...
         IF (CYCLING) THEN
            RP%CYCLING = CYCLING
            IF (RAMP_TYPE(N)/='TIME') THEN
               WRITE(MESSAGE,'(A,A,A)') 'ERROR(405): RAMP ',TRIM(RAMP_ID(N)),' CYCLING is only for time RAMPs.'
               CALL SHUTDOWN(MESSAGE) ; RETURN
            ENDIF
         ENDIF
...
USE COMPLEX_GEOMETRY, ONLY: IN_SPHERE_PT,IN_CYLINDER_PT,IN_CONE_PT,IN_OBB_PT
...
REAL(EB) :: TRANSPARENCY,XB1,XB2,XB3,XB4,XB5,XB6,BULK_DENSITY,VOL_ADJUSTED,VOL_SPECIFIED,UNDIVIDED_INPUT_AREA(3),&
            UNDIVIDED_INPUT_LENGTH(3),HEIGHT,RADIUS,XYZ(3),XYZCEN(3),ORIENTATION(3),ROTMAT(3,3),THETA,&
            LENGTH,WIDTH,SHAPE_AREA(3),XXI,YYJ,ZZK,DX_GHOST,DY_GHOST,DZ_GHOST,MATL_MASS_FRACTION(MAX_MATERIALS),&
            INTERNAL_HEAT_SOURCE,STRETCH_FACTOR,CELL_SIZE,CELL_SIZE_FACTOR
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               XB5 = MAX(XB5,ZS)
               XB6 = MIN(XB6,ZF)
               IF (XB1>XF+TWENTY_EPSILON_EB .OR. XB2<XS-TWENTY_EPSILON_EB .OR. &
                   XB3>YF+TWENTY_EPSILON_EB .OR. XB4<YS-TWENTY_EPSILON_EB .OR. &
                   XB5>ZF+TWENTY_EPSILON_EB .OR. XB6<ZS-TWENTY_EPSILON_EB .OR. REJECT_OBST) THEN
                  N = N-1
                  N_OBST = N_OBST-1
                  CYCLE I_MULT_LOOP
               ENDIF
               ...
               IF (SHAPE_TYPE>0) THEN
                  XYZCEN(1:3) = (/0.5_EB*(X(OB%I1)+X(OB%I2)),0.5_EB*(Y(OB%J1)+Y(OB%J2)),0.5_EB*(Z(OB%K1)+Z(OB%K2))/)
                  SELECT CASE (SHAPE_TYPE)
                     CASE (OBST_SPHERE_TYPE);   IS_INTERSECT = IN_SPHERE_PT(XYZ,RADIUS,XYZCEN)
                     CASE (OBST_CYLINDER_TYPE); IS_INTERSECT = IN_CYLINDER_PT(XYZ,HEIGHT,RADIUS,ROTMAT,XYZCEN)
                     CASE (OBST_CONE_TYPE);     IS_INTERSECT = IN_CONE_PT(XYZ,HEIGHT,RADIUS,ROTMAT,XYZCEN)
                     CASE (OBST_BOX_TYPE);      IS_INTERSECT = IN_OBB_PT(XYZ,LENGTH,WIDTH,HEIGHT,ROTMAT,XYZCEN)
                  END SELECT
               ...
               DO IOR=-3,3
                  ...
                  ENDDO
                  IF (ANY(OB%UNDIVIDED_INPUT_LENGTH<TWENTY_EPSILON_EB) .AND. SF%HT_DIM>1) THEN
               ...
               IF (BULK_DENSITY > 0._EB) THEN
                  ...
                  IF ((OB%UNDIVIDED_INPUT_LENGTH(1)<TWENTY_EPSILON_EB .OR. &
                       OB%UNDIVIDED_INPUT_LENGTH(2)<TWENTY_EPSILON_EB .OR. &
                       OB%UNDIVIDED_INPUT_LENGTH(3)<TWENTY_EPSILON_EB) .AND. .NOT.THICKEN) THEN
               ...
               DO NNN=1,MAX_MATERIALS
                  ...
                  IF (MATL_MASS_FRACTION(NNN)<TWENTY_EPSILON_EB) EXIT
                  FOUND = .FALSE.
                  DO NNNN=1,N_MATL
               ...
               OB%RAMP_IHS_INDEX = 0
               IF (ABS(OB%HEAT_SOURCE)>TWENTY_EPSILON_EB .AND. RAMP_IHS/='null') &
   ...
   READ_VENT_LOOP: DO
      ...
      ELSEIF (DB/='null') THEN
         ...
         SELECT CASE (DB)
            ...
            CASE('YMIN') ; XB(3:4) = YS_MIN+TWENTY_EPSILON_EB
            CASE('YMAX') ; XB(3:4) = YF_MAX-TWENTY_EPSILON_EB
            CASE('ZMIN') ; XB(5:6) = ZS_MIN+TWENTY_EPSILON_EB
            CASE('ZMAX') ; XB(5:6) = ZF_MAX-TWENTY_EPSILON_EB
            CASE DEFAULT
...
MESH_LOOP_2: DO NM=1,NMESHES
   ...
   VENT_LOOP_2: DO N=1,N_VENT
      ...
      IF (ABS(VT%UVW(ABS(VT%IOR))) < TWENTY_EPSILON_EB) THEN
...
IF (CYLINDRICAL .AND. XS<=TWENTY_EPSILON_EB) THEN
   ...
   IF (ANY(XYZ>-100000._EB)) THEN
      ...
      MESH_LOOP: DO NM=1,NMESHES
         ...
         IF (XYZ(1)>=M%XS .AND. XYZ(1)<=M%XF .AND. XYZ(2)>=M%YS .AND.  XYZ(2)<=M%YF .AND. XYZ(3)>=M%ZS .AND. XYZ(3)<=M%ZF) THEN
            IF (ABS(XYZ(1)-M%XS)<50._EB*TWENTY_EPSILON_EB) XYZ(1) = XYZ(1) + 0.01_EB*M%DXI
            IF (ABS(XYZ(1)-M%XF)<50._EB*TWENTY_EPSILON_EB) XYZ(1) = XYZ(1) - 0.01_EB*M%DXI
            IF (ABS(XYZ(2)-M%YS)<50._EB*TWENTY_EPSILON_EB) XYZ(2) = XYZ(2) + 0.01_EB*M%DETA
            IF (ABS(XYZ(2)-M%YF)<50._EB*TWENTY_EPSILON_EB) XYZ(2) = XYZ(2) - 0.01_EB*M%DETA
            IF (ABS(XYZ(3)-M%ZS)<50._EB*TWENTY_EPSILON_EB) XYZ(3) = XYZ(3) + 0.01_EB*M%DZETA
            IF (ABS(XYZ(3)-M%ZF)<50._EB*TWENTY_EPSILON_EB) XYZ(3) = XYZ(3) - 0.01_EB*M%DZETA
            EXIT MESH_LOOP
         ENDIF
      ENDDO MESH_LOOP
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IN%DRY           = DRY
            IF (HRRPUV > TWENTY_EPSILON_EB) INIT_HRRPUV = .TRUE.
            ...
            IF (IN%MASS_PER_VOLUME>TWENTY_EPSILON_EB .AND. IN%VOLUME<TWENTY_EPSILON_EB) THEN
...
DO NN=1,N_INIT
   ...
   IF (IN%MASS_PER_TIME>0._EB .OR. IN%MASS_PER_VOLUME>0._EB .OR. IN%BULK_DENSITY_FILE/='null') THEN
      ...
      IF (LPC%SURF_INDEX>0) THEN
         ...
         IF (MOIST_CONT>TWENTY_EPSILON_EB .AND. IN%DRY) THEN
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   IF (QUANTITY=='FIRE DEPTH'.AND.(QUANTITY_RANGE(1)-1.E20_EB)<TWENTY_EPSILON_EB) &
      QUANTITY_RANGE(1) = MIN(200._EB,20._EB/CHARACTERISTIC_CELL_SIZE)
   ...
   IF (STATISTICS_END>1.E6_EB) THEN
      STATISTICS_END = T_END + TWENTY_EPSILON_EB
   ELSE
   ...
   IF (NORM2(ORIENTATION)<1.E5) THEN
      ...
      DO I=1,N_ORIENTATION_VECTOR
         IF (ABS(ORIENTATION(1)-ORIENTATION_VECTOR(1,I))<TWENTY_EPSILON_EB .AND. &
             ABS(ORIENTATION(2)-ORIENTATION_VECTOR(2,I))<TWENTY_EPSILON_EB .AND. &
             ABS(ORIENTATION(3)-ORIENTATION_VECTOR(3,I))<TWENTY_EPSILON_EB .AND. &
             ABS(COS_HALF_VIEW_ANGLE(I)-VIEW_ANGLE)<TWENTY_EPSILON_EB) THEN
   ...
   IF (NEW_ORIENTATION_VECTOR) THEN
      ...
      IF (N_ORIENTATION_VECTOR>UBOUND(ORIENTATION_VECTOR,DIM=2)) THEN
         ...
         COS_HALF_VIEW_ANGLE => REALLOCATE(COS_HALF_VIEW_ANGLE,0,N_ORIENTATION_VECTOR+10)
      ENDIF
      IF (ALL(ABS(ORIENTATION(1:3))<TWENTY_EPSILON_EB)) THEN
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (XB(1)>-1.E5_EB) THEN
         ...
         CHECK_MESH_LOOP: DO NM=1,NMESHES
            ...
            IF (XB(1)/=XB(2) .AND. (MIN(MAX(XB(1),XB(2)),M%XF)-MAX(MIN(XB(1),XB(2)),M%XS)<TWENTY_EPSILON_EB)) &
               OVERLAPPING_X = .FALSE.
            IF (XB(3)==XB(4) .AND. (XB(3)> M%YF .OR. XB(4)< M%YS)) OVERLAPPING_Y = .FALSE.
            IF (XB(3)/=XB(4) .AND. (MIN(MAX(XB(3),XB(4)),M%YF)-MAX(MIN(XB(3),XB(4)),M%YS)<TWENTY_EPSILON_EB)) &
               OVERLAPPING_Y = .FALSE.
            IF (XB(5)==XB(6) .AND. (XB(5)> M%ZF .OR. XB(6)< M%ZS)) OVERLAPPING_Z = .FALSE.
            IF (XB(5)/=XB(6) .AND. (MIN(MAX(XB(5),XB(6)),M%ZF)-MAX(MIN(XB(5),XB(6)),M%ZS)<TWENTY_EPSILON_EB)) &
               OVERLAPPING_Z = .FALSE.
            ...
            IF (XB(1)==XB(2) .AND. OVERLAPPING_X) THEN
               IF (XYZ(1)==M%XS) CALL SEARCH_OTHER_MESHES(XYZ(1)-TWENTY_EPSILON_EB,XYZ(2),XYZ(3),NOM,IIG,JJG,KKG,XI,YJ,ZK)
               IF (NOM>0) OVERLAPPING_X = .FALSE. ! Device will be assigned to NOM
            ENDIF
            IF (XB(3)==XB(4) .AND. OVERLAPPING_Y) THEN
               IF (XYZ(2)==M%YS) CALL SEARCH_OTHER_MESHES(XYZ(1),XYZ(2)-TWENTY_EPSILON_EB,XYZ(3),NOM,IIG,JJG,KKG,XI,YJ,ZK)
               IF (NOM>0) OVERLAPPING_Y = .FALSE.
            ENDIF
            IF (XB(5)==XB(6) .AND. OVERLAPPING_Z) THEN
               IF (XYZ(3)==M%ZS) CALL SEARCH_OTHER_MESHES(XYZ(1),XYZ(2),XYZ(3)-TWENTY_EPSILON_EB,NOM,IIG,JJG,KKG,XI,YJ,ZK)
      ...
      MESH_LOOP: DO NM=1,NMESHES
         ...
         ENDIF
         IF (XYZ(1)>=M%XS .AND. XYZ(1)<=M%XF .AND. XYZ(2)>=M%YS .AND. XYZ(2)<=M%YF .AND. XYZ(3)>=M%ZS .AND. XYZ(3)<=M%ZF) THEN
            IF (ABS(XYZ(1)-M%XS)<TWENTY_EPSILON_EB .AND. SPATIAL_STATISTIC/='INTERPOLATION') THEN
               IF (IOR==-1) THEN
                  CYCLE MESH_LOOP
               ELSE
               ...
               ENDIF
            ENDIF
            IF (ABS(XYZ(1)-M%XF)<TWENTY_EPSILON_EB .AND. SPATIAL_STATISTIC/='INTERPOLATION') THEN
               IF (IOR==1) THEN
                  CYCLE MESH_LOOP
               ELSE
                  XYZ(1) = XYZ(1) - 0.01_EB*M%DXI
               ENDIF
            ENDIF
            IF (ABS(XYZ(2)-M%YS)<TWENTY_EPSILON_EB .AND. SPATIAL_STATISTIC/='INTERPOLATION') THEN
               IF (IOR==-2) THEN
                  CYCLE MESH_LOOP
               ELSE
               ...
               ENDIF
            ENDIF
            IF (ABS(XYZ(2)-M%YF)<TWENTY_EPSILON_EB .AND. SPATIAL_STATISTIC/='INTERPOLATION') THEN
               IF (IOR==2) THEN
                  CYCLE MESH_LOOP
               ELSE
                  XYZ(2) = XYZ(2) - 0.01_EB*M%DETA
               ENDIF
            ENDIF
            IF (ABS(XYZ(3)-M%ZS)<TWENTY_EPSILON_EB .AND. SPATIAL_STATISTIC/='INTERPOLATION') THEN
               IF (IOR==-3) THEN
                  CYCLE MESH_LOOP
               ELSE
               ...
               ENDIF
            ENDIF
            IF (ABS(XYZ(3)-M%ZF)<TWENTY_EPSILON_EB .AND. SPATIAL_STATISTIC/='INTERPOLATION') THEN
               IF (IOR==3) THEN
                  CYCLE MESH_LOOP
               ELSE
                  XYZ(3) = XYZ(3) - 0.01_EB*M%DZETA
...
IF (ABS(TIME_SHRINK_FACTOR-1._EB)>TWENTY_EPSILON_EB .AND. TSF_WARNING) THEN
...
REAL(EB) :: XYZ(3),DEPTHS(20)
CHARACTER(LABEL_LENGTH) :: QUANTITY,INIT_ID,MATL_ID,LABEL_DUM(2),PART_ID
LOGICAL :: CELL_CENTERED
TYPE (PROFILE_TYPE), POINTER :: PF
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
NAMELIST /PROF/ CELL_CENTERED,DEPTHS,FORMAT_INDEX,FYI,ID,INIT_ID,IOR,LP_TAG,MATL_ID,PART_ID,QUANTITY,XYZ
...
PROF_LOOP: DO NN=1,N_PROFO
   ...
   PF%DEPTHS       = DEPTHS
   PF%N_DEPTHS     = MINLOC(DEPTHS,MASK=DEPTHS<0._EB,DIM=1)-1
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,GEOM_INDEX,IOR,IND2
LOGICAL :: VECTOR,CELL_CENTERED,DEBUG,CULL_SLICE,DRY
CHARACTER(LABEL_LENGTH) :: QUANTITY,SPEC_ID,PART_ID,PROP_ID,REAC_ID,SLICETYPE
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IOR = 0
      IF (ABS(XB(1)-XB(2))<TWENTY_EPSILON_EB) IOR = 1
      IF (ABS(XB(3)-XB(4))<TWENTY_EPSILON_EB) IOR = 2
      IF (ABS(XB(5)-XB(6))<TWENTY_EPSILON_EB) IOR = 3
