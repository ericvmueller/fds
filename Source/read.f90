...
NAMELIST /MESH/ CHECK_MESH_ALIGNMENT,COLOR,CYLINDRICAL,FYI,ID,IJK,LEVEL,MPI_PROCESS,MULT_ID,RGB,TRNX_ID,TRNY_ID,TRNZ_ID,XB
...
NAMELIST /PRES/ BAROCLINIC,CHECK_POISSON,FISHPAK_BC,ITERATION_SUSPEND_FACTOR,INSEPARABLE_POISSON, &
                SCARC_COARSE, SCARC_KRYLOV_ITERATIONS, SCARC_KRYLOV_TOLERANCE, SCARC_MESH, SCARC_METHOD, &
                SCARC_MGM_BOUNDARY, SCARC_MGM_ITERATIONS, SCARC_MGM_LAPLACE_SOLVER, SCARC_MGM_TOLERANCE, &
                SCARC_PRECISION, SCARC_PRECON, SCARC_VERBOSE, &
...
NAMELIST /INIT/ BULK_DENSITY_FILE,CELL_CENTERED,CROWN_BASE_HEIGHT,CROWN_BASE_WIDTH,CTRL_ID,DB,DENSITY,DEVC_ID,&
                DIAMETER,DT_INSERT,DX,DY,DZ,HEIGHT,HRRPUV,ID,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,N_PARTICLES,&
         ...
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IF (IN%BULK_DENSITY_FILE/='null') THEN
               ...
               IF (IN%PACKING_RATIO>0._EB) THEN 
                  WRITE(MESSAGE,'(A,I0,A)') 'WARNING: INIT ',N,' PACKING_RATIO ignored in favor of BULK_DENSITY_FILE'
                  IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
               ENDIF
...
DO NN=1,N_INIT
   IN=>INITIALIZATION(NN)
   IF (IN%MASS_PER_TIME>0._EB .OR. IN%MASS_PER_VOLUME>0._EB .OR. IN%PACKING_RATIO>0._EB .OR. IN%BULK_DENSITY_FILE/='null') THEN
      LPC => LAGRANGIAN_PARTICLE_CLASS(IN%PART_INDEX)
      IF (LPC%DENSITY < 0._EB) THEN
