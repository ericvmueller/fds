...
GEOM_SLICE_AVG       = 0       ! test parameter
...
IF (GEOM_SLICE_AVG .LT. 0 .OR. GEOM_SLICE_AVG > 2 ) GEOM_SLICE_AVG = 0
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   IF (ML%I_RAMP_K_S > 0) THEN
      DO I=0,I_MAX_TEMP
         ML%K_S(I)=EVALUATE_RAMP(REAL(I,EB),0._EB,ML%I_RAMP_K_S)
      ENDDO
   ENDIF
   ...
   IF (ML%I_RAMP_C_S > 0) THEN
      ...
      DO I=0,I_MAX_TEMP
         ML%C_S(I)=EVALUATE_RAMP(REAL(I,EB),0._EB,ML%I_RAMP_C_S)
      ENDDO
   ENDIF
   ...
   DO NR=1,ML%N_REACTIONS
      IF (ML%I_RAMP_H_R(NR)>0) THEN
         ...
         ENDIF
         DO I=0,I_MAX_TEMP
            ML%H_R(NR,I)=EVALUATE_RAMP(REAL(I,EB),0._EB,ML%I_RAMP_H_R(NR))
         ENDDO
      ENDIF
   ENDDO
