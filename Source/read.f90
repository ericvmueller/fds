...
INTEGER :: N
CHARACTER(LABEL_LENGTH) :: RAMP_FVX_T,RAMP_FVY_T,RAMP_FVZ_T,RAMP_W0_T,&
...
NAMELIST /WIND/ CORIOLIS_VECTOR,DIRECTION,FORCE_VECTOR,FYI,GEOSTROPHIC_WIND,GROUND_LEVEL,L,LAPSE_RATE,LATITUDE,&
                ...
                RAMP_FVZ_T,RAMP_SPEED_T,RAMP_SPEED_Z,RAMP_TMP0_Z,RAMP_U0_Z,RAMP_V0_Z,&
...
IF (RAMP_U0_Z/='null') THEN  ! create dummy profile RAMPs and fill in later
   RAMP_SPEED_Z = RAMP_U0_Z
   RAMP_DIRECTION_Z = RAMP_U0_Z
ENDIF
IF (RAMP_V0_Z/='null') THEN  ! create dummy profile RAMPs and fill in later
   RAMP_SPEED_Z = RAMP_V0_Z
   RAMP_DIRECTION_Z = RAMP_V0_Z
ENDIF
...
IF (RAMP_SPEED_Z/='null') CALL GET_RAMP_INDEX(RAMP_SPEED_Z,'PROFILE',I_RAMP_SPEED_Z,DUPLICATE_RAMP=.TRUE.)
IF (RAMP_DIRECTION_T/='null') CALL GET_RAMP_INDEX(RAMP_DIRECTION_T,'TIME',I_RAMP_DIRECTION_T)
IF (RAMP_DIRECTION_Z/='null') CALL GET_RAMP_INDEX(RAMP_DIRECTION_Z,'PROFILE',I_RAMP_DIRECTION_Z,DUPLICATE_RAMP=.TRUE.)
...
! This short routine takes Cartesian profiles for U0 and V0 and converts to speed and direction

REAL(EB) :: THETA,SPEED,SPEED_FACTOR,SPEED_X,SPEED_Y
INTEGER :: I,N_PTS

SPEED = SQRT(U0**2+V0**2)
IF (SPEED>TWO_EPSILON_EB .AND. (I_RAMP_U0_Z/=0 .OR. I_RAMP_V0_Z/=0)) THEN
   SPEED_X = U0
   SPEED_Y = V0
   IF (I_RAMP_U0_Z/=0) N_PTS = RAMPS(I_RAMP_U0_Z)%NUMBER_INTERPOLATION_POINTS+1
   IF (I_RAMP_V0_Z/=0) N_PTS = RAMPS(I_RAMP_V0_Z)%NUMBER_INTERPOLATION_POINTS+1
   DO I=0,N_PTS
      IF (I_RAMP_U0_Z/=0) SPEED_X = RAMPS(I_RAMP_U0_Z)%INTERPOLATED_DATA(I)*U0
      IF (I_RAMP_V0_Z/=0) SPEED_Y = RAMPS(I_RAMP_V0_Z)%INTERPOLATED_DATA(I)*V0
      SPEED_FACTOR = SQRT(SPEED_X**2+SPEED_Y**2)/SPEED
      THETA = 270._EB-ATAN2(SPEED_Y,SPEED_X)/DEG2RAD
      RAMPS(I_RAMP_SPEED_Z)%INTERPOLATED_DATA(I) = SPEED_FACTOR
      RAMPS(I_RAMP_DIRECTION_Z)%INTERPOLATED_DATA(I) = THETA
   ENDDO
   U0 = SPEED
   V0 = SPEED
ENDIF
...
REAL(EB), DIMENSION(MAX_REACTIONS) :: A,E,HEATING_RATE,PYROLYSIS_RANGE,HEAT_OF_REACTION, &
          ...
          THRESHOLD_SIGN,GAS_DIFFUSION_DEPTH,NU_O2_CHAR,BETA_CHAR,MAX_REACTION_RATE
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ALLOW_SHRINKING,ALLOW_SWELLING,BETA_CHAR,BOILING_TEMPERATURE,&
                ...
                MAX_REACTION_RATE,MW,N_O2,N_REACTIONS,N_S,N_T,NU_MATL,NU_O2_CHAR,NU_PART,NU_SPEC,PART_ID,PCR,&
                PERMEABILITY,POROSITY,PYROLYSIS_RANGE,REFERENCE_RATE,REFERENCE_TEMPERATURE,REFRACTIVE_INDEX,RGB,&
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   ML%K_S                  = CONDUCTIVITY
   ML%MAX_REACTION_RATE(:) = MAX_REACTION_RATE(:)
   ML%MW                   = MW
...
CHARACTER(LABEL_LENGTH) :: PART_ID,RAMP_MF(MAX_SPECIES),RAMP_Q,RAMP_V,RAMP_T,MATL_ID(MAX_LAYERS,MAX_MATERIALS),&
                 ...
                 SPEC_ID(MAX_SPECIES),RAMP_T_B,RAMP_T_I,RAMP_V_X,RAMP_V_Y,RAMP_V_Z,NEAR_WALL_TURBULENCE_MODEL
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            Z_0,PARTICLE_EXTRACTION_VELOCITY,RENODE_DELTA_T(MAX_LAYERS),NEAR_WALL_EDDY_VISCOSITY
...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BLOWING,BURN_AWAY,BURN_DURATION,&
                ...
                N_LAYER_CELLS_MAX,NEAR_WALL_EDDY_VISCOSITY,NEAR_WALL_TURBULENCE_MODEL,NET_HEAT_FLUX,NO_SLIP,NPPC,PACKING_RATIO,&
...
NAMELIST /ZONE/ ID,LEAK_AREA,LEAK_PRESSURE_EXPONENT,LEAK_REFERENCE_PRESSURE,PERIODIC,XYZ
