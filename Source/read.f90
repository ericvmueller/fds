...
INTEGER  :: I,J,K,IZERO,IOS,N_INIT_RESERVED,MAX_LEAK_PATHS,I_DUM(10),IERROR,N_EVAC_ZONE,N_CONE_RAMP=0
...
CALL READ_TRNF    ; IF (STOP_STATUS==SETUP_STOP) RETURN
...
CALL PROC_INIT    ; IF (STOP_STATUS==SETUP_STOP) RETURN
...
IF (U_STAR>0._EB) SPEED = U_STAR*LOG(Z_REF/Z_0)/VON_KARMAN_CONSTANT
...
IF (STRATIFICATION) THEN
   ...
   IF (HVAC_SOLVE) THEN
      ZSW = MIN(ZS_MIN,MINVAL(DUCTNODE%XYZ(3)))
      ZFW = MAX(ZF_MAX,MAXVAL(DUCTNODE%XYZ(3)))
   ELSE
      ZSW = ZS_MIN
      ZFW = ZF_MAX
   ENDIF
   ...
   IF (ABS(OBUKHOV_LENGTH)>1.E-10_EB) THEN
      ...
      IF (U_STAR<0._EB) U_STAR = VON_KARMAN_CONSTANT*SPEED/LOG(Z_REF/Z_0)
      ...
      ENDIF
      RHO_REF = 1.2_EB
      P_REF = P_INF - RHO_REF*GRAV*(Z_REF-Z_0)
      THETA_REF = TMP_REF*(P_INF/P_REF)**0.286_EB
      IF (ABS(THETA_STAR)<1.E-10_EB) THEN
         THETA_0 = THETA_REF/(1._EB+U_STAR**2*LOG(Z_REF/Z_0)/(GRAV*VON_KARMAN_CONSTANT**2*OBUKHOV_LENGTH))
         THETA_STAR = U_STAR**2*THETA_0/(GRAV*VON_KARMAN_CONSTANT*OBUKHOV_LENGTH)
      ELSE
         THETA_0 = THETA_REF - THETA_STAR*LOG(Z_REF/Z_0)/VON_KARMAN_CONSTANT
      ENDIF
      TMPA = THETA_0  ! Make the ground temperature the new ambient temperature
      DO I=1,51
         ZETA = (I-1)*(ZFW-ZSW)/50.
         ZZZ  = Z_0*EXP(LOG(ZFW/Z_0)*(ZETA-ZSW)/(ZFW-ZSW))
...
REAL(EB) :: DIAMETER, GAMMA_D,AGE,INITIAL_TEMPERATURE,HEAT_OF_COMBUSTION, &
            ...
            PRIMARY_BREAKUP_DRAG_REDUCTION_FACTOR,HEAT_TRANSFER_COEFFICIENT_SOLID
...
LOGICAL :: TARGET_ONLY,MASSLESS,STATIC,MONODISPERSE,BREAKUP,CHECK_DISTRIBUTION,TURBULENT_DISPERSION,&
           SECOND_ORDER_PARTICLE_TRANSPORT,EMBER_PARTICLE
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                FREE_AREA_FRACTION,FYI,GAMMA_D,HEAT_OF_COMBUSTION,HEAT_TRANSFER_COEFFICIENT_SOLID,&
                HORIZONTAL_VELOCITY,ID,INITIAL_TEMPERATURE,KILL_DIAMETER,MASSLESS,MAXIMUM_DIAMETER,MINIMUM_DIAMETER,MONODISPERSE,&
...
LOGICAL :: DEFAULT,EVAC_DEFAULT,VEG_LSET_SPREAD,VEG_LSET_TAN2,TGA_ANALYSIS,COMPUTE_EMISSIVITY,&
           COMPUTE_EMISSIVITY_BACK,HT3D,THERM_THICK,BLOWING

NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BLOWING,BURN_AWAY,BURN_DURATION,CELL_SIZE_FACTOR,&
                ...
                LEAK_PATH,LEAK_PATH_ID,LENGTH,MASS_FLUX,MASS_FLUX_TOTAL,MASS_FLUX_VAR,MASS_FRACTION,MASS_TRANSFER_COEFFICIENT, &
                ...
                VEG_LSET_BETA,VEG_LSET_FUEL_INDEX,VEG_LSET_HT,VEG_LSET_IGNITE_TIME,&
                ...
                VEG_LSET_TAN2,VEG_LSET_WIND_EXP,VEG_LSET_SURF_LOAD,VEG_LSET_FIREBASE_TIME,VEG_LSET_CHAR_FRACTION,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (VEG_LSET_FIREBASE_TIME == 0._EB) VEG_LSET_FIREBASE_TIME = 75600._EB/VEG_LSET_SIGMA
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (ANY(SF%LEAK_PATH_ID/='null'))  THEN
      ...
      ZONE_LOOP: DO NR = 1, N_ZONE-N_EVAC_ZONE
   ...
   IF (SF%LEAK_PATH(1)>N_ZONE-N_EVAC_ZONE .OR. SF%LEAK_PATH(2)>N_ZONE-N_EVAC_ZONE) THEN
...
USE SCRC, ONLY: SCARC_METHOD, SCARC_GRID, SCARC_MATRIX, SCARC_ERROR_FILE, SCARC_ACCURACY, &
                ...
                SCARC_MULTIGRID_PRESMOOTH, SCARC_MULTIGRID_POSTSMOOTH, SCARC_MKL_PRECISION, &
...
NAMELIST /PRES/ BAROCLINIC,CHECK_POISSON,FISHPAK_BC,ITERATION_SUSPEND_FACTOR,LAPLACE_PRESSURE_CORRECTION, &
                SCARC_METHOD, SCARC_GRID, SCARC_MATRIX, SCARC_ERROR_FILE, SCARC_ACCURACY, &
                SCARC_KRYLOV, SCARC_KRYLOV_ITERATIONS, SCARC_KRYLOV_ACCURACY, SCARC_KRYLOV_INTERPOL, &
                SCARC_MULTIGRID, SCARC_MULTIGRID_ITERATIONS, SCARC_MULTIGRID_ACCURACY, SCARC_TWOLEVEL, &
                SCARC_MULTIGRID_CYCLE, SCARC_MULTIGRID_LEVEL, SCARC_MULTIGRID_COARSENING, SCARC_MULTIGRID_INTERPOL, &
                SCARC_MULTIGRID_PRESMOOTH, SCARC_MULTIGRID_POSTSMOOTH, SCARC_MKL_PRECISION, &
...
LOGICAL  :: CELL_CENTERED,UNIFORM,RTE_CORRECTION
...
NAMELIST /INIT/ CELL_CENTERED,CROWN_BASE_HEIGHT,CROWN_BASE_WIDTH,CTRL_ID,DB,DENSITY,DEVC_ID,DIAMETER,DT_INSERT,DX,DY,DZ,&
                ...
                RADIUS,RAMP_Q,RTE_CORRECTION,SHAPE,SPEC_ID,TEMPERATURE,T_INSERT,TREE_HEIGHT,UNIFORM,UVW,VOLUME_FRACTION,XB,XYZ
N_INIT = 0
N_INIT_READ = 0
REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
...
INTEGER  :: N,NM,NN,N_EVAC_MESH,NM_EVAC,IERR
...
READ_EVACUATION_ZONE_LOOP: DO N=N_ZONE-N_EVAC_ZONE+1,N_ZONE
   ...
   IF (N<1000) WRITE(ID,'(A,I3)') 'EZONE_',N
   IF (N<100)  WRITE(ID,'(A,I2)') 'EZONE_',N
   IF (N<10)   WRITE(ID,'(A,I1)') 'EZONE_',N
   LEAK_AREA     = 0._EB
   LEAK_REFERENCE_PRESSURE = 4._EB
   LEAK_PRESSURE_EXPONENT = 0.5_EB
   PERIODIC      = .FALSE.
...
! this routine is used to split a slice file into finer and finer sub-regions until at the last step each region is a grid cell
! I_IND and NI are intially defined to be NI=2, I_IND(1)=1, I_IND(2)=NI_ALL
