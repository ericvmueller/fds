...
VENT_LOOP: DO N=1,M%N_VENT
   ...
   EDDY_IF: IF (VT%N_EDDY>0) THEN
      ...
      ! DFSEM arrays (allocated if SIGMA_DFSEM is set)
      IF (SUM(ABS(VT%SIGMA_DFSEM)) > TWO_EPSILON_EB) THEN
         ALLOCATE(VT%ALPHA_EDDY(3,VT%N_EDDY),STAT=IZERO) ; CALL ChkMemErr('INIT','ALPHA_EDDY',IZERO)
         ALLOCATE(VT%POSITION0_EDDY(3,VT%N_EDDY),STAT=IZERO) ; CALL ChkMemErr('INIT','POSITION0_EDDY',IZERO)
         VT%ALPHA_EDDY=0._EB
         VT%POSITION0_EDDY=0._EB
         ! SIGMA_DFSEM, C1_DFSEM, and C2_DFSEM should be set in read.f90
      ENDIF
      VT%U_EDDY=0._EB
      VT%V_EDDY=0._EB
      VT%W_EDDY=0._EB
