...
PUBLIC INITIALIZE_MESH_VARIABLES_1,INITIALIZE_MESH_VARIABLES_2,INITIALIZE_GLOBAL_VARIABLES,OPEN_AND_CLOSE,INITIAL_NOISE,UVW_INIT, &
       INITIALIZE_DEVICES,INITIALIZE_PROFILES
...
IF (CORRECT_SUBGRID_TEMPERATURE) THEN
   ALLOCATE(M%SGS_TMP_COR(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','SGS_TMP_COR',IZERO)
ENDIF
...
IF (.NOT.EVACUATION_ONLY(NM)) WC%ONE_D%PRESSURE_ZONE = M%PRESSURE_ZONE(IIG,JJG,KKG)
